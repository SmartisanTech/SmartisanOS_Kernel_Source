...
/* CCM0009 add mbhc fs debug interface */
#ifdef WCD_MBHC_DEBUG_FS
...
/* CCM0007 bringup analog headset */
// used to record the uart gpio state before inserting the headet.
static int g_uart_gpio_original_cansleep = 0;

static int wcd_mbhc_usbc_ana_control_uart(struct wcd_mbhc *mbhc, bool active)
{
	int rc = 0;
	struct snd_soc_codec *codec = mbhc->codec;
	struct snd_soc_card *card = codec->component.card;
	struct device_node *usbc_uart_en_gpio_p = NULL;
	int usbc_uart_en_gpio = 10; // As schematic SW_SEL_UART GPIO_10
	int uart_gpio_cansleep = 0;

	pr_debug("%s: active = %d\n", __func__, active);

	usbc_uart_en_gpio_p = of_parse_phandle(card->dev->of_node,
			"qcom,usbc-analog-uart-en-gpio", 0);
	if (!usbc_uart_en_gpio_p) {
		pr_err("%s: property %s not detected in node %s\n", __func__,
				"qcom,usbc-analog-uart-en-gpio", card->dev->of_node->full_name);
		return -EINVAL;
	}

	uart_gpio_cansleep = gpio_get_value_cansleep(usbc_uart_en_gpio);
	pr_debug("%s: uart_gpio_cansleep = %d, g_uart_gpio_original_cansleep = %d\n",
		   __func__, uart_gpio_cansleep, g_uart_gpio_original_cansleep);
	if (active) {
		// disable uart when insert headset
		if (uart_gpio_cansleep) {
			msm_cdc_pinctrl_select_active_state(usbc_uart_en_gpio_p);
			g_uart_gpio_original_cansleep = uart_gpio_cansleep;
		}
	} else {
		// resume uart gpio status after remove headset
		if (g_uart_gpio_original_cansleep &&
			   (uart_gpio_cansleep != g_uart_gpio_original_cansleep)) {
			msm_cdc_pinctrl_select_sleep_state(usbc_uart_en_gpio_p);
			g_uart_gpio_original_cansleep = uart_gpio_cansleep;
		}
	}
...
static int wcd_mbhc_usbc_ana_force_detect(struct wcd_mbhc *mbhc, bool active)
{
	int rc = 0;
	struct snd_soc_codec *codec = mbhc->codec;
	struct snd_soc_card *card = codec->component.card;
	struct device_node *usbc_analog_force_detect_gpio_p = NULL;

	pr_debug("%s: active = %d\n", __func__, active);

	usbc_analog_force_detect_gpio_p = of_parse_phandle(card->dev->of_node,
			"qcom,usbc-analog-force-detect-gpio", 0);
	if (!usbc_analog_force_detect_gpio_p) {
		pr_err("%s: property %s not detected in node %s", __func__,
				"qcom,usbc-analog-force-detect-gpio", card->dev->of_node->full_name);
		return -EINVAL;
	}

	if (active) {
		//force detect headset
		pr_debug("%s: active force detect gpio\n", __func__);
		msm_cdc_pinctrl_select_active_state(usbc_analog_force_detect_gpio_p);
	} else {
		pr_debug("%s: sleep force detect gpio\n", __func__);
		msm_cdc_pinctrl_select_sleep_state(usbc_analog_force_detect_gpio_p);
	}
					   ...
					   unsigned long mode, void *ptr)
{
	...
	if (mode == POWER_SUPPLY_TYPEC_SINK_AUDIO_ADAPTER) {
		...
		/* CCM0007 Start: disable uart and force detect */
		wcd_mbhc_usbc_ana_control_uart(mbhc, true);
		wcd_mbhc_usbc_ana_force_detect(mbhc, true);
	} else if (mode == POWER_SUPPLY_TYPEC_NONE) {
		wcd_mbhc_usbc_ana_force_detect(mbhc, false);
		wcd_mbhc_usbc_ana_control_uart(mbhc, false);
		/* CCM0007 End */
	}
