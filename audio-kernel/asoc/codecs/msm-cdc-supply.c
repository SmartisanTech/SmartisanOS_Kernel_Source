 ...
 * @cdc_vreg: pointer to platform regulator data
 * @num_supplies: number of supplies
 * @supply_name: supply name to be enabled
 * @enable_ear_pa: indicates enable or disable ear pa
 *
 * Return error code if supply update failed
 */
int msm_cdc_update_supply(struct device *dev,
				    struct regulator_bulk_data *supplies,
				    struct cdc_regulator *cdc_vreg,
				    int num_supplies,
				    char *supply_name,
					int enable_ear_pa)
{
	int rc, i;
	int current_uV;
	unsigned int current_mode, expect_mode;

	if ((!supply_name) || (!supplies) || (!cdc_vreg)) {
		pr_err("%s: either dev or supplies or cdc_vreg is NULL\n",
				__func__);
		return -EINVAL;
	}
	...
	for (i = 0; i < num_supplies; i++) {
		if (!strcmp(cdc_vreg[i].name, supply_name)) {
			current_uV = regulator_get_voltage(supplies[i].consumer);
			current_mode = regulator_get_mode(supplies[i].consumer);
			dev_dbg(dev, "%s: regulator current_uV = %d, current_mode = %u, enable_ear_pa = %d\n",
					__func__, current_uV, current_mode, enable_ear_pa);
			if (enable_ear_pa) {
				expect_mode = REGULATOR_MODE_FAST;
				cdc_vreg[i].min_uV = 3700000; //3.7V
				cdc_vreg[i].max_uV = 3700000;
			} else {
				expect_mode = REGULATOR_MODE_STANDBY;
				cdc_vreg[i].min_uV = 3300000; //3.3V
				cdc_vreg[i].max_uV = 3300000;
			}
			dev_dbg(dev, "%s: set regulator expect_mode = %u, min_uV = %d, max_uV = %d\n",
					__func__, expect_mode, cdc_vreg[i].min_uV, cdc_vreg[i].max_uV);
			// set regulator voltage
			rc = regulator_set_voltage(supplies[i].consumer,
				 cdc_vreg[i].min_uV,
				 cdc_vreg[i].max_uV);
			if (rc) {
				dev_err(dev, "%s: set regulator voltage failed for %s, err:%d\n",
			...
			// set regulator mode
			rc = regulator_set_mode(supplies[i].consumer, expect_mode);
			if (rc) {
				dev_err(dev, "%s: set regulator mode failed for %s, err:%d\n",
						__func__, supplies[i].supply, rc);
				break;
			}
			break;
...
EXPORT_SYMBOL(msm_cdc_update_supply);
/* CCM1007 End */
