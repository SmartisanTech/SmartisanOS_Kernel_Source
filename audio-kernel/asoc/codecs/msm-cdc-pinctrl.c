...
static int msm_cdc_pinctrl_probe(struct platform_device *pdev)
{
	...
	/* CCM0007 skip setting to sleep state for uart gpio */
	gpio_data->gpio = of_get_named_gpio(pdev->dev.of_node,
						"qcom,analog_uart_en_gpio", 0);
	/* skip setting to sleep state for LPI_TLMM GPIOs */
	if (!of_property_read_bool(pdev->dev.of_node, "qcom,lpi-gpios") && (uart_gpio != gpio_data->gpio)) {
