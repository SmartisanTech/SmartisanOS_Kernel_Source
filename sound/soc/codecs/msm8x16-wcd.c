				     ...
				     struct snd_kcontrol *kcontrol, int event)
{
	...
	switch (event) {
	case SND_SOC_DAPM_POST_PMU:
		gpio_set_value_cansleep(pdata->ext_pa_en_gpio, 1);
		for (i = 0; i < mode; i++) {
			//udelay(1);
...
static int msm8x16_wcd_codec_remove(struct snd_soc_codec *codec)
{
	...
	kfree(msm8x16_wcd_priv->fw_data);
	kfree(msm8x16_wcd_priv);
	gpio_free(pdata->ext_pa_en_gpio);
				...
				struct device_attribute *attr, char *buf)
{
	...
	return snprintf(buf, PAGE_SIZE, "%d\n",
			gpio_get_value_cansleep(pdata->ext_pa_en_gpio));
}
				...
				const char *buff, size_t size)
{
	...
	if ((sscanf(buff, "%d", &buf) == 1) && (buf >= 0)) {
		gpio_set_value_cansleep(pdata->ext_pa_en_gpio, !!buf);
		return size;
	}
	return -EINVAL;
}
...
static int msm8x16_wcd_spmi_remove(struct spmi_device *spmi)
{
	...
	gpio_free(pdata->speaker_id_gpio);
	device_remove_file(&spmi->dev, &dev_attr_enable_ext_pa);
