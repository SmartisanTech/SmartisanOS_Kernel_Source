...
static ssize_t tfa98xx_dbgfs_speaker_id_get(struct file *file,
					    char __user *user_buf, size_t count,
					    loff_t *ppos)
{
	struct i2c_client *i2c = file->private_data;
	...
	if (gpio_is_valid(tfa98xx->spk_id)) {
		if (!gpio_get_value_cansleep(tfa98xx->spk_id)) {
			str = "bosheng\n";
		} else {
			str = "aac\n";
		}
...
static const struct file_operations tfa98xx_dbgfs_speaker_id_fops = {
	.open = simple_open,
	.read = tfa98xx_dbgfs_speaker_id_get,
	.llseek = default_llseek,
};
...
void tfa98xx_debug_init(struct tfa98xx *tfa98xx, struct i2c_client *i2c)
{
	...
	debugfs_create_file("speaker", S_IRUGO, tfa98xx->dbg_dir,
						i2c, &tfa98xx_dbgfs_speaker_id_fops);
...
static char *tfa9891_fw_bs = "tfa9891_bs.cnt";
	...
	struct device_node *np)
{
	...
	tfa98xx->spk_id = of_get_named_gpio(np, "speaker-id", 0);
	if (tfa98xx->spk_id < 0)
		dev_dbg(dev, "No Speaker id GPIO provided.\n");
...
static ssize_t show_speaker_id(struct device *dev,
...
static DEVICE_ATTR(speaker_id, S_IRUGO, show_speaker_id, NULL);
	...
	const struct i2c_device_id *id)
{
	...
	if (gpio_is_valid(tfa98xx->spk_id)) {
		ret = devm_gpio_request_one(&i2c->dev, tfa98xx->spk_id,
					    GPIOF_DIR_IN, "SPEAKER_ID");
		if (ret)
			goto err;
	}
	/* Power up! */
...
#else
	ret = snd_soc_register_codec(&i2c->dev, &soc_codec_dev_tfa98xx, tfa98xx_dai, ARRAY_SIZE(tfa98xx_dai));
#endif
	if (ret < 0) {
...
#endif
	/* Register the sysfs files for speaker id identify*/
	ret = device_create_file(&i2c->dev, &dev_attr_speaker_id);
	if (ret)
		dev_info(&i2c->dev, "error creating speaker_id sysfs files\n");
...
static int tfa98xx_i2c_remove(struct i2c_client *i2c)
{
	...
	device_remove_file(&i2c->dev, &dev_attr_speaker_id);
