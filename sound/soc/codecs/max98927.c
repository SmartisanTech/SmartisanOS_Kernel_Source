...
static void max98927_set_calib_status (bool value, int ch) {
	if (ch == MAX98927L)
...
static bool max98927_get_calib_status (int ch) {
	if (ch == MAX98927L)
...
static bool max98927_can_use_dsm(struct max98927_priv *max98927)
{
...
#define CALIBRATE_FILE_L   "/persist/spkr_calib_l.bin"
#define CALIBRATE_FILE_R   "/persist/spkr_calib_r.bin"
...
static bool rdc_check_valid(uint32_t rdc, int ch)
{
	int rdc_min, rdc_max;
					     ...
					     loff_t *ppos)
{
	...
	if (ret == 0) {
		...
		max989xx->ref_RDC[0] = impedance_l;
		...
		if (max989xx->mono_stereo == 3) {
			...
			max989xx->ref_RDC[1] = impedance_r;
					  ...
					  loff_t *ppos)
{
	...
	*ppos = pos;
	...
	n += scnprintf(kbuf+n, sizeof(kbuf)-n, "%d", !!max98927_get_calib_status(MAX98927L));
...
static void max989xx_debug_init(struct max98927_priv *max989xx, struct i2c_client *i2c)
{
	...
	debugfs_create_file("status", S_IRUGO, max989xx->dbg_dir,
	...
	if (stream == SNDRV_PCM_STREAM_PLAYBACK) {
		if (mute) {
			if (max98927_can_use_dsm(max98927))
				afe_dsm_ramp_dn_cfg(payload, 25);
			...
			for (i = 0; i < MAX_CHANNEL_NUM; i++) {
				if(i2c_states & (0x1 << i)) {
					if (max98927->spk_mode & (0x1 << i)) {
						pr_info("%s: enable %s channel\n", __func__,
							i ? "(right) speaker" : "(left) receiver");
						regmap_update_bits(max98927->regmap[i], MAX98927_AMP_enables,
							1, 1);
						regmap_update_bits(max98927->regmap[i], MAX98927_Global_Enable,
							1, 1);
					} else {
						pr_info("%s: disable %s channel\n", __func__,
							i ? "(right) speaker" : "(left) receiver");
						regmap_update_bits(max98927->regmap[i], MAX98927_AMP_enables,
							1, 0);
						regmap_update_bits(max98927->regmap[i], MAX98927_Global_Enable,
				      ...
				      int event)
{
	...
	case SND_SOC_DAPM_PRE_PMU:
		...
		if (max98927_can_use_dsm(max98927)) {
			mutex_lock(&dsm_lock);
			if (!rdc_check_valid(max98927->ref_RDC[0], MAX98927L)) {
				rc = max989xx_calib_get(&impedance, MAX98927L);
				if (rdc_check_valid(impedance, MAX98927L))
					pr_info("%s: valid rdc from file, ref_rdc[0]=%d \n", __func__, impedance);
				else {
					impedance = max98927_get_default_impedance(MAX98927L);
					pr_info("%s: invalid rdc, use default: %d \n", __func__, impedance);
				}
				max98927->ref_RDC[0] = impedance;
			} else {
				impedance = max98927->ref_RDC[0];
				pr_info("%s: valid rdc, ref_rdc[0]=%d \n", __func__, impedance);
			}
			*payload = impedance;

			if (max98927->mono_stereo == 3) {
				if (!rdc_check_valid(max98927->ref_RDC[1], MAX98927R)) {
					rc = max989xx_calib_get(&impedance, MAX98927R);
					if (rdc_check_valid(impedance, MAX98927R))
						pr_info("%s: valid rdc from file, ref_rdc[1]=%d \n", __func__, impedance);
					else {
						impedance = max98927_get_default_impedance(MAX98927R);
						pr_info("%s: invalid rdc, use default: %d \n", __func__, impedance);
					}
					max98927->ref_RDC[1] = impedance;
				} else {
					impedance = max98927->ref_RDC[1];
					pr_info("%s: valid rdc, ref_rdc[1]=%d \n", __func__, impedance);
				}
				*(payload+1) = impedance;
			}
				      ...
				      struct snd_ctl_elem_value *ucontrol)
{
	...
	if (mix) {
		...
		for (i = 0; i<ARRAY_SIZE(reg_channel_map[0]); i++)
			regmap_write(max98927->regmap[MAX98927L], reg_channel_map[0][i][0],
				reg_channel_map[0][i][1]);
		max98927->rcv_mix_mode = true;
	} else {
		pr_info("%s: Exit Mix Mode\n", __func__);
		for (i = 0; i<ARRAY_SIZE(reg_channel_map[MAX98927L+1]); i++)
			regmap_write(max98927->regmap[MAX98927L], reg_channel_map[MAX98927L+1][i][0],
				reg_channel_map[MAX98927L+1][i][1]);
