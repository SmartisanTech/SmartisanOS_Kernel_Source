...
//Currently, osborn speaker dc resistance is 6.8 +/-15% (5.4 --> 8.2)
//1<<27 is 134217728, for example: 5.4 / 3.33 * 134217728 = 214748365
#define RDC_MIN  (214748365)  //1.6 * (1<<27), 1.6 * MULTIPLE = 5.4 ohm
#define RDC_MAX  (335544320)  //2.5 * (1<<27), 2.5 * MULTIPLE = 8.2 ohm


static uint32_t gParam[PKG_HEADER+PAYLOAD_COUNT];
static bool calib_status = false;

static void max_set_calib_status (bool value) {
	if (calib_status != value)
		calib_status = !!value;
}
...
static int max989xx_calib_save(uint32_t calib_value)
{
	...
	pfile = filp_open(CALIBRATE_FILE, O_RDWR | O_CREAT, 0644);
	if (!IS_ERR(pfile)) {
	...
	if (ret == 0) {
		...
		if (!rdc_check_valid(impedance_l)) {
			...
			*payload = SPK_MUTE_VALUE;
			max_set_calib_status(false);
		}
		max989xx->ref_RDC = impedance_l;
		if (max989xx_calib_save(impedance_l) < 0)
			max_set_calib_status(false);
...
static void max989xx_debug_init(struct max98927_priv *max989xx, struct i2c_client *i2c)
{
	...
	debugfs_create_file("calibrate", S_IRUGO|S_IWUSR, max989xx->dbg_dir,
						i2c, &max989xx_dbgfs_calibrate_fops);
	debugfs_create_file("impedance", S_IRUGO|S_IWUSR, max989xx->dbg_dir,
						i2c, &max989xx_dbgfs_impedance_fops);
	debugfs_create_file("f0detect", S_IRUGO|S_IWUSR, max989xx->dbg_dir,
						i2c, &max989xx_dbgfs_f0_fops);
	debugfs_create_file("temperature", S_IRUGO|S_IWUSR, max989xx->dbg_dir,
	...
	struct snd_pcm_hw_params *params)
{
	...
	max98927_wrap_update_bits(max98927, reg, mask, value);
	return 0;
}
				      ...
				      int event)
{
	...
	case SND_SOC_DAPM_PRE_PMU:
		if (likely(max98927->board_id > HW_DVT2_ID && !factory_test)) {
			mutex_lock(&dsm_lock);
...
static const char * const max98927_factory_test_text[] = {
	"OFF", "ON"
};
...
static const struct soc_enum max98927_enum[] = {
	...
	SOC_ENUM_SINGLE_EXT(ARRAY_SIZE(max98927_factory_test_text), max98927_factory_test_text),
};
...
#endif
			max98927->board_id = get_hw_version_id();
			pr_info("%s: get board_id: %d\n", __func__, max98927->board_id);
		}
	}else
