...
static int aw8898_voice_spk_get(struct snd_kcontrol *kcontrol,
        struct snd_ctl_elem_value *ucontrol)
{
    pr_debug("%s: aw8898_voice_spk_control=%d\n", __func__, aw8898_voice_spk_control);
...
static int aw8898_check_i2s_state(struct aw8898 *aw8898)
{
    unsigned int reg_val = 0;
    ...
    for(i=0; i<iis_check_max; i++) {
        ...
        if(reg_val & AW8898_BIT_SYSST_PLLS) {
            pr_info("%s iis signal check pass!\n", __func__);
            return 0;
        }
        usleep_range(1000, 1100);
    }
    /* iis signal check error */
    pr_err("%s: iis signal check error, power down\n", __func__);
    return -1;
}
...
static int aw8898_mute(struct snd_soc_dai *dai, int mute, int stream)
{
    ...
    pr_info("%s: mute state=%d  stream=%d\n", __func__, mute, stream);
        ...
        struct snd_soc_dai *dai)
{
    ...
    pr_debug("%s: enter, pstream=%d, cstream=%d\n", __func__, aw8898->pstream, aw8898->cstream);
    if ((aw8898->pstream == 0) && (aw8898->cstream == 0)) {
        aw8898->rate = 0;
#ifdef AW8898_VBAT_MONITOR
        ...
        pr_info("%s: stream exits. can not power down the pa\n", __func__);
    }
}
        ...
        char *buf)
{
    ...
    if(aw8898->spk_rcv_mode == AW8898_MUSIC_SPEAKER_MODE) {
        ...
        len += snprintf(buf+len, PAGE_SIZE-len, "aw8898 spk_rcv: %d, voice speaker mode\n", aw8898->spk_rcv_mode);
    } else {
...
static struct of_device_id aw8898_dt_match[] = {
    ...
    { .compatible = "mediatek,aw8898_smartpa" },
    { },
};
