...
static void hsuart_power(int on)
{
	if (test_bit(BT_SUSPEND, &flags))
		return;
/*
	if ((bsi->uport == NULL) || (bsi->uport->state->port.count)) {
		pr_err("bsi->uport == NULL, do nothing\n");
		return;
	}
*/
	if (on) {
...
static void bluesleep_sleep_work(struct work_struct *work)
{
	if (bluesleep_can_sleep()) {
		...
		if (msm_hs_tx_empty(bsi->uport)) {
			...
			WARN_ON("This should never happens\n");
			return;
		}
	}
    /*cannot sleep but UART has already sleep*/
	else if (test_bit(BT_ASLEEP, &flags))
	{
		...
		/*add a timer to make sure that UART would not be turned on&off very frequentently*/
...
/**
 * Handles prop action when btwrite deassert.Sets BT_EXT_WAKE.
 */
...
/**
 * Handles receive timer expiration.
 * @param data Not used.
 */
static void bluesleep_rx_timer_expire(unsigned long data)
{
...
static int __init bluesleep_init(void)
{
	...
	BT_ERR("BlueSleep Mode Driver Ver %s", VERSION);
