...
static int fusb301_set_dfp_power(struct fusb301_chip *chip, u8 hcurrent)
{
	...
	if (hcurrent == chip->dfp_power) {
		dev_info(cdev, "vaule is not updated(%d)\n",
					hcurrent);
		return rc;
	}
...
static void fusb301_select(struct fusb301_chip *chip, enum FUSB_SELECTION value) {
	struct device *cdev = &chip->client->dev;
...
static void fusb301_trigger(struct fusb301_chip *chip, enum AUDIO_DETECTION value) {
	struct device *cdev = &chip->client->dev;
				...
				char *buf)
{
	...
	ret = snprintf(buf, PAGE_SIZE, "%s\n",
			gpio_get_value_cansleep(chip->pdata->usb_switch_asel) ? "audio" : "usb");
	mutex_unlock(&chip->mlock);
	return ret;
}
				...
				const char *buff, size_t size)
{
	...
	if (gpio >= 0) {
		mutex_lock(&chip->mlock);
		gpio_set_value_cansleep(chip->pdata->usb_switch_asel, gpio);
		mutex_unlock(&chip->mlock);
		return size;
	}
	return -EINVAL;
				...
				char *buf)
{
	...
	mutex_lock(&chip->mlock);
	ret = snprintf(buf, PAGE_SIZE, "%d\n",
			gpio_get_value_cansleep(chip->pdata->hs_det));
	mutex_unlock(&chip->mlock);
	return ret;
}
				...
				const char *buff, size_t size)
{
	...
	if ((sscanf(buff, "%d", &buf) == 1) && (buf >= 0)) {
		mutex_lock(&chip->mlock);
		gpio_set_value_cansleep(chip->pdata->hs_det, !!buf);
		mutex_unlock(&chip->mlock);
		return size;
	}
	return -EINVAL;
...
static int fusb301_create_devices(struct device *cdev)
{
	...
	ret = device_create_file(cdev, &dev_attr_usb_switch_asel);
	if (ret < 0) {
		dev_err(cdev, "failed to create dev_attr_usb_switch_asel\n");
		ret = -ENODEV;
		goto err11;
	}
	ret = device_create_file(cdev, &dev_attr_hs_det);
	if (ret < 0) {
		dev_err(cdev, "failed to create dev_attr_hs_det\n");
		ret = -ENODEV;
		goto err12;
	}
	return ret;
err12:
	device_remove_file(cdev, &dev_attr_usb_switch_asel);
err11:
	device_remove_file(cdev, &dev_attr_fccdebounce_timeout);
err10:
...
static void fusb301_destory_device(struct device *cdev)
{
	...
	device_remove_file(cdev, &dev_attr_fregdump);
	device_remove_file(cdev, &dev_attr_usb_switch_asel);
	device_remove_file(cdev, &dev_attr_hs_det);
}
...
static void fusb301_detach(struct fusb301_chip *chip)
{
	...
	case FUSB_STATE_AUDIO_ACCESSORY:
		fusb301_trigger(chip, AUDIO_REMOVE);
		break;
...
static void fusb301_attach(struct fusb301_chip *chip)
{
	...
	if (type != FUSB301_TYPE_AUD_ACC && type != FUSB301_TYPE_PWR_AUD_ACC)
		fusb301_select(chip, FUSB_USB);
...
static int fusb301_parse_dt(struct fusb301_chip *chip)
{
	...
	dev_info(cdev, "init_mode:%d dfp_power:%d toggle_time:%d\n",
	...
	if (data->try_snk_emulation) {
		...
		dev_info(cdev, "try snk in firmware\n");
	}
...
static int dual_role_set_prop(struct dual_role_phy_instance *dual_role,
	...
	if (prop == DUAL_ROLE_PROP_MODE) {
		if (*val == DUAL_ROLE_PROP_MODE_DFP) {
			dev_info(cdev, "%s: Setting SRC mode\n", __func__);
			mode = FUSB301_SRC;
			...
			dev_info(cdev, "%s: Setting SNK mode\n", __func__);
			mode = FUSB301_SNK;
			...
			const struct i2c_device_id *id)
{
	...
	if (IS_ENABLED(CONFIG_DUAL_ROLE_USB_INTF)) {
		...
		desc->cc_vendor = DUAL_ROLE_CC_FAIRCHILD;
	...
	pr_info("houzn:fusb301_probe ok!\n");
	return 0;
err5:
	if (IS_ENABLED(CONFIG_DUAL_ROLE_USB_INTF))
