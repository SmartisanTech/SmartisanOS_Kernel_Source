...
#include "delta_ct_rm692a9.h"
#include "../sde/sde_hw_mdss.h"
...
extern struct dsi_display *set_display;
static bool bl_max_flag = false;
static int bl_max_count = 0;
static struct mutex bl_max_mutex;
bool is_ct_enter = false;
bool is_eye_care_mode = false;
bool is_color_temprature_mode = false;
bool is_source_backlight_mode = false;
bool is_local_hbm_enable = false;
bool is_hbm_need_disable_dimming = false;
bool is_lp_mode = false;
int is_CGM_enable = DSI_PANEL_IE_CGM_P3;
u32 set_color_enable[2] = {0};
...
static int dsi_panel_power_off(struct dsi_panel *panel)
{
	...
	if(true == is_local_hbm_enable)
		is_local_hbm_enable = false;
	if(true == is_hbm_need_disable_dimming)
		is_hbm_need_disable_dimming = false;
	if(true == is_ct_enter)
		is_ct_enter = false;

	mutex_lock(&bl_max_mutex);
	bl_max_flag = false;
	bl_max_count = 0;
	mutex_unlock(&bl_max_mutex);

	pr_info("power off panel\n");
	return rc;
}
static int dsi_panel_tx_cmd_set(struct dsi_panel *panel,
				enum dsi_cmd_set_type type, int color_flag)
{
	int rc = 0, i = 0;
	ssize_t len;
	...
	bool continue_send_cmd = true;
	const struct mipi_dsi_host_ops *ops = panel->host->ops;

	if ((0 == panel->panel_power_state) && (DSI_CMD_SET_NOLP != type) && (type > DSI_CMD_SET_POST_OFF))
		continue_send_cmd = false;

	if (!panel || !panel->cur_mode || (false == continue_send_cmd))
		return -EINVAL;

	pr_info("%s %d %d\n", __func__, type, color_flag);
...
extern bool ack_need;
int dsi_panel_disable(struct dsi_panel *panel);
int dsi_panel_post_unprepare(struct dsi_panel *panel);
int dsi_panel_pre_prepare(struct dsi_panel *panel);
int dsi_panel_enable(struct dsi_panel *panel);

void dsi_panel_power_control(struct dsi_panel *panel, bool enable)
{
	if (!panel) {
	...
	if (enable == false) {
		dsi_panel_disable(panel);
		dsi_panel_post_unprepare(panel);
	} else {
		dsi_panel_pre_prepare(panel);
		dsi_panel_enable(panel);
	}
}
EXPORT_SYMBOL(dsi_panel_power_control);
...
#define XBL_BL_LVL 0x200
static int bl_update_count = 0;
extern struct sde_hw_dim_layer dim_layer;
	...
	u32 bl_lvl)
{
	...
	u32 brightness = 0;
	int index = 0, resolution = 0;
	u32 *local_gamma_lumen = NULL;
	...
	if(true == is_hbm_need_disable_dimming) {
		is_hbm_need_disable_dimming = false;
		dsi_panel_tx_cmd_set(panel,DSI_CMD_HBM_DISABLE,COLD_COLOR);
	}

	dsi = &panel->mipi_device;

	if (is_source_backlight_mode)
		local_gamma_lumen = dc_gamma_lumen;
	else
		local_gamma_lumen = gamma_lumen;

	if ( bl_lvl < panel->bl_config.bl_min_level )
		brightness = 0;
	else if ( bl_lvl < BL_GAP )
		brightness = local_gamma_lumen[0];
	else if ( bl_lvl < panel->bl_config.bl_max_level )
	{
		index = (bl_lvl - 15) / 24;
		resolution = local_gamma_lumen[index + 1] - local_gamma_lumen[index];
		brightness = ((local_gamma_lumen[index] * 25) + resolution * (bl_lvl - 15 - (24 * index))) / 25;
	}
	else
		brightness = panel->bl_config.brightness_max_level;

	if(bl_update_count < 2) {
		bl_update_count++;
		brightness = XBL_BL_LVL;
	}

	pr_debug("update backlight %d %d\n", bl_lvl, brightness);
...
int dsi_panel_set_backlight(struct dsi_panel *panel, u32 bl_lvl)
{
	...
	if ((true == bl_max_flag) && (bl_lvl != panel->bl_config.bl_max_level)) {
 		pr_info("set backlight to max due to local hbm");
 		return 0;
 	}
...
const char *cmd_set_prop_map[DSI_CMD_SET_MAX] = {
	...
	"qcom,mdss-dsi-panel-eye-protect-enter",
	...
	"qcom,mdss-dsi-local-hbm-enter",
	"qcom,mdss-dsi-local-hbm-exit",
	"qcom,mdss-dsi-CGM-P3-feature",
	"qcom,mdss-dsi-CGM-sRGB-feature",
	"qcom,mdss-dsi-CGM-Native-feature",
	"qcom,mdss-dsi-roi-prepare",
	"qcom,mdss-dsi-source-backlight-enter",
	"qcom,mdss-dsi-source-backlight-exit",
};
...
const char *cmd_set_state_map[DSI_CMD_SET_MAX] = {
	...
	"qcom,mdss-dsi-panel-eye-protect-enter-state",
	...
	"qcom,mdss-dsi-local-hbm-enter-state",
	"qcom,mdss-dsi-local-hbm-exit-state",
	...
	"qcom,mdss-dsi-source-backlight-enter-state",
	"qcom,mdss-dsi-source-backlight-exit-state",
};
...
static void dsi_panel_set_warm_cold_pp()
{
	int rc = 0;

	if(false == is_ct_enter) {
		is_ct_enter = true;
		rc = dsi_panel_tx_cmd_set(set_panel, DSI_CMD_CT_ENTER, COLD_COLOR);
		if (rc) {
			pr_err("[%s] failed to send DSI_CMD_CT_ENTER cmds, rc=%d\n",set_panel->name, rc);
		}
	}

	if(true == is_eye_care_mode) {
		rc = dsi_panel_tx_cmd_set(set_panel, DSI_CMD_SET_MAX, WARM_COLOR);
		if (rc) {
			pr_err("[%s] failed to send eye protect cmd\n",set_panel->name);
		}
	} else {
		rc = dsi_panel_tx_cmd_set(set_panel, (set_color_enable[1]+DSI_CMD_SET_MAX), set_color_enable[0]);
		if (rc) {
			pr_err("[%s] failed to send cmd %d %d\n",set_panel->name, (set_color_enable[1]+DSI_CMD_SET_MAX), set_color_enable[0]);
		}
	}
}

extern int sde_encoder_wait_for_event(struct drm_encoder *drm_enc, enum msm_event_wait event);
...
void dsi_panel_set_ie_level(u32 ie_level)
{
	...
	if (set_panel->panel_power_state == 0 && ie_level == DSI_PANEL_IE_SET_BACKLIGHT_TO_MAX_ENTER) {
		pr_info("%s:in power off state and ie_level is %d.\n", __func__, ie_level);
		mutex_lock(&bl_max_mutex);
		bl_max_count++;
		mutex_unlock(&bl_max_mutex);
	}
	if (set_panel->panel_power_state == 0 && ie_level == DSI_PANEL_IE_SET_BACKLIGHT_TO_MAX_EXIT) {
		pr_info("%s:in power off state and ie_level is %d.\n", __func__, ie_level);
		mutex_lock(&bl_max_mutex);
		bl_max_count--;
		mutex_unlock(&bl_max_mutex);
	...
	switch (ie_level) {
	case DSI_PANEL_IE_EYE_PROTECT_ENTER:
		is_eye_care_mode = true;
		dsi_panel_set_warm_cold_pp();
		break;
	case DSI_PANEL_IE_EYE_PROTECT_EXIT:
		if(false == is_eye_care_mode)
			break;
		is_eye_care_mode = false;
		dsi_panel_set_warm_cold_pp();
		break;
	case DSI_PANEL_IE_SET_SOURCE_ENTER:
		is_source_backlight_mode = true;
		rc = dsi_panel_tx_cmd_set(set_panel, DSI_CMD_SOURCE_BACKLIGHT_ENTER, COLD_COLOR);
		if (rc) {
			pr_err("[%s] failed to send DSI_CMD_SOURCE_BACKLIGHT_ENTER cmds, rc=%d\n",
			       set_panel->name, rc);
		}

		rc = dsi_panel_set_backlight(set_panel, set_panel->bl_config.bl_level);
		if (rc)
			pr_err("source set backlight %d failed.\n", set_panel->bl_config.bl_level);

		break;
	case DSI_PANEL_IE_SET_SOURCE_EXIT:
		is_source_backlight_mode = false;
		rc = dsi_panel_set_backlight(set_panel, set_panel->bl_config.bl_level);
		if (rc)
			pr_err("source set backlight %d failed.\n", set_panel->bl_config.bl_level);
		rc = dsi_panel_tx_cmd_set(set_panel, DSI_CMD_SOURCE_BACKLIGHT_EXIT, COLD_COLOR);
		if (rc) {
			pr_err("[%s] failed to send DSI_CMD_SOURCE_BACKLIGHT_EXIT cmds, rc=%d\n",
			       set_panel->name, rc);
		}
		break;
	case DSI_PANEL_IE_SET_LP_ENTER:
		is_lp_mode = true;
		rc = dsi_panel_tx_cmd_set(set_panel, DSI_CMD_SET_LP1, COLD_COLOR);
		if (rc) {
			pr_err("[%s] failed to send DSI_CMD_SET_LP1 cmds, rc=%d\n",
			       set_panel->name, rc);
		}
		break;
	case DSI_PANEL_IE_SET_LP_EXIT:
		is_lp_mode = false;
		rc = dsi_panel_tx_cmd_set(set_panel, DSI_CMD_SET_NOLP, COLD_COLOR);
		if (rc) {
			pr_err("[%s] failed to send DSI_CMD_SET_NOLP cmds, rc=%d\n",
			       set_panel->name, rc);
		}
		break;
	case DSI_PANEL_IE_SET_BACKLIGHT_TO_MAX_ENTER:
		mutex_lock(&bl_max_mutex);
		bl_max_flag = true;
		bl_max_count++;
		mutex_unlock(&bl_max_mutex);
		pr_info("enter bl to max %d.\n", bl_max_count);
		rc = dsi_panel_set_backlight(set_panel, set_panel->bl_config.bl_max_level);
		if (rc)
			pr_err("enable hbm set backlight to max fail.\n");
		sde_encoder_wait_for_event(set_display->drm_conn->encoder, MSM_ENC_VBLANK);

		break;
	case DSI_PANEL_IE_SET_BACKLIGHT_TO_MAX_EXIT:
		mutex_lock(&bl_max_mutex);
		bl_max_count--;
		if (bl_max_count <= 0)
			bl_max_flag = false;
		mutex_unlock(&bl_max_mutex);
		pr_info("exit bl to max %d.\n", bl_max_count);

		if (false == bl_max_flag) {
			if (set_panel->bl_config.bl_level > 0) {
				pr_info("return previous backlight=%d\n", set_panel->bl_config.bl_level);
				rc = dsi_panel_set_backlight(set_panel, set_panel->bl_config.bl_level);
				if (rc)
					pr_err("disable hbm set backlight fail.\n");
			}
		}
		mdelay(8);
		break;
	case DSI_PANEL_IE_SET_LOCAL_HBM_ENTER:
		is_local_hbm_enable = true;

		rc = dsi_panel_tx_cmd_set(set_panel, DSI_CMD_LOCAL_HBM_ENABLE, COLD_COLOR);
		if (rc) {
			pr_err("[%s] failed to send DSI_CMD_LOCAL_HBM_ENABLE cmds, rc=%d\n",
			       set_panel->name, rc);
		}

		break;
	case DSI_PANEL_IE_SET_LOCAL_HBM_EXIT:
		rc = dsi_panel_tx_cmd_set(set_panel, DSI_CMD_LOCAL_HBM_DISABLE, COLD_COLOR);
		if (rc) {
			pr_err("[%s] failed to send DSI_CMD_LOCAL_HBM_DISABLE cmds, rc=%d\n",
				   set_panel->name, rc);
		}
		...
		break;
	case DSI_PANEL_IE_HBM_ENABLE:
		dsi_panel_tx_cmd_set(set_panel,DSI_CMD_HBM_ENABLE,COLD_COLOR);
		break;
	case DSI_PANEL_IE_HBM_DISABLE:
		//dsi_panel_tx_cmd_set(set_panel,DSI_CMD_HBM_DISABLE,COLD_COLOR);
		rc = dsi_panel_set_backlight(set_panel, set_panel->bl_config.bl_level);
		if (rc)
			pr_err("disable hbm set backlight fail.\n");
		is_hbm_need_disable_dimming = true;
		break;
	case DSI_PANEL_IE_CGM_P3:
		is_CGM_enable = DSI_PANEL_IE_CGM_P3;
		is_ct_enter = false;
		dsi_panel_tx_cmd_set(set_panel,DSI_CMD_CGM_P3,COLD_COLOR);
		break;
	case DSI_PANEL_IE_CGM_SRGB:
		is_CGM_enable = DSI_PANEL_IE_CGM_SRGB;
		dsi_panel_tx_cmd_set(set_panel,DSI_CMD_CGM_sRGB,COLD_COLOR);
		break;
	case DSI_PANEL_IE_CGM_NATIVE:
		is_CGM_enable = DSI_PANEL_IE_CGM_NATIVE;
		dsi_panel_tx_cmd_set(set_panel,DSI_CMD_CGM_Native,COLD_COLOR);
		break;
	case TOUCHSCREEN_SET_GLOVE_DISABLE:
		fts_glove_mode_set(DISABLE);
		break;
	case TOUCHSCREEN_SET_GLOVE_ENABLE:
...
static ssize_t dsi_panel_parse_color_data(void)
{
	...
	for ( i = 0; i < CT_SIZE*DCS_CMD_LENGTH; i++ )
	{
		switch( i % DCS_CMD_LENGTH ){
		case DCS_CMD_MSG_TYPE:
			cool_data [i] = 0x15;
			warm_data [i] = 0x15;
			reset_data [i] = 0x15;
			break;
		case DCS_CMD_MSG_IS_LAST_COMMAND:
			cool_data [i] = 0x01;
			warm_data [i] = 0x01;
			reset_data [i] = 0x01;
			break;
		case DCS_CMD_MSG_CHANNEL:
			cool_data [i] = 0x00;
			warm_data [i] = 0x00;
			reset_data [i] = 0x00;
			break;
		case DCS_CMD_MSG_FLAG:
			cool_data [i] = 0x00;
			warm_data [i] = 0x00;
			reset_data [i] = 0x00;
			break;
		case DCS_CMD_MSG_WAIT_TIME:
			cool_data [i] = 0x00;
			warm_data [i] = 0x00;
			reset_data [i] = 0x00;
			break;
		case DCS_CMD_MSG_LEN1:
			cool_data [i] = 0x00;
			warm_data [i] = 0x00;
			reset_data [i] = 0x00;
			break;
		case DCS_CMD_MSG_LEN2:
			cool_data [i] = 0x02;
			warm_data [i] = 0x02;
			reset_data [i] = 0x02;
			break;
		case DCS_CMD_MSG_REG:
			cool_data [i] = reg[ i / DCS_CMD_LENGTH];
			warm_data [i] = reg[ i / DCS_CMD_LENGTH];
			reset_data [i] = reg[ i / DCS_CMD_LENGTH];
			break;
		default:
			break;
		}
	...
	i = (4*DCS_CMD_LENGTH) + DCS_CMD_MSG_WAIT_TIME;
	j = (145*DCS_CMD_LENGTH) + DCS_CMD_MSG_WAIT_TIME;
	cool_data[i] = 1;
	cool_data[j] = 20;
	warm_data[i] = 1;
	warm_data[j] = 20;
	...
	do {
		for (i = 0;i < CT_SIZE*DCS_CMD_LENGTH;i++){
			if( i % DCS_CMD_LENGTH == DCS_CMD_MSG_DATA)
			{
				cool_data [i] = cool_data_color[j][i / DCS_CMD_LENGTH];
				warm_data [i] = warm_data_color[j][i / DCS_CMD_LENGTH];
				if(0 == j)
					reset_data [i] = reset_data_color[j][i / DCS_CMD_LENGTH];
			}
		}

		if(0 == j) {
			rc = dsi_panel_parse_color_cmd(&reset_cmd[j], reset_data, CT_SIZE*DCS_CMD_LENGTH/*arry len*/);
			if(rc < 0){
				pr_err("%s:failed to set reset cmd\n", __func__);
			}
		}

		rc = dsi_panel_parse_color_cmd(&warm_cmd[j], warm_data, CT_SIZE*DCS_CMD_LENGTH/*arry len*/);

		if(rc < 0){
			pr_err("%s:failed to set warm cmd\n", __func__);
		}
		rc = dsi_panel_parse_color_cmd(&cool_cmd[j], cool_data, CT_SIZE*DCS_CMD_LENGTH/*arry len*/);
		if(rc < 0){
						...
						unsigned long value)
{
	...
	if(set_panel->panel_power_state == 0){
		if(flags == SMARTISAN_IE_SET && value == DSI_PANEL_IE_EYE_PROTECT_EXIT){
			is_eye_care_mode = false;
		} else if (flags == SMARTISAN_IE_SET && value == DSI_PANEL_IE_EYE_PROTECT_ENTER) {
			is_eye_care_mode = true;
		} else if (flags == SMARTISAN_IE_SET && value == DSI_PANEL_IE_SET_SOURCE_ENTER) {
			is_source_backlight_mode = true;
		} else if (flags == SMARTISAN_IE_SET && value == DSI_PANEL_IE_SET_SOURCE_EXIT) {
			is_source_backlight_mode = false;
		} else if (flags == SMARTISAN_IE_SET && value == DSI_PANEL_IE_SET_LOCAL_HBM_ENTER) {
			is_local_hbm_enable = true;
		} else if (flags == SMARTISAN_IE_SET && value == DSI_PANEL_IE_SET_LOCAL_HBM_EXIT) {
			is_local_hbm_enable = false;
		} else if (flags == DSI_PANEL_WARM_PP) {
			is_color_temprature_mode = true;
			set_color_enable[0] = WARM_COLOR;
			set_color_enable[1] = value;
		} else if (flags == DSI_PANEL_COLD_PP) {
			is_color_temprature_mode = true;
			set_color_enable[0] = COLD_COLOR;
			set_color_enable[1] = value;
		} else if (flags == DSI_PANEL_RESET_PP) {
			is_color_temprature_mode = false;
		}
		pr_err("%s:in power off state.\n", __func__);
		return -EAGAIN;
	}
	switch (flags) {
		...
		case DSI_PANEL_RESET_PP:
			is_color_temprature_mode = false;
			set_color_enable[0] = RESET_COLOR;
			set_color_enable[1] = 0;
			dsi_panel_set_warm_cold_pp();
			break;
...
static const struct file_operations color_panel_fops = {
	...
	.compat_ioctl = color_panel_ioctl,
};
...
static ssize_t mdss_fb_set_bl_curve(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t len)
{
	...
	u32 temp_data;
	int retval = -EINVAL;
	if (buf == NULL || strlen(input) < STRING_LENGTH)
		return retval;
	pr_debug("enter set curve: len is %d buf is %s\n", strlen(buf), input);

	while (input != NULL && index < BUFFER_LENGTH) {
		token = strsep(&input, " ");
		retval = sscanf(token, "%d", &temp_data);
		pr_debug("temp_data:%d token:%s index:%d\n", temp_data, token, index);
		if (retval < 0) {
			pr_err("%s: Failed to convert \"%s\" to decimal number\n",
					__func__, token);
			goto exit;
		}
		gamma_lumen[index] = temp_data;
		index++;
	}
...
static ssize_t mdss_fb_set_delay_time(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t len)
{

	if (sscanf(buf, "%d", &delay_time) != 1) {
...
static DEVICE_ATTR(msm_fb_ie_level, S_IRUGO | S_IWUSR,
	NULL, mdss_fb_set_ie_level);

static DEVICE_ATTR(msm_fb_delay_time, S_IRUGO | S_IWUSR,
	NULL, mdss_fb_set_delay_time);

static DEVICE_ATTR(msm_fb_bl_curve, S_IRUGO | S_IWUSR,
	NULL, mdss_fb_set_bl_curve);

static struct attribute *mdss_fb_attrs[] = {
	&dev_attr_msm_fb_ie_level.attr,
	&dev_attr_msm_fb_bl_curve.attr,
	&dev_attr_msm_fb_delay_time.attr,
	NULL,
};
...
int dsi_panel_update_pps(struct dsi_panel *panel)
{
	...
	rc = dsi_panel_tx_cmd_set(panel, DSI_CMD_SET_PPS, COLD_COLOR);
	if (rc) {
...
#ifndef BYTEDANCE_DEFAULT_USE_AOD
...
int dsi_panel_set_lp1(struct dsi_panel *panel)
{
	...
	rc = dsi_panel_tx_cmd_set(panel, DSI_CMD_SET_LP1, COLD_COLOR);
	if (rc)
...
int dsi_panel_set_lp2(struct dsi_panel *panel)
{
	...
	rc = dsi_panel_tx_cmd_set(panel, DSI_CMD_SET_LP2, COLD_COLOR);
	if (rc)
...
int dsi_panel_set_nolp(struct dsi_panel *panel)
{
	...
	rc = dsi_panel_tx_cmd_set(panel, DSI_CMD_SET_NOLP, COLD_COLOR);
	if (rc)
...
int dsi_panel_prepare(struct dsi_panel *panel)
{
	...
	rc = dsi_panel_tx_cmd_set(panel, DSI_CMD_SET_PRE_ON, COLD_COLOR);
	if (rc) {
		...
		int ctrl_idx)
{
	...
	rc = dsi_panel_tx_cmd_set(panel, DSI_CMD_SET_QSYNC_ON, COLD_COLOR);
	if (rc)
		...
		int ctrl_idx)
{
	...
	rc = dsi_panel_tx_cmd_set(panel, DSI_CMD_SET_QSYNC_OFF, COLD_COLOR);
	if (rc)
		...
		struct dsi_rect *roi)
{
	...
	rc = dsi_panel_tx_cmd_set(panel, DSI_CMD_ROI_RPEPARE, COLD_COLOR);
	if (rc)
		pr_err("[%s] failed to send DSI_CMD_ROI_RPEPARE cmds, rc=%d\n",
				panel->name, rc);

	rc = dsi_panel_tx_cmd_set(panel, DSI_CMD_SET_ROI, COLD_COLOR);
	if (rc)
...
int dsi_panel_switch(struct dsi_panel *panel)
{
	...
	rc = dsi_panel_tx_cmd_set(panel, DSI_CMD_SET_TIMING_SWITCH, COLD_COLOR);
	if (rc)
...
int dsi_panel_post_switch(struct dsi_panel *panel)
{
	...
	rc = dsi_panel_tx_cmd_set(panel, DSI_CMD_SET_POST_TIMING_SWITCH, COLD_COLOR);
	if (rc)
...
int dsi_panel_enable(struct dsi_panel *panel)
{
	...
	rc = dsi_panel_tx_cmd_set(panel, DSI_CMD_SET_ON, COLD_COLOR);
	if (rc) {
	...
	if ((bl_max_count > 0) && (false == bl_max_flag)) {
		mutex_lock(&bl_max_mutex);
		bl_max_count--;
		mutex_unlock(&bl_max_mutex);
		dsi_panel_set_ie_level(DSI_PANEL_IE_SET_BACKLIGHT_TO_MAX_ENTER);
	}

#ifndef CONFIG_FACTORY_SUPPORT
	dsi_panel_set_ie_level(is_CGM_enable);
#endif
	if(true == is_eye_care_mode || true == is_color_temprature_mode)
		dsi_panel_set_warm_cold_pp();
	else
		dsi_panel_set_warm_cold_pp();
	if(true == is_source_backlight_mode)
		dsi_panel_set_ie_level(DSI_PANEL_IE_SET_SOURCE_ENTER);
	if (true == is_local_hbm_enable) {
		dsi_panel_set_ie_level(DSI_PANEL_IE_SET_LOCAL_HBM_ENTER);
	}
...
int dsi_panel_post_enable(struct dsi_panel *panel)
{
	...
	rc = dsi_panel_tx_cmd_set(panel, DSI_CMD_SET_POST_ON, COLD_COLOR);
	if (rc) {
...
int dsi_panel_pre_disable(struct dsi_panel *panel)
{
	...
	rc = dsi_panel_tx_cmd_set(panel, DSI_CMD_SET_PRE_OFF, COLD_COLOR);
	if (rc) {
...
int dsi_panel_disable(struct dsi_panel *panel)
{
	...
	if (!atomic_read(&panel->esd_recovery_pending)) {
		rc = dsi_panel_tx_cmd_set(panel, DSI_CMD_SET_OFF, COLD_COLOR);
		if (rc) {
			/*
...
int dsi_panel_unprepare(struct dsi_panel *panel)
{
	...
	rc = dsi_panel_tx_cmd_set(panel, DSI_CMD_SET_POST_OFF, COLD_COLOR);
	if (rc) {
