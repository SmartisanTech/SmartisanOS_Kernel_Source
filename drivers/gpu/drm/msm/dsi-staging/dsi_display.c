...
#define DSI_DCS_CMD_LENGTH       9
#define DSI_COMMAND_MIN_LENGTH   4
#define DSI_COMMAND_LEN_OFFSET   1
#define DSI_COMMAND_REG_OFFSET   6
#define DSI_COMMAND_HEAD_LENGTH  7
#define USER_COMMAND_HEAD_LENGTH 2
...
extern int _sde_encoder_switch_to_watchdog_vsync(struct drm_encoder *drm_enc, bool enable);
extern void dsi_panel_power_control(struct dsi_panel *panel, bool enable);
bool ack_need = true;
EXPORT_SYMBOL(ack_need);
bool ack_need_change;
EXPORT_SYMBOL(ack_need_change);
bool rsc_state_en;
EXPORT_SYMBOL(rsc_state_en);
struct dsi_display *set_display;
EXPORT_SYMBOL(set_display);
		...
		void *display, u32 bl_lvl)
{
	...
	if(0 == dsi_display->panel->panel_power_state)
		return -EINVAL;
...
static unsigned char recv[BUFFER_LENGTH];
		...
		const char *buf, size_t count)
{
	...
	if (buf == NULL || (display->panel->panel_power_state == 0) || (display->panel->control_power_state == 0))
		return retval;
	...
	recv_len = data[index-1];
	if (cmd.last_command) {
		cmd.msg.flags |= MIPI_DSI_MSG_LASTCOMMAND;
		flags |= DSI_CTRL_CMD_LAST_COMMAND;
	}

	retval = dsi_display_cmd_engine_enable(display);
	if (retval) {
		pr_err("cmd engine enable failed\n");
		return -EPERM;
	}

	mutex_unlock(&display->panel->transfer_mutex);
		...
		const char *buf, size_t count)
{
	...
	int loop = 0;
	int dsi_command_len = 0;
	int packet_count = 0;
	const unsigned char default_data[5] = {0x1,0x0,0x0,0x0,0x0};
	int i = 0;
	u32 size;
	...
	ops = display->panel->host->ops;

	if (buf == NULL || (display->panel->panel_power_state == 0) || (display->panel->control_power_state == 0))
	{
		pr_err("buf is NULL or panel_power_state is %d or control_power_state is %d\n", display->panel->panel_power_state, display->panel->control_power_state);
		return retval;
	}

	pr_info("%s->%d %s %d\n", __func__, __LINE__, buf, count);
	mutex_lock(&display->panel->transfer_mutex);

	while (((token = strsep(&input, " ")) != NULL) && index <= BUFFER_LENGTH)
	{
		retval = sscanf(token, "%x", &temp_data);
		if (retval < 0) {
			pr_err("%s: Failed to convert \"%s\" to hex number\n",
	...
	/* parse param */
	retval = -EINVAL;
	user_data = kzalloc((BUFFER_LENGTH>>2)*DSI_DCS_CMD_LENGTH, GFP_KERNEL);
	if (!user_data) {
		pr_err("failed to malloc memory for user data.\n");
		goto exit;
	}
	memset(user_data, 0, (BUFFER_LENGTH>>2)*DSI_DCS_CMD_LENGTH);
	p = data;
	if (index >= DSI_COMMAND_MIN_LENGTH)
	{
		loop = index;
		do
		{
			dsi_command_len = *(p + DSI_COMMAND_LEN_OFFSET);
			if(dsi_command_len == 2 || dsi_command_len == 3)
			{
				if((loop - USER_COMMAND_HEAD_LENGTH) < dsi_command_len)
				{
					pr_err("Error:user dsi command error.\n");
					goto exit;
				}
				loop = loop - dsi_command_len - USER_COMMAND_HEAD_LENGTH;
				packet_count++;
				user_data[i] = *p;
				memcpy(user_data + i + DSI_COMMAND_LEN_OFFSET, default_data, sizeof(default_data));
				memcpy(user_data + i + DSI_COMMAND_REG_OFFSET, p + DSI_COMMAND_LEN_OFFSET, dsi_command_len + 1);
				i += DSI_COMMAND_HEAD_LENGTH + dsi_command_len;
				p += USER_COMMAND_HEAD_LENGTH + dsi_command_len;
			}
			else
			{
				loop = 0;
				pr_err("Failed to parse user dsi command\n");
				goto exit;
			}
		}while(loop > 0);
	}

	size = packet_count * sizeof(struct dsi_cmd_desc);
	...
	retval = dsi_panel_create_cmd_packets(user_data, index, packet_count, cmds);
	if (retval) {
		pr_err("Failed to create cmd packets, retval=%d\n", retval);
		goto exit;
	}

	for (i = 0; i < packet_count; i++) {
		if (cmds[i].last_command)
			cmds[i].msg.flags |= MIPI_DSI_MSG_LASTCOMMAND | MIPI_DSI_MSG_USE_LPM;

		retval = ops->transfer(display->panel->host, &cmds[i].msg);
		if (retval  < 0) {
			pr_err("failed to set cmd rc=%d tx_len=%d\n",  retval, cmds[i].msg.tx_len);
			goto exit;
		}
		...
		struct device_attribute *attr, char *buf)
{
	...
	ret = scnprintf(buf, PAGE_SIZE, "panel_power_on = %d\n",
						display->panel->panel_power_state);

	return ret;
}
static ssize_t panel_power_control_sysfs_store(struct device *dev,
					       struct device_attribute *attr,
					       const char *buf, size_t count)
{
	int enable_power = 0;
	...
	sde_connector = to_sde_connector(display->drm_conn);

	sscanf(buf, "%d", &enable_power);

	pr_info("control_power_state %d\n", display->panel->control_power_state);

	mutex_lock(&display->display_lock);
	if (enable_power == 1) {
		if ((display->panel->panel_power_state == 1) && (display->panel->control_power_state == 0)) {
			dsi_panel_power_control(display->panel, true);
			if (rsc_state_en) {
				ack_need_change = false;
				_sde_encoder_switch_to_watchdog_vsync(display->drm_conn->encoder, false);
			} else
				ack_need_change = true;
			ack_need = true;
			display->panel->control_power_state = 1;
			schedule_delayed_work(&sde_connector->status_work,
				msecs_to_jiffies(STATUS_CHECK_INTERVAL_MS));
		}
	} else {
		if ((display->panel->panel_power_state == 1) && (display->panel->control_power_state == 1)) {
			cancel_delayed_work_sync(&sde_connector->status_work);
			ack_need = false;
			if (rsc_state_en) {
				ack_need_change = false;
				_sde_encoder_switch_to_watchdog_vsync(display->drm_conn->encoder, true);
			} else
				ack_need_change = true;
			//mdelay(10);
			dsi_panel_power_control(display->panel, false);
			display->panel->control_power_state = 0;
		}
	}
	mutex_unlock(&display->display_lock);
...
static DEVICE_ATTR(panel_power_control, (S_IWUSR | S_IWGRP),
		   NULL, panel_power_control_sysfs_store);

static struct attribute *dsi_access_attrs[] = {
	&dev_attr_read.attr,
	&dev_attr_write.attr,
	&dev_attr_show_blank_event.attr,
	&dev_attr_panel_power_control.attr,
	NULL,
};
...
int dsi_display_enable(struct dsi_display *display)
{
	...
	if (display->panel->control_power_state == 0)
		display->panel->control_power_state = 1;
