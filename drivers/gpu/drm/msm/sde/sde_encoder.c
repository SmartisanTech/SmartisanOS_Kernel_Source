...
extern bool rsc_state_en;
extern bool ack_need;
extern bool ack_need_change;
...
int _sde_encoder_switch_to_watchdog_vsync(struct drm_encoder *drm_enc, bool enable)
{
	struct sde_encoder_virt *sde_enc;
	struct msm_display_info disp_info;
	...
	disp_info.is_te_using_watchdog_timer = enable;
...
EXPORT_SYMBOL(_sde_encoder_switch_to_watchdog_vsync);
		...
		struct sde_encoder_rsc_config *config, bool enable)
{
	...
	rsc_state_en = enable;
	if (ack_need_change && enable) {
		ack_need_change = false;
		if (!ack_need)
			_sde_encoder_switch_to_watchdog_vsync(drm_enc, true);
		else
			_sde_encoder_switch_to_watchdog_vsync(drm_enc, false);
	}
...
EXPORT_SYMBOL(sde_encoder_wait_for_event);
	...
	bool skip_pre_kickoff)
{
	 ...
	 */
	_sde_encoder_switch_to_watchdog_vsync(enc, true);
