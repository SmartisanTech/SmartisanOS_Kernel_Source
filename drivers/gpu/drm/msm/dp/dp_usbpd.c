...
struct dp_usbpd_private *globle_pd;
...
{
	if (!globle_pd) {
		pr_info("invalid dp_usbpd\n");
		goto error;
	}

	globle_pd->dp_usbpd.base.hpd_high = hpd;
	globle_pd->forced_disconnect = !hpd;
	if (hpd)
		globle_pd->dp_cb->configure(globle_pd->dev);
	else
		globle_pd->dp_cb->disconnect(globle_pd->dev);

error:
	pr_info("%s done\n", __func__);
	return ;
}
EXPORT_SYMBOL(dp_usbpd_force_connect);
...
void dp_usbpd_connect(bool enable)
{
	...
	pr_info("%s: %d, hpd = %d\n", __func__, enable, globle_pd->dp_usbpd.base.hpd_high);
	if (globle_pd->dp_usbpd.base.hpd_high == 0)
	{
		pr_info("%s return\n", __func__);
		return;
	}

	globle_pd->dp_usbpd.base.hpd_high = enable;
	...
	globle_pd->dp_usbpd.base.hpd_high = 1;

error:
	pr_info("%s done\n", __func__);
	return ;
}
EXPORT_SYMBOL(dp_usbpd_connect);
...
struct dp_hpd *dp_usbpd_get(struct device *dev, struct dp_hpd_cb *cb)
{
	...
	globle_pd = usbpd;
	globle_pd->dp_usbpd.base.hpd_high = 0;
