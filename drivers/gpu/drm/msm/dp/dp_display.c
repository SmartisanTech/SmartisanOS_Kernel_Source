...
static int dp_power_status = 1;
bool dp_enable_start = false;
EXPORT_SYMBOL(dp_enable_start);
#define DP_MST_DEBUG(fmt, ...) pr_debug(fmt, ##__VA_ARGS__)

static struct dp_display *g_dp_display;
#define HPD_STRING_SIZE 30
#define FORCE_DP_OFF 3
#define FORCE_DP_ON 4

int dp_resolution_switch = 3840;
EXPORT_SYMBOL(dp_resolution_switch);
int dp_hdisplay = 3840;
EXPORT_SYMBOL(dp_hdisplay);
...
/*	vdd = regulator_get(&dp->pdev->dev, "vdd");
	if (IS_ERR(vdd)) {
		printk("dp get vconn failed\n");
	}
	rc = regulator_enable(vdd);
	if (rc) {
		printk("dp enable vconn failed\n");
		regulator_put(vdd);
	}
*/
...
/*	rcc = regulator_disable(vdd);
	if (rcc) {
		printk("dp disable vconn failed");
	}
	regulator_put(vdd);
*/
	return rc;
...
int lcd_panel_state = 1;
EXPORT_SYMBOL(lcd_panel_state);

static ssize_t dp_ctrl_status_store(struct device *dev,
                struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (!dp->is_connected)
	{
		pr_info("%s: dp not connect\n", __func__);
		return count;
	}
	rc = kstrtoint(buf, 0, &dp_power_status);
	if (rc)
	{
		pr_err("%s: kstrtoint failed. rc=%d\n", __func__, rc);
		return rc;
	}
	pr_info("dp controller idle status=%d\n", dp_power_status);
	if (dp_power_status == FORCE_DP_OFF)
	{
		pr_info("%s: force dp ctrl power down\n", __func__);
		dp_usbpd_force_connect(false);
		return count;
	}
	else if(dp_power_status == FORCE_DP_ON)
	{
		pr_info("%s: force dp ctrl power on\n", __func__);
		dp_usbpd_force_connect(true);
		return count;
	}

/*	if (!strncmp(global_mfg_id, "SMN", 3))
	{
		pr_debug("smartisan TNT return\n");
		return count;
	}*/
	if (audio_enabled)
	{
		pr_info("audio is enabled! return\n");
		return count;
	}

	if (dp_power_status == 0)
	{
		if (lcd_panel_state != 0)
		{
			pr_info("lcd panel is on\n");
			return count;
		}
		if (dp_enable_start)
		{
			pr_info("%s: wait dp enable done, return\n", __func__);
			dp_power_status = 1;
			return count;
		}
		dp_display_pre_disable(g_dp_display, dp->panel);
		dp_display_disable(g_dp_display, dp->panel);
	}
	else
	{
		if (dp_enable_start)
			return count;
		dp_enable_start = true;
		dp_display_enable(g_dp_display, dp->panel);
		dp_display_post_enable(g_dp_display, dp->panel);
	}

	return count;
}

static ssize_t dp_ctrl_status_show(struct device *dev,
                struct device_attribute *attr, char *buf)
{
	int len = 0;
	if (!dp_power_status)
		len = snprintf(buf, 10, "idle\n");
	else
		len = snprintf(buf, 10, "active\n");
	return len;
}

static ssize_t dp_resolution_store(struct device *dev,
                struct device_attribute *attr, const char *buf, size_t count)
{
	int rc = 0;
	int temp;
	...
	if (!dp)
	{
		pr_err("dp device erro\n");
		return count;
	}

	rc = kstrtoint(buf, 0, &temp);
	if (rc)
	{
		pr_err("%s: kstrtoint failed. rc=%d\n", __func__, rc);
		return rc;
	...
	dp_resolution_switch = temp;
	pr_info("dp switch resolution: %d\n", dp_resolution_switch);
	pr_info("last resolution: %dx%d@%dfps\n", dp->panel->pinfo.h_active,
			dp->panel->pinfo.v_active, dp->panel->pinfo.refresh_rate);
	if (dp->panel->pinfo.v_active == dp_resolution_switch)
	{
		pr_info("dp resolution no need switch\n");
		return count;
	}
reconnect:
	dp_usbpd_connect(false);
	dp_usbpd_connect(true);
	g_dp_display->post_open = NULL;
	return count;
}

static ssize_t dp_resolution_show(struct device *dev,
                struct device_attribute *attr, char *buf)
{
	int len = 0;
	len = snprintf(buf, SZ_8, "%d\n", dp_hdisplay);
...
static DEVICE_ATTR(dp_ctrl_status, 0444, dp_ctrl_status_show, dp_ctrl_status_store);
static DEVICE_ATTR(dp_resolution, 0444, dp_resolution_show, dp_resolution_store);

static const struct attribute *usb_dp_attrs[] = {
	&dev_attr_link_training.attr,
	&dev_attr_connected.attr,
	&dev_attr_dp_ctrl_status.attr,
	&dev_attr_dp_resolution.attr,
	NULL,
};
...
static int dp_display_probe(struct platform_device *pdev)
{
	...
	if (IS_ERR(vdd)) {
		printk("probe: DP get vconn failed\n");
	}
	rc = regulator_enable(vdd);
	if (rc) {
		printk("probe: DP enable vconn failed\n");
	}
