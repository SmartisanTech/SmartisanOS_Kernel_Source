...
/* modified based on show_ion_usage() of kernel 4.9, which is no long existing */
void ion_meminfo_dev(struct ion_meminfo_data *d, struct ion_device *dev)
{
	struct ion_heap *heap;

	if (!down_read_trylock(&dev->lock)) {
		pr_err("Ion lock busy, do not print info\n");
		return;
	}
	plist_for_each_entry(heap, &dev->heaps, node) {
		d->total_alloc += (unsigned long) atomic_long_read(&heap->total_allocated);
		if (heap->type == ION_HEAP_TYPE_SYSTEM) {
			 d->system_alloc += (unsigned long) atomic_long_read(&heap->total_allocated);
			 if (heap->ops->cached)
				 d->cached += heap->ops->cached(heap);
		}
	}
	up_read(&dev->lock);
