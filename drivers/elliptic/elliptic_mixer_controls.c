...
static struct elliptic_engine_diagnostics_data
	elliptic_engine_diagnostics_data_cache = { .reserved = {
...
static uint32_t ultrasound_headphonestatus_cache;

int elliptic_ultrasound_headphonestatus_get(struct snd_kcontrol *kcontrol,
					  struct snd_ctl_elem_value *ucontrol)
{
	ucontrol->value.integer.value[0] = ultrasound_headphonestatus_cache;
	return 0;
}

int elliptic_ultrasound_headphonestatus_set(struct snd_kcontrol *kcontrol,
					  struct snd_ctl_elem_value *ucontrol)
{
	int32_t msg[4] = {0, 0, 0, 0};

	ultrasound_headphonestatus_cache = ucontrol->value.integer.value[0];

	msg[0] = ultrasound_headphonestatus_cache ? 1 : 0;

	return elliptic_data_write(
		ELLIPTIC_ULTRASOUND_HEADPHONE_STATUS,
		(const char *)msg, sizeof(msg));
}
...
static const struct snd_kcontrol_new ultrasound_filter_mixer_controls[] = {
	...
	SOC_ENUM_EXT("Ultrasound HeadphoneStatus",
	elliptic_enum[0],
	elliptic_ultrasound_headphonestatus_get,
	elliptic_ultrasound_headphonestatus_set),
