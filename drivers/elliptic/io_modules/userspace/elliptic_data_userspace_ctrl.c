...
// static void set_ping_buffer_size(struct elliptic_userspace_ctrl_device *dev,
//     size_t data_size)
// {
//     dev->ping_pong_buffer_size[dev->ping_pong_idx] = data_size;
// }
    ...
    size_t user_buf_length, loff_t *ppos)
{
    ...
    if(user_buf_length < ELLIPTIC_MSG_BUF_SIZE){
        EL_PRINT_E("user space buffer user_buf_length too small : %zu", user_buf_length);
    }
    ...
    result = wait_event_interruptible( ctrl_device.data_available, atomic_read( &ctrl_device.data_state ) != 0 );
    if( 0 == result )
    {
        ...
        if( state > 0 )
        {
            ...
            if( 0 == result )
            {
                ...
                if(bytes_read > user_buf_length){
                    EL_PRINT_E("ping buffer size %zu larger than user buffer",
                        bytes_read);
                    goto fail;
                }

                copy_result = copy_to_user( buff, ping_buffer, bytes_read );
                if(copy_result > 0) {
...
void elliptic_userspace_ctrl_driver_exit(void)
{
    ...
    unregister_chrdev(elliptic_userspace_ctrl_major, USERSPACE_CTRL_IO_DEVICE_NAME);
    up(&ctrl_device.sem);
}
