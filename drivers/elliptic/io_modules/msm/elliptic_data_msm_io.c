...
int elliptic_io_open_port(int portid){
	if (portid == ULTRASOUND_RX_PORT_ID)
		return 0;//afe_start_pseudo_port(AFE_MSM_RX_PSEUDOPORT_ID);
	else
		return 0;//afe_start_pseudo_port(AFE_MSM_TX_PSEUDOPORT_ID);
}

int elliptic_io_close_port(int portid){
	if (portid == ULTRASOUND_RX_PORT_ID)
		return 0;//afe_stop_pseudo_port(AFE_MSM_RX_PSEUDOPORT_ID);
	else
