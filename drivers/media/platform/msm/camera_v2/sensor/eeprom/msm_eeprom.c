	...
	for (j = 0; j < eeprom_map_array->msm_size_of_max_mappings; j++) {
		...
		for (i = 0; i < eeprom_map->memory_map_size; i++) {
			if ((eeprom_map->mem_settings[i].i2c_operation == MSM_CAM_READ) ||
			        (eeprom_map->mem_settings[i].i2c_operation == MSM_CAM_READ_GC8034) ||
			        (eeprom_map->mem_settings[i].i2c_operation == MSM_CAM_READ_CONTINUOUS))
			{
                                   ...
                                   struct msm_eeprom_memory_map_array *eeprom_map_array)
{
	int rc =  0, i, j, k , gc;
	uint8_t *memptr;
	uint16_t gc_read = 0, gc_readf4 = 0;
	struct msm_eeprom_mem_map_t *eeprom_map;

	uint16_t hi_data = 0x0201;
	...
	for (j = 0; j < eeprom_map_array->msm_size_of_max_mappings; j++) {
		...
		for (i = 0; i < eeprom_map->memory_map_size; i++) {
			...
			case MSM_CAM_READ_CONTINUOUS: {
				...
				for (k = 0; k < eeprom_map->mem_settings[i].reg_data; k += 12) {

					e_ctrl->i2c_client.i2c_func_tbl->i2c_write(
					    &(e_ctrl->i2c_client), 0x070a, (uint8_t)((hi_data>>8)&0xFF), 1);

					e_ctrl->i2c_client.i2c_func_tbl->i2c_write(
					    &(e_ctrl->i2c_client), 0x070b, (uint8_t)(hi_data&0xFF), 1);

					e_ctrl->i2c_client.i2c_func_tbl->i2c_write(
					    &(e_ctrl->i2c_client), 0x0702, 0x01, 1);

					e_ctrl->i2c_client.i2c_func_tbl->i2c_write(
					    &(e_ctrl->i2c_client), 0x0f12, 0x01, 1);

					rc = e_ctrl->i2c_client.i2c_func_tbl->
					     i2c_read_seq(&(e_ctrl->i2c_client),
					                  eeprom_map->mem_settings[i].reg_addr,
					                  memptr,
					                  (eeprom_map->mem_settings[i].reg_data - k) >= 12 ? 12 : (eeprom_map->mem_settings[i].reg_data - k));
					msleep(eeprom_map->mem_settings[i].delay);
					if (rc < 0) {
						pr_err("%s: read failed\n",
						       __func__);
					...
					e_ctrl->i2c_client.i2c_func_tbl->i2c_write(
					    &(e_ctrl->i2c_client), 0x0f12, 0x00, 1);
...
//begin: eko add for writing data to eeprom 2016-12-02
                                  ...
                                  struct msm_eeprom_write_map_array *eeprom_map_array)
{
	...
	for (j = 0; j < eeprom_map_array->msm_size_of_max_mappings; j++) {
		...
		CDBG("[EEPDEBUG]Slave Addr: 0x%X\n", eeprom_map->slave_addr);
		CDBG("[EEPDEBUG]Memory map Size: %d",
...
//end: eko add for writing data to eeprom 2016-12-02
...
//begin: eko add for write data to eeprom,almost the same as eerpom_init_config32 2016-12-02
...
//end: eko add for write data to eeprom,almost the same as eerpom_init_config32 2016-12-02
