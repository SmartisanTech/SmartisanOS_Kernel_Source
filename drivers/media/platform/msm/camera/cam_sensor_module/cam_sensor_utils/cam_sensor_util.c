...
#define FRONT_CAM_RESET_GPIO 28
#define REAR_CAM_RESET_GPIO 25
#define SWITCH_CSI_GPIO 42
	...
	struct msm_camera_gpio_num_info *gpio_num_info, int val)
{
	...
	if (gpio_num_info->valid[gpio_offset] == 1) {
        ...
        /*gpio SWITCH_CSI_GPIO:42 switch 1 for 20M front camera mipi csi<if FRONT_CAM_RESET_GPIO:28 to reset 1 when power up, only for delta>*/
        if(gpio_num_info->gpio_num[gpio_offset] == FRONT_CAM_RESET_GPIO && val == 1){
            cam_res_mgr_gpio_set_value(
                    SWITCH_CSI_GPIO, 1);
        }

        /*gpio SWITCH_CSI_GPIO:42 switch 0 for 5M mipi csi<if REAR_CAM_RESET_GPIO:25 to reset 1 when power up, only for delta>*/
        if(gpio_num_info->gpio_num[gpio_offset] == REAR_CAM_RESET_GPIO && val == 1){
            cam_res_mgr_gpio_set_value(
                    SWITCH_CSI_GPIO, 0);
        }
	}
