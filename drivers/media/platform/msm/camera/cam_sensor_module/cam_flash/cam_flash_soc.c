...
#define FLASH_VERSION_0 0  //low version which only has one flash
#define FLASH_VERSION_1 1  //high version of mahjong for delta which has three flash
	...
	struct cam_flash_private_soc *soc_private)
{
	...
	uint32_t flash_version_flag = 0, index_start = 0;
	...
	if (of_get_property(of_node, "flash-source", &count)) {
        ...
        flash_version_flag = (get_flash_version() == 1)? FLASH_VERSION_1 : FLASH_VERSION_0;
        CAM_INFO(CAM_FLASH, "flash-source flash_version_flag:%d", flash_version_flag);
		if(flash_version_flag == FLASH_VERSION_1 && count == 3)
		{
			index_start = 1;
			CAM_INFO(CAM_FLASH, "flash-source index_start:%d", index_start);
		}
	...
	if (of_get_property(of_node, "torch-source", &count)) {
        ...
        flash_version_flag = (get_flash_version() == 1)? FLASH_VERSION_1 : FLASH_VERSION_0;
        CAM_INFO(CAM_FLASH, "torch-source flash_version_flag:%d", flash_version_flag);
		if(flash_version_flag == FLASH_VERSION_1 && count == 3)
		{
			index_start = 1;
			CAM_INFO(CAM_FLASH, "torch-source index_start:%d", index_start);
		}
