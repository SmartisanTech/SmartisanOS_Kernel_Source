...
#include "../../../../drivers/input/touchscreen/novatek_touch/nt36xxx.h"
...
struct dsi_panel_cmds color_temperature_cmds_ft8716_first;
struct dsi_panel_cmds color_temperature_cmds_reset_ft8716_first;
struct dsi_panel_cmds color_temperature_cmds_sharp_ft8716_first;
struct dsi_panel_cmds color_temperature_cmds_reset_sharp_ft8716_first;
struct dsi_panel_cmds cabc_ui_mode_ft8716;
struct dsi_panel_cmds cabc_still_mode_ft8716;
struct dsi_panel_cmds cabc_moving_mode_ft8716;
struct dsi_panel_cmds cabc_moving_mode_sharp_ft8716;
struct dsi_panel_cmds cabc_moving_mode_nt36672;
...
static void nvt_glove_write(int mode)
{
	char tmp_buf[8];
...
static int fts_enter_glove_mode(struct i2c_client *client, int mode)
{
    int ret = 0;
...
void mdss_dsi_panel_ie_level_setting(struct mdss_panel_data *pdata, int level)
{
	...
	if ((level > 3) && (level < 26))
	{
		...
		case 9:
			if ( gpio_id1 == 0 && gpio_id2 == 1)
				color_temperature_cmd = &color_temperature_cmds_ft8716_first;
			else if (gpio_id1 == 0 && gpio_id2 == 0) {
				color_temperature_cmd = &color_temperature_cmds_sharp_ft8716_first;
			}
			break;
		case 19:
			if ( gpio_id1 == 0 && gpio_id2 == 1)
				color_temperature_cmd = &color_temperature_cmds_reset_ft8716_first;
			else if (gpio_id1 == 0 && gpio_id2 == 0) {
				color_temperature_cmd = &color_temperature_cmds_reset_sharp_ft8716_first;
			}
			break;
		case 6:
			color_temperature_cmd = &cabc_ui_mode_ft8716;
			break;
		case 7:
			color_temperature_cmd = &cabc_still_mode_ft8716;
			break;
		case 8:
			if ( gpio_id1 == 0 && gpio_id2 == 1)
				color_temperature_cmd = &cabc_moving_mode_ft8716;
			else if (gpio_id1 == 0 && gpio_id2 == 0) 
				color_temperature_cmd = &cabc_moving_mode_sharp_ft8716;
			break;
		case 13:
			color_temperature_cmd = &cabc_moving_mode_nt36672;
			break;
		case 18:
	...
	else if (level == 178)
	{
		...
		if((gpio_id1 == 1) && (gpio_id2 == 1)){
			nvt_point_stable_write(regvalue);
		}
		else if(( gpio_id1 == 0 && gpio_id2 == 1) || (gpio_id1 == 0 && gpio_id2 == 0)){
			mutex_lock(&fts_input_dev->mutex);
			fts_i2c_write_reg(fts_i2c_client, FTS_REG_POINT_STABLE_VER, regvalue);
			mutex_unlock(&fts_input_dev->mutex);
		}
	}else if(level == 261){
		glove_open = 1;
		if((gpio_id1 == 1) && (gpio_id2 == 1)){
			nvt_glove_write(0x71);
		}
		else if(( gpio_id1 == 0 && gpio_id2 == 1) || (gpio_id1 == 0 && gpio_id2 == 0)){
			fts_enter_glove_mode(fts_i2c_client, 1);
		}
	}else if(level == 260){
		glove_open = 0;
		if((gpio_id1 == 1) && (gpio_id2 == 1)){
			nvt_glove_write(0x72);
		}
		else if(( gpio_id1 == 0 && gpio_id2 == 1) || (gpio_id1 == 0 && gpio_id2 == 0)){
			fts_enter_glove_mode(fts_i2c_client, 0);
		}
	}
	else {
...
static bool mdss_dsi_cmp_panel_reg_v2(struct mdss_dsi_ctrl_pdata *ctrl)
{
	...
	pr_err("%s: errrrrr:xu: reg val is %x......\n", __func__, ctrl->return_buf[i]);
	return false;
}
		...
		bool default_timing)
{
	...
	mdss_dsi_parse_dcs_cmds(np, &color_temperature_cmds_ft8716_first,
		"qcom,mdss-dsi-panel-color-temperature-command-ft8716-first", NULL);

	mdss_dsi_parse_dcs_cmds(np, &color_temperature_cmds_reset_ft8716_first,
		"qcom,mdss-dsi-panel-color-temperature-command-restore-ft8716-first", NULL);

	mdss_dsi_parse_dcs_cmds(np, &color_temperature_cmds_sharp_ft8716_first,
		"qcom,mdss-dsi-panel-color-temperature-command-sharp-ft8716-first", NULL);

	mdss_dsi_parse_dcs_cmds(np, &color_temperature_cmds_reset_sharp_ft8716_first,
		"qcom,mdss-dsi-panel-color-temperature-command-restore-sharp-ft8716-first", NULL);

	mdss_dsi_parse_dcs_cmds(np, &cabc_still_mode_ft8716,
		"qcom,mdss-dsi-panel-cabc-still-mode-ft8716", NULL);

	mdss_dsi_parse_dcs_cmds(np, &cabc_moving_mode_ft8716,
		"qcom,mdss-dsi-panel-cabc-moving-mode-ft8716", NULL);

	mdss_dsi_parse_dcs_cmds(np, &cabc_moving_mode_sharp_ft8716,
		"qcom,mdss-dsi-panel-cabc-moving-mode-sharp-ft8716", NULL);
		
	mdss_dsi_parse_dcs_cmds(np, &cabc_moving_mode_nt36672,
		"qcom,mdss-dsi-panel-cabc-moving-mode-nt36672", NULL);

	mdss_dsi_parse_dcs_cmds(np, &cabc_ui_mode_ft8716,
		"qcom,mdss-dsi-panel-cabc-ui-mode-ft8716", NULL);
