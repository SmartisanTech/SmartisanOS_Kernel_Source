...
struct gf_key_map maps[] = {
	...
	{ EV_KEY, GF_KEY_INPUT_FINGERPRINT },
...
static void gf_kernel_key_input(struct gf_dev *gf_dev, struct gf_key *gf_key)
{
	...
	} else if (GF_KEY_FINGERPRINT == gf_key->key) {
		key_input = GF_KEY_INPUT_FINGERPRINT;
	} else {
	...
	if ((GF_KEY_POWER == gf_key->key || GF_KEY_BACK == gf_key->key || GF_KEY_FINGERPRINT == gf_key->key)
			& (gf_key->value == 1))
	{
...
int goodix_lcd_state_chg_callback(int lcd_on)
{
	char msg = 0;
	struct gf_dev *gf_dev = &gf;

	pr_info("[info] %s go to the goodix_lcd_state_chg_callback value = %d\n",
			__func__,lcd_on);
...
#endif
{
	...
	vreg_setup(gf_dev, "goodix_pwr2", 1);
...
static int __init gf_init(void)
{
	...
	pr_warn("%s goodix, fingerprint_id = %s \n", __func__, fingerprint_id);
	if (strcmp(fingerprint_id, "goodix")) {
		pr_err("%s goodix error, fingerprint_id = %s \n", __func__, fingerprint_id);
		return -1;
	}
