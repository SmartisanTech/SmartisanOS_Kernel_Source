...
static int get_fingerprint_id(char *src)
{
    ...
    NVT_DBG_ERR("kernel detect fingerprint_id = *%s*\n", fingerprint_id);
 ...
 */
static int __init nvt_fpr_sdm670_init(void)
{
    int retval = 0;
...
static void __exit nvt_fpr_sdm670_exit(void)
{
...
late_initcall(nvt_fpr_sdm670_init);
module_exit(nvt_fpr_sdm670_exit);
