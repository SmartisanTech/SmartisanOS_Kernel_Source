...
static bool touchscroll_flag = false;
static int y_start = 0;
static int x_start = 0;
static int y_end = 0;
static int x_end = 0;
int input_boost_ms_temp = 40;
u64 t_start = 0;
u64 t_end = 0;
extern void cpuboost_touch_up_event(int);
//static struct delayed_work touch_up_work;
static struct workqueue_struct *touch_up_wq;

static bool sleep_state = false;
static bool suspend_end = false;

//static void touch_scroll_event(struct work_struct *work);
...
static void touch_scroll_event(struct work_struct *work)
{
	...
	int time_diff = (int)(t_end - t_start);
	if (y_diff < 0)
		y_diff = -y_diff;
	if (x_diff < 0)
		x_diff = -x_diff;
	mv_velocity = y_diff * 10 / time_diff;
	if ((y_diff < 80) || (x_diff > y_diff))
	{
		input_boost_ms_temp = 100;
		cpuboost_touch_up_event(input_boost_ms_temp);
	}
		...
		struct synaptics_rmi4_fn *fhandler)
{
	...
	for (finger = 0; finger < fingers_to_process; finger++) {
		...
		if (finger_status == F12_FINGER_STATUS) {
			...
			if (!touchscroll_flag)
			{
				touchscroll_flag = true;
				t_start = ktime_to_ms(ktime_get());
				y_start = y;
				x_start = x;
			}
			y_end = y;
	...
	if (touch_count == 0) {
		//cancel_delayed_work_sync(&touch_up_work);
#ifdef F12_DATA_15_WORKAROUND
		fingers_already_present = 0;
#endif
		touchscroll_flag = false;
		input_report_key(rmi4_data->input_dev,
				BTN_TOUCH, 0);
		input_report_key(rmi4_data->input_dev,
				BTN_TOOL_FINGER, 0);
		...
		input_mt_sync(rmi4_data->input_dev);
#endif
		t_end = ktime_to_ms(ktime_get());
	}

	input_sync(rmi4_data->input_dev);

//	if (touch_count == 0)
//		queue_delayed_work(touch_up_wq, &touch_up_work, msecs_to_jiffies(50));
...
static void synaptics_init(struct work_struct *work)
{
	struct synaptics_rmi4_data *rmi4_data =
		container_of(to_delayed_work(work), struct synaptics_rmi4_data, init_work);
...
static int synaptics_rmi4_probe(struct platform_device *pdev)
{
	...
	if (bdata->i2c_pull_up) {
		rmi4_data->vcc_i2c = bdata->vcc_i2c;
	...
	touch_up_wq = create_singlethread_workqueue("touchup_wq");
	//INIT_DELAYED_WORK(&touch_up_work, touch_scroll_event);
	rmi4_data->init_workqueue = create_singlethread_workqueue("init_workqueue");
	INIT_DELAYED_WORK(&rmi4_data->init_work, synaptics_init);
...
#else
err_set_gpio:
	/* if lvs1 off, i2c bus will lead
	 * system can't sleep
	if (rmi4_data->vcc_i2c) {
				...
				unsigned long event, void *data)
{
	...
	case LCD_EVENT_ON_START:
		...
		if(sleep_state == true)
		{
			printk("synaptics tp on start\n");
			if (bdata->disable_gpios) {
				if (rmi4_data->ts_pinctrl) {
					retval = pinctrl_select_state(rmi4_data->ts_pinctrl,
							rmi4_data->pinctrl_state_active);
					if (retval < 0)
						printk("Cannot get default pinctrl state\n");
				}

				retval = synaptics_dsx_gpio_configure(rmi4_data, true);
				if (retval < 0)
					printk("Failed to put gpios in active state\n");
			}
		}
#endif
		gpio_set_value(bdata->reset_gpio, 1);
		if (!cancel_delayed_work_sync(&rmi4_data->init_work))
			queue_delayed_work(rmi4_data->init_workqueue, &rmi4_data->init_work, msecs_to_jiffies(60));
		break;
	case LCD_EVENT_ON_END:
	//	queue_delayed_work(rmi4_data->init_workqueue, &rmi4_data->init_work, msecs_to_jiffies(220));
		break;
	case LCD_EVENT_OFF_START:
		if(sleep_state == false)
		{
			printk("synaptics tp off start\n");
			if (!cancel_delayed_work_sync(&rmi4_data->init_work))
			{
			...
			if (bdata->disable_gpios) {
				if (rmi4_data->ts_pinctrl) {
					retval = pinctrl_select_state(rmi4_data->ts_pinctrl,
							rmi4_data->pinctrl_state_suspend);
					if (retval < 0) {
						printk("Cannot get idle pinctrl state\n");
					}
				}

				retval = synaptics_dsx_gpio_configure(rmi4_data, false);
				if (retval < 0) {
					printk("failed to put gpios in suspend state\n");
				}
			}
#endif
		}
