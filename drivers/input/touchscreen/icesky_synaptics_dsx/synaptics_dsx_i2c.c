...
static int synaptics_rmi4_parse_dt(struct device *dev, struct synaptics_dsx_board_data *pdata)
{
	...
	pdata->irq_gpio = of_get_named_gpio_flags(np, "synaptics,irq-gpio", 0, &pdata->irq_flags);
	...
	if (of_find_property(np, "synaptics,id-gpio", NULL))
		pdata->id_gpio = of_get_named_gpio_flags(np, "synaptics,id-gpio", 0, NULL);
	else
		pdata->id_gpio = -1;
		...
		const struct i2c_device_id *dev_id)
{
	...
	if (client->dev.of_node) {
		...
		if (hw_if.board_data->id_gpio > 0) {
			retval = gpio_get_value(hw_if.board_data->id_gpio);
			if (1 == retval)
			{
				printk("gpio %d is 1, JDI Panel not used\n", hw_if.board_data->id_gpio);
				return -EIO;
			}
		}
	}
