...
static unsigned int major_parameter = 70;
...
#ifdef REPORT_TOUCH_PRESSURE
	TOUCH_FORCE_VALUE = 0xc9,
#endif
};
 ...
 */
bool need_report_pressure = true;
...
#ifdef REPORT_TOUCH_PRESSURE
		case TOUCH_FORCE_VALUE:
			if (!need_report_pressure)
				continue;
			bits = config_data[idx++];
			retval = touch_get_report_data(offset, bits, &data);
			if (retval < 0) {
				LOGE(tcm_hcd->pdev->dev.parent,
						"Failed to get object force value\n");
				return retval;
			}
			object_data[obj].force_val = data;
			offset += bits;
#endif
		}
	}
	...
	for (idx = 0; idx < touch_hcd->max_objects; idx++) {
		...
		case GLOVED_FINGER:
...
#ifdef REPORT_TOUCH_PRESSURE
			pressure = object_data[idx].force_val;
			pressure = pressure > 0 ? pressure : 1;
			if (pressure > touch_hcd->force_max)
					pressure = touch_hcd->force_max;
			input_report_abs(touch_hcd->input_dev,
			...
			LOGD(tcm_hcd->pdev->dev.parent,
					"Finger %d: x = %d, y = %d size(max) = %d size(min) = %d pressure = %d major_parameter:%d\n",
					idx, x, y, max(wx, wy), min(wx, wy), pressure, major_parameter);
			touch_count++;
			break;
		default:
			break;
...
static int touch_set_input_params(void)
{
	...
	input_set_abs_params(touch_hcd->input_dev,
			ABS_MT_TOUCH_MAJOR, 0, major_parameter, 0, 0);
...
#ifdef REPORT_TOUCH_PRESSURE
	input_set_abs_params(touch_hcd->input_dev,
				ABS_MT_PRESSURE, 0, touch_hcd->force_max, 0, 0);
#endif
