...
static int testing_dynamic_range(void)
{
	...
	if (IS_ERR(pfile)) {
		...
		//BSP_SYS: Alex_Ma(majianbo@smatisan.com) fix leak issue with Coverity tool on 2018-4-12,begin
			if (fbufp) {
				kfree(fbufp);
				fbufp = NULL;
				}
		//BSP_SYS: Alex_Ma(majianbo@smatisan.com) fix leak issue with Coverity tool on 2018-4-12,end
		return -EIO;
	...
	for (row = 0; row < rows; row++) {
		for (col = 0; col < cols; col++) {
			data = le2_to_uint(&buf[idx * 2]);
			temp_count = row * cols + col;
			sprintf(fbufp + temp_count * 7, "%5d,", data);
			if (data > drt_hi_limits[row][col] ||
					data < drt_lo_limits[row][col]) {
				testing_hcd->result = false;
				break;
		...
		sprintf(fbufp + (temp_count + 1) * 7 ,"\r\n");
	}
	vfs_write(pfile, (char __user *)fbufp,  rows * cols * 7, &pos);
...
static int testing_pt11(void)
{
	...
	fbufp = (char *)kzalloc(8192, GFP_KERNEL);
	memset(filepath, 0, sizeof(filepath));
	sprintf(filepath, "%s%s", SYNA_INI_FILE_PATH, "opendata.csv");
	if (NULL == pfile)
		pfile = filp_open(filepath, O_CREAT|O_RDWR, 0);
	if (IS_ERR(pfile)) {
		pr_err("error occured while opening file %s.",  filepath);
...
static int testing_pt12(void)
{
	...
	fbufp = (char *)kzalloc(8192, GFP_KERNEL);
	memset(filepath, 0, sizeof(filepath));
	sprintf(filepath, "%s%s", SYNA_INI_FILE_PATH, "h_shortdata.csv");
	if (NULL == pfile)
		pfile = filp_open(filepath, O_CREAT|O_RDWR, 0);
	if (IS_ERR(pfile)) {
		pr_err("error occured while opening file %s.",  filepath);
