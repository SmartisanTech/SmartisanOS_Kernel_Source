...
struct ft5x06_ts_data *globle_ft5x06_ts_data;
...
struct ft5x06_point_record {
        int is_record;
        int updated;
};

struct ft5x06_point_record record[10];

#define POINT_IN_RECT(x,y,keyRect)  (abs(x - keyRect.x) <= keyRect.dx) && (abs(y - keyRect.y) <= keyRect.dy)
...
int get_tp_vendor_id (void)
{
	unsigned char reg_addr;
	unsigned char reg_value;
	int err;
	reg_addr= 0xa8;
	err = ft5x06_i2c_read(this_client, &reg_addr, 1, &reg_value, 1);
	if (err < 0) {
		dev_err(&this_client->dev, "vendor id  read failed");
	}
	return reg_value;
}
...
static int ft5x0x_touch_key_process(struct ft5x06_ts_data *data, struct input_dev *dev, int x, int y, int touch_event)
{
        ...
        pdata = data->pdata;
        for(i = 0; i < pdata->tsp_key_num; i++)
        {
                if(POINT_IN_RECT(x,y,pdata->tsp_key[i].key_rect))
                {
                        if(touch_event == FT_KEY_PRESS) {
                                pr_debug("report press!!\n");
                                input_report_key(dev, pdata->tsp_key[i].key_code, 1);
                                pdata->tsp_key[i].key_status = FT_KEY_PRESS;
				pdata->tsp_key[i].update_status = 1;
                        }else if(touch_event == FT_KEY_RELEASE){
                                if(pdata->tsp_key[i].key_status == FT_KEY_PRESS){
                                        pr_debug("report release!!\n");
                                        input_report_key(dev, pdata->tsp_key[i].key_code, 0);
                                }
                                pdata->tsp_key[i].key_status = FT_KEY_RELEASE;
				pdata->tsp_key[i].update_status = 1;
                        }
                }
        }
...
static void ft5x0x_ts_reset_work (struct work_struct *work)
{
        struct ft5x06_ts_data *data = container_of(work, struct ft5x06_ts_data, tp_reset_work);
...
static void ft5x0x_ts_pen_irq_work(struct work_struct *work)
{
	...
	if (rc < 0) {
                ...
                {
                        gpio_set_value (data->pdata->reset_gpio,0);
                        schedule_work (&data->tp_reset_work);
                        fail_num = 0;
                        data->read_error = 1;
                }
	...
	for (i = 0; i < data->pdata->num_max_touches; i++) {
		...
		y = (buf[FT_TOUCH_Y_H_POS + FT_ONE_TCH_LEN * i] & 0x0F) << 8 |
			(buf[FT_TOUCH_Y_L_POS + FT_ONE_TCH_LEN * i]);
                ...
                press = buf[FT_TOUCH_PRESS_POS+ FT_ONE_TCH_LEN * i];
		...
		if (0) {
                        ft5x0x_touch_key_process(data, ip_dev, x, y, status);
                        continue;
                }
		...
		if (status == FT_TOUCH_DOWN || status == FT_TOUCH_CONTACT) {
			...
			input_report_abs(ip_dev, ABS_MT_POSITION_Y, y);
                        input_report_abs(ip_dev, ABS_MT_TOUCH_MAJOR,(int)(press*7/6));
                        record[id].is_record = 1;
                        record[id].updated = 1;
		}  else {
			input_mt_report_slot_state(ip_dev, MT_TOOL_FINGER, 0);
                        record[id].is_record = 0;
                        record[id].updated = 0;
		}
	}

	for (i = 0; i < data->pdata->num_max_touches; i++) {
                if (record[i].is_record && !record[i].updated) {
                        /*Now we lost the finger release event.*/
                        input_mt_slot(ip_dev, i);
                        input_mt_report_slot_state(ip_dev, MT_TOOL_FINGER, 0);
                        record[i].is_record = 0;
                }
                record[i].updated = 0;
        }
       if (suspend_state){
	for (i = 0; i < data->pdata->num_max_touches; i++) {
        ...
        {
                                if(data->pdata->tsp_key[i].key_status == FT_KEY_PRESS){
	...
	for(i = 0; i < data->pdata->tsp_key_num; i++)
	{
		...
		data->pdata->tsp_key[i].update_status = 0;
	}
	#endif
	if (update_input) {
...
static irqreturn_t ft5x06_ts_interrupt(int irq, void *dev_id)
{
	struct ft5x06_ts_data *data = dev_id;
        ...
        ft5x0x_ts_pen_irq_work(&data->pen_event_work);
        if (!data->read_error)
                enable_irq(this_client->irq);
        return IRQ_HANDLED;
}
...
static void ft5x0x_ts_delay_resume_work(struct work_struct *work)
{
	...
	enable_irq(data->client->irq);
	    if(data->usb_in)
        {
               ft5x0x_write_reg(data->client,0x8b,1);
        }
...
static int ft5x06_power_on(struct ft5x06_ts_data *data, bool on)
{
	...
	if (rc) {
        ...
        rc = gpio_direction_output(data->pdata->reset_gpio, 0);
        if (rc) {
                dev_err(&data->client->dev,
	...
	if (rc) {
        ...
        msleep(3);
        rc = gpio_direction_output(data->pdata->reset_gpio, 1);
        if (rc) {
                dev_err(&data->client->dev,
                        "set_direction for reset gpio failed\n");
                return rc;
...
extern bool get_usb_presenti_for_tp(void);
...
static int ft5x06_ts_resume(struct device *dev)
{
	...
	if (gpio_is_valid(data->pdata->reset_gpio)) {
        ...
        //msleep(data->pdata->soft_rst_dly-50);
        schedule_delayed_work(&data->delay_resume_work,msecs_to_jiffies(data->pdata->soft_rst_dly));
	/*msleep(data->pdata->soft_rst_dly);
...
static ssize_t ft5x06_tp_vendor_show(struct device *dev,
				struct device_attribute *attr, char *buf)
{
	...
	return sprintf(buf,  "%s\n", tp_vendor[data->tp_id]);
}
static DEVICE_ATTR(tp_vendor, 0444, ft5x06_tp_vendor_show,
                                 NULL);
	...
	if (prop) {
		...
		if (num_buttons > MAX_BUTTONS) {
			return -EINVAL;
                ...
                pdata->tsp_key_num = num_buttons;

		rc = of_property_read_u32_array(np,
			"focaltech,button-map", button_map,
			num_buttons);

                pdata->tsp_key = (struct tsp_key *)kmalloc(sizeof(struct tsp_key) * num_buttons, GFP_KERNEL);
                unit_rect = (pdata->panel_maxx - pdata->panel_minx) / num_buttons;
                for (i = 0; i < num_buttons; i++) {
                        pdata->tsp_key[i].key_code = button_map[i];
                        pdata->tsp_key[i].key_status = FT_KEY_RELEASE;
			pdata->tsp_key[i].update_status = 0;
                        pdata->tsp_key[i].key_rect.x =
                                i * unit_rect + unit_rect / 2;
                        pdata->tsp_key[i].key_rect.y =
                                pdata->y_max + (pdata->panel_maxy - pdata->y_max) / 2;
                        pdata->tsp_key[i].key_rect.dx =
                                unit_rect / 3;
                        pdata->tsp_key[i].key_rect.dy =
                                (pdata->panel_maxy - pdata->y_max) / 3;
                        ...
                        pr_debug("tpkey[%d] :\n"
                                ...
                                "key_rect dx is %d\n"
                                "key_rect dy is %d\n",
                                i, pdata->tsp_key[i].key_code,
                                pdata->tsp_key[i].key_status,
                                pdata->tsp_key[i].key_rect.x,
                                pdata->tsp_key[i].key_rect.y,
                                pdata->tsp_key[i].key_rect.dx,
                                pdata->tsp_key[i].key_rect.dy);
			   ...
			   const struct i2c_device_id *id)
{
	...
	u8 reg_addr;
	int err, len, i;
        if (!power_supply_get_by_name("battery"))
        {
                return -EPROBE_DEFER;
                }
	...
	__set_bit(INPUT_PROP_DIRECT, input_dev->propbit);
        ...
        if (pdata->tsp_key) {
                ...
                input_dev->keycode = pdata->tsp_key;
        }
	...
	input_set_abs_params(input_dev, ABS_MT_POSITION_Y, pdata->y_min,
        ...
        for (i = 0; i < pdata->tsp_key_num; i++) {
                input_set_capability(input_dev, EV_KEY, pdata->tsp_key[i].key_code);
        }
	...
	err = device_create_file(&client->dev, &dev_attr_tp_vendor);
	if (err) {
		dev_err(&client->dev, "sys file creation failed\n");
