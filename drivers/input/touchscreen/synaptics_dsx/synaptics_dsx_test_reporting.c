...
#define SYNA_INI_FILE_PATH	"/data/local/tmp/" 
...
#define EE_SHORT_TEST_LIMIT_PART2  80         // ( unit = ratio )
...
#define ELEC_OPEN_TEST_LIMIT_ONE 130
#define ELEC_OPEN_TEST_LIMIT_TWO 70
...
unsigned int raw_limit_low = 0;
unsigned int raw_limit_high = 4000;
...
#ifdef ENABLE_EXTEND_EE_SHORT
show_store_prototype(td43xx_ee_short)
#else
show_store_prototype(td43xx_noextern_ee_short)
#endif
#ifdef ENABLE_AMP_OPEN_B7
...
#else
	attrify(td43xx_noextern_ee_short),
#endif
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (rmi4_data->suspend) {
		dev_err(rmi4_data->pdev->dev.parent, "%s: in suspend state, return\n", __func__);
		mutex_unlock(&rmi4_data->rmi4_exp_init_mutex);
		return -EINVAL;
	}
	fbufp = (char *)kzalloc(8192, GFP_KERNEL);
	memset(filepath, 0, sizeof(filepath));
	sprintf(filepath, "%s%s", SYNA_INI_FILE_PATH, "shortDataPart1.csv");
	if (NULL == pfile)
		pfile = filp_open(filepath, O_CREAT|O_RDWR, 0);
	if (IS_ERR(pfile)) {
		pr_err("error occured while opening file %s.",  filepath);
		mutex_unlock(&rmi4_data->rmi4_exp_init_mutex);
		return -EIO;
	}
	...
	for (i = 0; i < tx_num; i++) {
...
#endif
			temp_count = i * rx_num + j;
			sprintf(fbufp + temp_count * 7 + i * 2,"%5d,",tddi_rt95_part_one[i*rx_num + j]);
		...
		sprintf(fbufp + (temp_count + 1) * 7 + i * 2,"\r\n");
	}
#ifdef F54_SHOW_MAX_MIN
	pr_info("%s : image part 1 data range (max, min) = (%-4d, %-4d)\n", __func__, max, min);
#endif

	vfs_write(pfile, (char __user *)fbufp,	tx_num * rx_num * 7 + tx_num * 2, &pos);
	...
	fbufp = (char *)kzalloc(8192, GFP_KERNEL);
	memset(filepath, 0, sizeof(filepath));
	sprintf(filepath, "%s%s", SYNA_INI_FILE_PATH, "shortDataPart2.csv");
	if (NULL == pfile2)
		pfile2 = filp_open(filepath, O_CREAT|O_RDWR, 0);
	if (IS_ERR(pfile2)) {
		pr_err("error occured while opening file %s.",	filepath);
		mutex_unlock(&rmi4_data->rmi4_exp_init_mutex);
		return -EIO;
...
#endif
			temp_count = i * rx_num + j;
			sprintf(fbufp + temp_count * 7 + i * 2,"%5d,",tddi_rt95_part_two[i*rx_num + j]);
	...
	vfs_write(pfile2, (char __user *)fbufp,	tx_num * rx_num * 7 + tx_num * 2, &pos);
	filp_close(pfile2, NULL);
	set_fs(old_fs);
	if (fbufp) {
		kfree(fbufp);
		fbufp = NULL;
...
static ssize_t test_sysfs_td43xx_noextern_ee_short_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
	int retval = 0;
	int i, j, offset;
	...
	fbufp = (char *)kzalloc(8192, GFP_KERNEL);
	memset(filepath, 0, sizeof(filepath));
	sprintf(filepath, "%s%s", SYNA_INI_FILE_PATH, "shortdata.csv");
	if (NULL == pfile)
		pfile = filp_open(filepath, O_CREAT|O_RDWR, 0);
	if (IS_ERR(pfile)) {
		pr_err("error occured while opening file %s.",  filepath);
	...
	set_fs(KERNEL_DS);
	pos = 0;
	report_data_16 = td43xx_rt95_part_one;

	for (i = 0; i < tx_num; i++) {
		for (j = 0; j < rx_num; j++) {
			temp_count = i * rx_num + j;
			sprintf(fbufp + temp_count * 7 + i * 2,"%5d,", *report_data_16);
			report_data_16++;
		}
		sprintf(fbufp + (temp_count + 1) * 7 + i * 2,"\r\n");
	}
...
static ssize_t test_sysfs_td43xx_noextern_ee_short_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	int i, j;
	...
	for (i = 0; i < tx_num; i++) {
		for (j = 0; j < rx_num; j++) {
			pr_err("%d	", td43xx_ee_short_data[i * rx_num + j]);
		...
		struct device_attribute *attr, char *buf)
{
	...
	fbufp = (char *)kzalloc(8192, GFP_KERNEL);
	memset(filepath, 0, sizeof(filepath));
	sprintf(filepath, "%s%s", SYNA_INI_FILE_PATH, "noisedata.csv");
	if (NULL == pfile)
		pfile = filp_open(filepath, O_CREAT|O_RDWR, 0);
	if (IS_ERR(pfile)) {
		pr_err("error occured while opening file %s.",  filepath);
	...
	for (i = 0; i < tx_num; i++) {
		for (j = 0; j < rx_num; j++) {
			temp_count = i * rx_num + j;
			sprintf(fbufp + temp_count * 7 + i * 2,"%5d,", td43xx_noise_delta[i * rx_num + j]);
			pr_err("%d	", td43xx_noise_delta[i * rx_num + j]);
		...
		struct device_attribute *attr, char *buf)
{
	...
	fbufp = (char *)kzalloc(8192, GFP_KERNEL);
	memset(filepath, 0, sizeof(filepath));
	sprintf(filepath, "%s%s", SYNA_INI_FILE_PATH, "rawdata.csv");
	if (NULL == pfile)
		pfile = filp_open(filepath, O_CREAT|O_RDWR, 0);
	if (IS_ERR(pfile)) {
		pr_err("error occured while opening file %s.",  filepath);
	...
	for (ii = 0; ii < tx_num; ii++) {
		for (jj = 0; jj < (rx_num - 1); jj++) {
			temp_count = ii * rx_num + jj;
			sprintf(fbufp + temp_count * 7 + ii * 2,"%5d,", *report_data_16);
			if((raw_limit_low >  *report_data_16) || (raw_limit_high < *report_data_16) )
				result = 1;
			report_data_16++;
		}
		sprintf(fbufp + (temp_count + 1)* 7 + ii * 2,"%5d,", *report_data_16);
		sprintf(fbufp + (temp_count + 2) * 7 + ii * 2,"\r\n");
		report_data_16++;
	}
	vfs_write(pfile, (char __user *)fbufp,  tx_num * rx_num * 7 + tx_num * 2, &pos);
	filp_close(pfile, NULL);
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	fbufp = (char *)kzalloc(8192, GFP_KERNEL);
	memset(filepath, 0, sizeof(filepath));
	sprintf(filepath, "%s%s", SYNA_INI_FILE_PATH, "openDataPart1.csv");
	if (NULL == pfile)
		pfile = filp_open(filepath, O_CREAT|O_RDWR, 0);
	if (IS_ERR(pfile)) {
		pr_err("error occured while opening file %s.",  filepath);
...
#endif
			temp_count = i * rx_num + j;
			sprintf(fbufp + temp_count * 7 + i * 2,"%5d,",p_rt92_delta_image[i * rx_num + j]);
			if (p_rt92_delta_image[i * rx_num + j] < ELEC_OPEN_TEST_LIMIT_ONE) {
				dev_err(f54->rmi4_data->pdev->dev.parent,
						"%s: fail at (tx%-2d, rx%-2d) = %-4d at phase 1 (limit = %d)\n",
						__func__, i, j, p_rt92_delta_image[i*rx_num + j], ELEC_OPEN_TEST_LIMIT_ONE);
	...
	sprintf(filepath, "%s%s", SYNA_INI_FILE_PATH, "openDataPart2.csv");
	if (NULL == pfile2){
		pfile2 = filp_open(filepath, O_CREAT|O_RDWR, 0);
	}
	if (IS_ERR(pfile2)) {
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	for (i = 0; i < button_count; i++) {
		...
		pr_err("%d	", g_abs_0d_open_data_output[i]);
