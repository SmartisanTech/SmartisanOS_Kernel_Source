...
#define MAX_F12_TOUCH_WIDTH 130
#else
#define MAX_F12_TOUCH_WIDTH 15
#endif
...
#ifdef CONFIG_LCD_COLOMBO
#define F54_ANALOG_CTRL188_ADDR 0x012e  /*calibrate register*/
#define F54_ANALOG_DATA31_ADDR 0x010e   /*status register*/
#endif
...
#ifdef CONFIG_LCD_COLOMBO
static ssize_t synaptics_rmi4_touch_calibrate_show(struct device *dev,
		struct device_attribute *attr, char *buf);

static ssize_t synaptics_rmi4_touch_calibrate_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count);
#endif
...
static struct device_attribute attrs[] = {
	...
	__ATTR(flashprog, 0444,
			...
			synaptics_rmi4_store_error),
	__ATTR(0dbutton, 0444,
			synaptics_rmi4_0dbutton_show,
			synaptics_rmi4_0dbutton_store),
	__ATTR(suspend, 0444,
			synaptics_rmi4_show_error,
			synaptics_rmi4_suspend_store),
	__ATTR(wake_gesture, 0444,
			synaptics_rmi4_wake_gesture_show,
			synaptics_rmi4_wake_gesture_store),
#ifdef CONFIG_LCD_COLOMBO
	__ATTR(touch_calibrate, S_IWUSR | S_IRUSR,
			synaptics_rmi4_touch_calibrate_show,
			synaptics_rmi4_touch_calibrate_store),
#endif
};
		...
		struct device_attribute *attr, char *buf)
{
	...
	retval = synaptics_rmi4_reg_read(rmi4_data, F54_ANALOG_DATA31_ADDR, &command, sizeof(command));
	if(retval < 0)
		return retval;
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	retval = synaptics_rmi4_reg_write(rmi4_data, F54_ANALOG_CTRL188_ADDR, &command, sizeof(command));
	if(retval < 0)
		return retval;
...
flash_prog_mode:
	...
	f01_query = (unsigned char *)kmalloc(F01_STD_QUERY_LEN, GFP_KERNEL);
	if (!f01_query) {
		dev_err(rmi4_data->pdev->dev.parent,
...
static void synaptics_resume_delayed_work(struct work_struct *work)
{
	struct delayed_work *delayed_work = to_delayed_work(work);
	struct synaptics_rmi4_exp_fhandler *exp_fhandler;
	struct synaptics_rmi4_data *rmi4_data = container_of(delayed_work, struct synaptics_rmi4_data,
						resume_delayed_work);
...
static int synaptics_rmi4_probe(struct platform_device *pdev)
{
	...
	INIT_DELAYED_WORK(&rmi4_data->resume_delayed_work, synaptics_resume_delayed_work);
...
static int synaptics_rmi4_remove(struct platform_device *pdev)
{
	...
	cancel_delayed_work(&rmi4_data->resume_delayed_work);
		...
		unsigned long event, void *data)
{
	...
	case LCD_EVENT_ON_END:
		if (sleep_flag == false)
		{
			printk("synaptics Touch has wakeup\n");
			rmi4_data->fb_ready = true;
			return 0;
		}
		sleep_flag = false;
		synaptics_rmi4_resume(&rmi4_data->pdev->dev);
		printk("synaptics Touch ON\n");
		rmi4_data->fb_ready = true;
		break;
	case LCD_EVENT_OFF_START:
		if (sleep_flag == true)
		{
			printk("synaptics Touch has sleep\n");
			rmi4_data->fb_ready = false;
			return 0;
		}
		sleep_flag = true;
		synaptics_rmi4_suspend(&rmi4_data->pdev->dev);
		printk("synaptics Touch OFF\n");
		rmi4_data->fb_ready = false;
		break;
...
static int synaptics_rmi4_resume(struct device *dev)
{
	...
	schedule_delayed_work(&rmi4_data->resume_delayed_work, msecs_to_jiffies(200));
	//synaptics_rmi4_sleep_enable(rmi4_data, false);
	//synaptics_rmi4_irq_enable(rmi4_data, true, false);
