		...
		struct device_attribute *attr, char *buf)
{
	...
	rmi4_data->firmware_id = (unsigned int)build_id[0] +
		...
		bool enable)
{
	...
	if (enable)
		reporting_control[rmi4_data->set_wakeup_gesture] = F12_WAKEUP_GESTURE_MODE;  //--- joey
	else
		reporting_control[rmi4_data->set_wakeup_gesture] = F12_CONTINUOUS_MODE;    //--- joey
...
static void gpio_keys_gpio_report_event(struct input_dev *input, unsigned int code, int state)
{
	if (state < 0) {
	...
	input_sync(input);
	pr_info("synap Button:EV_KEY (keycode = %d  value = %d)\n", code, !!state);
}

static unsigned char key_status;
static unsigned int key_code;
		...
		struct synaptics_rmi4_fn *fhandler)
{
	...
	unsigned short data_addr;
	unsigned char key_addr = 0x06;
	unsigned char key_addr_status[17];
	int x;
	int y;
	int wx;
	int wy;
...
//			rmi4_data->suspend = false;
	...
	for (finger = 0; finger < fingers_to_process; finger++) {
		...
		retval = synaptics_rmi4_reg_read(rmi4_data, key_addr, key_addr_status, sizeof(key_addr_status));
		if (key_addr_status[0] == 0x11) {
			key_code = KEY_INSERT;
			key_status = 2;
			dev_dbg(rmi4_data->pdev->dev.parent, "detect smart finger switch\n");
		} else if (key_status == 2 && key_addr_status[0] == 0) {
			key_code = KEY_INSERT;
			key_status = 0;
		} else if (key_addr_status[0] == 0x10 || key_addr_status[8] == 0x10 || key_addr_status[16] == 0x10) {
			if (key_addr_status[0] == 0x10)
				key_code = KEY_BACK;
			if (key_addr_status[8] == 0x10)
				key_code = KEY_HOME;
			if (key_addr_status[16] == 0x10)
		...
		if (key_code != 0) {
			gpio_keys_gpio_report_event(rmi4_data->input_dev, key_code, !!key_status);
			/* set key_code to 0 when key up */
			if (key_status == 0)
				key_code = 0;
			mutex_unlock(&(rmi4_data->rmi4_report_mutex));
			return 0;
		...
		case F12_GLOVED_FINGER_STATUS:
			...
			if (rmi4_data->wedge_sensor) {
				...
				if ((max_delta / min_delta >= 2))
					input_report_abs(rmi4_data->input_dev,
							ABS_MT_TOUCH_MAJOR, 9); //touch size: 9 / 27 = 0.33
				else {
					input_report_abs(rmi4_data->input_dev,
							ABS_MT_TOUCH_MAJOR,
							max_delta);
...
static int synaptics_rmi4_set_input_dev(struct synaptics_rmi4_data *rmi4_data)
{
	...
	set_bit(KEY_INSERT, rmi4_data->input_dev->keybit);
            ...
            unsigned long event, void *data)
{
		...
		if (event == LCD_EVENT_OFF) {
			synaptics_rmi4_suspend(&rmi4_data->pdev->dev);
			rmi4_data->fb_ready = false;
		} else if (event == LCD_EVENT_ON) {
