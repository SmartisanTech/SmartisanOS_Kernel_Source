...
static char *g_image_name[] = {
	"synaptics/startup_fw_tianma_update.img",
	"synaptics/startup_fw_txd_update.img",
	"synaptics/startup_fw_boe_update.img",
};

static int module_id = TIANMA;
static unsigned short glove_addr;
static unsigned short charge_addr;
static unsigned short grip_enable_addr;
static unsigned short flim_enable_addr;
static unsigned short force_update_addr;
//extern int lcd_id;
...
static int fwu_start_reflash(void)
{
	...
	if (fwu->image == NULL && g_image_name[module_id] != NULL) {
		retval = secure_memcpy(fwu->image_name, MAX_IMAGE_NAME_LEN,
				g_image_name[module_id], strlen(g_image_name[module_id]) + 1,
				strlen(g_image_name[module_id]) + 1);
		if (retval < 0) {
			dev_err(rmi4_data->pdev->dev.parent,
		...
		struct device_attribute *attr, char *buf)
{
	...
	config_value = fwu->config_id[3];

	retval =  snprintf(buf, PAGE_SIZE, "%x\n", config_value);
...
static void fw_name_init(struct synaptics_rmi4_data *rmi4_data)
{
	module_id = TIANMA;
	glove_addr = 0x1b;
	charge_addr = 0x0d;
	grip_enable_addr = 0x20;
	flim_enable_addr = 0x416;
}

void synaptics_rmi4_set_glove_mode(int mode)
{
	unsigned char glove_data = !!mode ? 0x25 : 0x05;
	int retval = 0;

	if (fwu == NULL)
		return;
	retval = synaptics_rmi4_reg_write(fwu->rmi4_data, glove_addr, &glove_data, sizeof(unsigned char));
	if (retval < 0) {
		dev_err(fwu->rmi4_data->pdev->dev.parent,
				"%s: %s glove mode failed\n",
				__func__, mode ? "enable" : "disable");
	}
	dev_info(fwu->rmi4_data->pdev->dev.parent,
			"%s: %s glove mode success\n",
			__func__, mode ? "enable" : "disable");
}

void synaptics_rmi4_grip_swtich(int mode)
{
	unsigned char grip_data;
	int retval = 0;
	...
	if(mode == 0){
		grip_data &= ~(0x03);
		retval = synaptics_rmi4_reg_write(fwu->rmi4_data, grip_enable_addr, &grip_data, sizeof(unsigned char));
	}else{
		grip_data |= 0x03;
		retval = synaptics_rmi4_reg_write(fwu->rmi4_data, grip_enable_addr, &grip_data, sizeof(unsigned char));
	}
...
void synaptics_rmi4_charger_switch(int mode)
{
	int retval = 0;
	unsigned char charge_status = !!mode? 0x20: 0x00;

	if (fwu == NULL || (syna_starting == 1) || (syna_suspend == 1) || (charge_addr == 0x00))
		return;

	if (fwu->rmi4_data->stay_awake){
		return;
	}

	retval = synaptics_rmi4_reg_write(fwu->rmi4_data, charge_addr, &charge_status, sizeof(unsigned char));
	if (retval < 0) {
		dev_dbg(fwu->rmi4_data->pdev->dev.parent,
				"%s: %s set charge status failed\n",
				__func__, mode ? "enable" : "disable");
	} else {
		dev_dbg(fwu->rmi4_data->pdev->dev.parent,
				"%s: %s set charge status success\n",
				__func__, mode ? "enable" : "disable");
	}
}

void synaptics_rmi4_phone_film_switch(int mode)
{
	int retval = 0;
	unsigned char charge_status = !!mode? 0x01: 0x00;
	unsigned char force_update_data = 0x04;
	force_update_addr = 0x141;

	if (fwu == NULL || (syna_starting == 1) || (syna_suspend == 1))
		return;

	if (fwu->rmi4_data->stay_awake){
		return;
	}
	retval = synaptics_rmi4_reg_write(fwu->rmi4_data, flim_enable_addr, &charge_status, sizeof(unsigned char));
	if (retval < 0) {
		dev_err(fwu->rmi4_data->pdev->dev.parent,
				"%s: %s set charge status failed\n",
				__func__, mode ? "enable" : "disable");
	...
	retval = synaptics_rmi4_reg_write(fwu->rmi4_data,
		force_update_addr,
		&force_update_data,
		sizeof(unsigned char));
	if (retval < 0) {
		dev_err(fwu->rmi4_data->pdev->dev.parent,
				"%s: %s force update status failed\n",
				__func__, mode ? "enable" : "disable");
	} else {
		dev_dbg(fwu->rmi4_data->pdev->dev.parent,
				"%s: %s force update status success\n",
				__func__, mode ? "enable" : "disable");
	}
}
