...
#define FW_IMAGE_NAME "synaptics/startup_fw_update_colombo.img"
#else
...
static struct device_attribute attrs[] = {
	__ATTR(dorecovery, 0444,
			synaptics_rmi4_show_error,
			fwu_sysfs_do_recovery_store),
	__ATTR(doreflash, 0644,
			synaptics_rmi4_show_error,
			fwu_sysfs_do_reflash_store),
	__ATTR(writeconfig, 0444,
			synaptics_rmi4_show_error,
			fwu_sysfs_write_config_store),
	__ATTR(readconfig, 0444,
			synaptics_rmi4_show_error,
			fwu_sysfs_read_config_store),
	__ATTR(configarea, 0444,
			synaptics_rmi4_show_error,
			fwu_sysfs_config_area_store),
	__ATTR(imagename, 0444,
			synaptics_rmi4_show_error,
			fwu_sysfs_image_name_store),
	__ATTR(imagesize, 0444,
			synaptics_rmi4_show_error,
...
static int fwu_start_reflash(void)
{
	...
	dev_err(rmi4_data->pdev->dev.parent, "%s:*****fwu->bl_version is %d, fwu->img.bl_version is %d\n", __FUNCTION__, fwu->bl_version,fwu->img.bl_version);
		...
		char *buf, loff_t pos, size_t count)
{
	...
	if (count < fwu->config_size) {
		dev_err(rmi4_data->pdev->dev.parent,
				"%s: Not enough space ( bytes) in buffer\n",
				__func__);
		return -EINVAL;
	}
