...
//static int old_charging_status=0;
extern bool synaptics_probe_status;
...
int synaptics_reg_write(unsigned short addr,unsigned char data,unsigned int len)
{
	int retval;
	if(fwu == NULL ||synaptics_probe_status==true)
		 return -1;
	if(fwu->rmi4_data->suspend == true||fwu->rmi4_data->stay_awake)
		 return -1;
	retval  = synaptics_rmi4_reg_write(fwu->rmi4_data,addr,&data,len);
	if (retval < 0)
		return retval;
	return 0;
}
...
int  synaptics_get_device_config_id(unsigned char *str)
{
	int retval;
	int i;
	unsigned char config_id_size;
	...
	if (retval < 0)
		return retval;
         for(i=0;i<4;i++)
	{
	    str[i] =fwu->config_id[i];
	}
	return 0;
}
int  synaptics_adjust_charging_status(bool status)
{
	int retval;
	unsigned char charge_data;
	unsigned short data_addr=0x000d;
	if(fwu == NULL ||synaptics_probe_status==true)
		return -1;
	if(fwu->rmi4_data->suspend == true||fwu->rmi4_data->stay_awake)
		 return -1;
	if(status)
	{
			retval = synaptics_rmi4_reg_read(fwu->rmi4_data,data_addr,&charge_data,1);
	                  if(retval>=0)
			{
			      charge_data=charge_data|(0x01<<5);
			     printk("%s----charging-----retval=%d----charge_data=0x%02x----\n",__func__,retval,charge_data);
			      synaptics_reg_write(data_addr,charge_data,1);
			 }
	 }
	else //discharging
	{
...
void synaptics_adjust_grip_swtich(bool mode)
{
	...
	int retval = 0;
	unsigned short grip_enable_addr=0x001f;
	if(fwu == NULL ||synaptics_probe_status==true)
		return ;
	if(fwu->rmi4_data->suspend == true||fwu->rmi4_data->stay_awake)
		 return ;
...
static int fwu_start_reflash(void)
{
	...
	pr_err("%s: Start of reflash process\n", __func__);
...
static int fwu_start_recovery(void)
{
	...
	pr_err("%s: Start of recovery process\n", __func__);
