...
//#0249842 madan@smartisan.com add to adjust tp size value begin.
#define TP_SIZE_VALUE_DENOMINATOR			58
//#0249842 madan@smartisan.com add to adjust tp size value end.
...
static int fts_reset_suspend(struct fts_ts_data *data)
{
    int ret = -1;
    ...
    if (gpio_is_valid(data->pdata->reset_gpio)) {
        ...
        if (ret) {
            FTS_ERROR("[GPIO]set_direction for reset gpio output low failed");
        }
    }
...
static int fts_power_ctrl(struct fts_ts_data *data, int enable)
{
    int ret = -1;

    FTS_FUNC_ENTER();

    if (gpio_is_valid(data->pdata->pwr_gpio)) {
        if (enable) {
            ret = gpio_direction_output(data->pdata->pwr_gpio, 1);
            if (ret) {
                FTS_ERROR("[GPIO]set_direction for power gpio output high failed");
            }
        } else {
            ret = gpio_direction_output(data->pdata->pwr_gpio, 0);
            if (ret) {
                FTS_ERROR("[GPIO]set_direction for power gpio output low failed");
            }
        }
    }
...
#if FTS_X_Y_AREA_SUPPORT
#define    MAX_X_Y_AREA      ((15 << 16) + 30 + 1)
static int x_y_area[FTS_MAX_ID] = {0};
#endif
...
#if FTS_X_Y_AREA_SUPPORT
            input_report_abs(data->input_dev, ABS_MT_TOUCH_MINOR, x_y_area[i]);
#endif

            touchs |= BIT(events[i].id);
            data->touchs |= BIT(events[i].id);

            LOGD(data->input_dev->dev.parent,"[B]P%d(%d, %d)[p:%d,tm:%d] DOWN!", events[i].id, events[i].x,
                      events[i].y, events[i].p, events[i].area);
            //FTS_DEBUG("[B]P%d(%d, %d)[x_y_area:%x,tm:%d] DOWN!", events[i].id, events[i].x,
                      //events[i].y, x_y_area[i], events[i].area);
        } else {
            uppoint++;
...
//#0249842 madan@smartisan.com to adjust size, change size register begin.
int current_fw_version = 0;
//#0249842 madan@smartisan.com to adjust size, change size register end.
...
#if FTS_X_Y_AREA_SUPPORT
    /* x_y_area 4-7 bit is x_area and 0-3 bit is y_area.*/
    for (i = 0; i < data->point_num ; i++) {
        base = FTS_ONE_TCH_LEN * i;
        x_y_area[i] = buf[FTS_TOUCH_PRE_POS + base];        //events[i].p
    }
#endif
...
#if FTS_X_Y_AREA_SUPPORT
    input_set_abs_params(input_dev, ABS_MT_TOUCH_MINOR, 0, MAX_X_Y_AREA, 0, 0);
#endif
    input_set_abs_params(input_dev, ABS_MT_POSITION_X, pdata->x_min, pdata->x_max, 0, 0);
    input_set_abs_params(input_dev, ABS_MT_POSITION_Y, pdata->y_min, pdata->y_max, 0, 0);
//#0249842 madan@smartisan.com add to adjust tp size value begin.
    input_set_abs_params(input_dev, ABS_MT_TOUCH_MAJOR, 0, TP_SIZE_VALUE_DENOMINATOR, 0, 0);
//#0249842 madan@smartisan.com add to adjust tp size value end.
#if FTS_REPORT_PRESSURE_EN
...
static int fts_gpio_configure(struct fts_ts_data *data)
{
    ...
    /* request power gpio */
    if (gpio_is_valid(data->pdata->pwr_gpio)) {
        ret = gpio_request(data->pdata->pwr_gpio, "fts_power_gpio");
        if (ret) {
            FTS_ERROR("[GPIO]pwr gpio request failed");
            goto err_irq_gpio_dir;
        }
...
static int fts_parse_dt(struct device *dev, struct fts_ts_platform_data *pdata)
{
    ...
    pdata->pwr_gpio = of_get_named_gpio_flags(np, "focaltech,pwr-gpio", 0, &pdata->pwr_gpio_flags);
    if (pdata->pwr_gpio < 0)
    ...
    FTS_INFO("max touch number:%d, irq gpio:%d, reset gpio:%d, pwr_gpio:%d",
             pdata->max_touch_number, pdata->irq_gpio, pdata->reset_gpio, pdata->pwr_gpio);
...
//#0244395 bsp madan@smartisan.com add to fix tp suspend when unplug the dp line begin.
    if (drm_data != NULL && drm_data->data != NULL && touch_needed) {
        ...
        if (*blank == MSM_DRM_BLANK_POWERDOWN) {
            ...
            flush_workqueue(fts_data->resume_workqueue);
            fts_ts_suspend(&fts_data->client->dev);
        } else if (*blank == MSM_DRM_BLANK_UNBLANK) {
            queue_work(fts_data->resume_workqueue,
...
static int tp_state2_value = -1;
#define FOCAL_IC 0
#define GOODIX_IC 1
int if_use_focal_ic(void)
{
    if(FOCAL_IC == tp_state2_value)
        return true;
    else if(GOODIX_IC == tp_state2_value)
        return false;
    return -1;
}

static int get_tp_ic_info(struct device *dev)
{
    s32 ret = 0;
    struct device_node *np;
    int tp_state2_gpio = -1;

    if (!dev)
        return -ENODEV;
    np = dev->of_node;

    tp_state2_gpio = of_get_named_gpio(np, "tp,state2-gpio", 0);

    if (!gpio_is_valid(tp_state2_gpio)) {
        FTS_ERROR("Invalid GPIO, state2-gpio:%d", tp_state2_gpio);
        return -EINVAL;
    }

    ret = gpio_request(tp_state2_gpio, "TP_STATE2_GPIO");
    if (ret < 0) {
        FTS_ERROR("Failed to request GPIO:%d, ERRNO:%d", tp_state2_gpio, ret);
        return -ENODEV;
    }
    gpio_direction_input(tp_state2_gpio);

    tp_state2_value = gpio_get_value(tp_state2_gpio);

    FTS_INFO("tp state2:%d.", tp_state2_value);

    gpio_free(tp_state2_gpio);
...
static int fts_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    if (0 == ret) {
        ...
        if (FOCAL_IC != get_tp_ic_info(&client->dev)) {
            FTS_INFO("not focal ic, return");
            return -ENODEV;
        }
    } else {
        FTS_ERROR("pinctrl init error");
    }
#endif
...
#if defined(CONFIG_FB)
    ts_data->resume_workqueue =
      create_singlethread_workqueue("fts_resume");
...
static int fts_ts_remove(struct i2c_client *client)
{
    ...
    flush_workqueue(ts_data->resume_workqueue);
    destroy_workqueue(ts_data->resume_workqueue);
...
static int fts_ts_suspend(struct device *dev)
{
    ...
    fts_irq_disable();
    ret = fts_reset_suspend(ts_data);
    if (ret < 0) {
        FTS_ERROR("reset output low fail, ret=%d", ret);
    }
    ret = fts_power_ctrl(ts_data, DISABLE);
    if (ret < 0) {
