...
static int fts_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if((!((gpio_id2 == 1) && (gpio_id1 == 0))) &&  (!((gpio_id2 == 0) && (gpio_id1 == 0)))) {
		pr_err("%s:the other lcd will be selected\n", __FUNCTION__);
		return -EINVAL;
	}
	else {
		if((gpio_id2 == 1) && (gpio_id1 == 0)) {
			pr_info("%s: the lcd is from tianma\n", __FUNCTION__);
			panel_vendor = 1;
		}
		else if((gpio_id2 == 0) && (gpio_id1 == 0)) {
			pr_info("%s: the lcd is from boyi\n", __FUNCTION__);
			panel_vendor = 0;
		}
		else {
			pr_info("%s: the lcd is not from tianma and boyi\n", __FUNCTION__);
		}
	}
	...
	wake_lock_init(&data->wlock, WAKE_LOCK_SUSPEND, "ft8716_wake");
