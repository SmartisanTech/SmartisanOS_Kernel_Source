    ...
    if (readlen > 0) {
        if (writelen > 0) {
            ...
            for (i = 0; i < I2C_RETRY_NUMBER; i++) {
                ...
                if (ret < 0) {
                    //FTS_DEBUG("[IIC]: i2c_transfer(write) error, ret=%d!!", ret);
                } else
                    break;
            }
        } else {
            ...
            for (i = 0; i < I2C_RETRY_NUMBER; i++) {
                ...
                if (ret < 0) {
                    //FTS_DEBUG("[IIC]: i2c_transfer(read) error, ret=%d!!", ret);
                } else
                    break;
            }
        }
...
int fts_i2c_write(struct i2c_client *client, char *writebuf, int writelen)
{
    ...
    if (writelen > 0) {
        ...
        for (i = 0; i < I2C_RETRY_NUMBER; i++) {
            ret = i2c_transfer(client->adapter, msgs, 1);
            if (ret < 0) {
                FTS_DEBUG("%s: i2c_transfer(write) error, ret=%d", __func__, ret);
            } else
                break;
        }
    }
