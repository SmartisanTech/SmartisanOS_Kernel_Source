...
int fts_write_reg_gesture_support(bool fod_en, bool click_en)
{
	int i = 0;
	u8 state = 0xFF;
	...
	for (i = 0; i < 5; i++) {
		if (fod_en) {
			if (click_en) {
				fts_write_reg(FTS_REG_GESTURE_SUPPORT_ADDRESS, 0x03);
				msleep(1);
				fts_read_reg(FTS_REG_GESTURE_SUPPORT_ADDRESS, &state);
				if (0x03 == state) {
					FTS_INFO("FOD enable, CLICK enable!");
					break;
				}
			} else {
				fts_write_reg(FTS_REG_GESTURE_SUPPORT_ADDRESS, 0x02);
				msleep(1);
				fts_read_reg(FTS_REG_GESTURE_SUPPORT_ADDRESS, &state);
				if (0x02 == state) {
					FTS_INFO("FOD enable, CLICK disable!");
					break;
				}
			}
		} else {
			if (click_en) {
				fts_write_reg(FTS_REG_GESTURE_SUPPORT_ADDRESS, 0x01);
				msleep(1);
				fts_read_reg(FTS_REG_GESTURE_SUPPORT_ADDRESS, &state);
				if (0x01 == state) {
					FTS_INFO("FOD disable, CLICK enable!");
					break;
				}
			} else {
				fts_write_reg(FTS_REG_GESTURE_SUPPORT_ADDRESS, DISABLE);
				msleep(1);
				fts_read_reg(FTS_REG_GESTURE_SUPPORT_ADDRESS, &state);
				if (DISABLE == state) {
					FTS_INFO("FOD disable, CLICK disable!");
					break;
				}
			}
		}
	...
	if (i >= 5) {
		FTS_ERROR("Write gesture reg(0xCF) fail");
		return -EIO;
	}
...
int fts_write_reg_gesture_en(bool enable)
{
	int i = 0;
	u8 state = 0xFF;
	...
	if (i >= 5) {
		FTS_ERROR("Write gesture reg(0xD0) fail");
		return -EIO;
	}
...
int fts_gesture_suspend(struct fts_ts_data *ts_data)
{
	...
	ret = fts_write_reg_gesture_en(true);
	if (ret) {
		fts_gesture_data.active = DISABLE;
		return -EIO;
	}
#ifdef TOUCHSCREEN_FTS_FOD_EN
	fts_write_reg_gesture_support(true, true);
#endif
...
int fts_gesture_resume(struct fts_ts_data *ts_data)
{
	...
	fts_gesture_data.active = DISABLE;
	fts_write_reg_gesture_en(false);
#ifdef TOUCHSCREEN_FTS_FOD_EN
	fts_write_reg_gesture_support(true, false);
#endif
