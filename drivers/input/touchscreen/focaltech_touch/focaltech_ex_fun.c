...
static void esd_process(u8 *writebuf, int buflen, bool flag)
{
    if (flag)
    {
        ...
        else if ( (writebuf[1] == 0x00) && (writebuf[2] == 0x40) && (buflen == 0x03) ) /* factory mode bit 4 5 6 */
        {
            FTS_DEBUG("%s: Entry factory mode(%x %x %x)", __func__, writebuf[0], writebuf[1], writebuf[2]);
            fts_esdcheck_switch(DISABLE);
        }
        else if ( (writebuf[1] == 0x00) && (writebuf[2] == 0x00) && (buflen == 0x03) ) /* normal mode bit 4 5 6 */
        {
...
static ssize_t fts_debug_write(struct file *filp, const char __user *buff, size_t count, loff_t *ppos)
{
    ...
    {
        FTS_ERROR("%s:copy from user error", __func__);
        return -EFAULT;
    }
#if FTS_ESDCHECK_EN
    ...
    switch (proc_operate_mode)
    {
        case PROC_UPGRADE:
        {
            ...
            if (ret < 0)
            {
                FTS_ERROR("%s:upgrade failed.", __func__);
            }
        }
        break;

        case PROC_SET_TEST_FLAG:
            FTS_DEBUG("%s: PROC_SET_TEST_FLAG = %x", __func__, writebuf[1]);
#if FTS_ESDCHECK_EN
            if (writebuf[1] == 0)
            {
                fts_esdcheck_switch(DISABLE);
        ...
        case PROC_READ_REGISTER:
            ...
            if (ret < 0)
            {
                FTS_ERROR("%s:write iic error", __func__);
            }
            break;
...
static ssize_t fts_debug_read(struct file *filp, char __user *buff, size_t count, loff_t *ppos)
{
    ...
    switch (proc_operate_mode)
    {
        case PROC_UPGRADE:
            ...
            if (ret < 0)
                ...
                num_read_chars = sprintf(buf, "current fw version:0x%02x", regvalue);
            break;
        case PROC_READ_REGISTER:
            readlen = 1;
            ...
            if (ret < 0)
            {
                ...
                fts_esdcheck_proc_busy(0);
#endif
                FTS_ERROR("%s:read iic error", __func__);
                return ret;
            }
            num_read_chars = 1;
            break;
    ...
    {
        FTS_ERROR("%s:copy to user error", __func__);
        return -EFAULT;
    }
...
static ssize_t fts_get_point_stable(struct device *dev, struct device_attribute *attr, char *buf)
{
	ssize_t num_read_chars = 0;
	u8 fwver = 0;

	mutex_lock(&fts_input_dev->mutex);
	if (fts_i2c_read_reg(fts_i2c_client, FTS_REG_POINT_STABLE_VER, &fwver) < 0)
    {
...
static ssize_t fts_set_point_stable(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
    /* place holder for future use */
	u32 regvalue;
	mutex_lock(&fts_input_dev->mutex);
	if (sscanf(buf, "%d", &regvalue) != 1) {
...
static int fts_hex_to_str(char *hex, int iHexLen, char *ch, int *iChLen)
{
    ...
    FTS_DEBUG("iHexLen: %d in function:%s", iHexLen, __func__);
    ...
    ch[iCharLen] = '\0';
    *iChLen = iCharLen;
    FTS_DEBUG("iCharLen: %d, iChLen: %d in function:%s", iCharLen, *iChLen, __func__);
    return 0;
}
...
static ssize_t fts_fwupdate_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
    ...
    if (i_ret == 0)
    {
        ...
        FTS_DEBUG("%s [FTS] upgrade to new version 0x%x", __func__, uc_host_fm_ver);
    }
    else
    {
        FTS_ERROR("%s ERROR:[FTS] upgrade failed ret=%d.", __func__, i_ret);
    }
...
static ssize_t fts_module_config_show(struct device *dev, struct device_attribute *attr, char *buf)
{
    ...
    /*count += sprintf(buf+count, "FTS_MT_PROTOCOL_B_EN: \t\t%s\n", FTS_MT_PROTOCOL_B_EN ? "ON" : "OFF");*/
...
static DEVICE_ATTR(point_stable_control, S_IRUGO|S_IWUSR, fts_get_point_stable, fts_set_point_stable);
...
static struct attribute *fts_attributes[] =
{
	...
	&dev_attr_point_stable_control.attr,
...
int fts_create_sysfs(struct i2c_client * client)
{
    ...
    if (0 != err)
    {
        FTS_ERROR("%s() - ERROR: sysfs_create_group() failed.", __func__);
        sysfs_remove_group(&client->dev.kobj, &fts_attribute_group);
        return -EIO;
    }
    else
    {
        FTS_INFO("sysfs_create_group() succeeded.");
    }
    return err;
}
/************************************************************************
