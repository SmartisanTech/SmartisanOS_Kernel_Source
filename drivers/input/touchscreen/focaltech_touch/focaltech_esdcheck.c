...
static bool get_chip_id( void )
{
    ...
    for (i = 0; i < 3; i++)
    {
        ...
        if ( err < 0 )
        {
            FTS_DEBUG("[%s] Read Reg 0xA3 failed ret = %d", __func__, err);
            fts_esdcheck_data.i2c_nack_cnt++;
        }
        else
        {
            if ( (reg_value == fts_updateinfo_curr.CHIP_ID) || (reg_value == 0xEF)|| (reg_value == 0x87) ) /* Upgrade sometimes can't detect */
            {
                break;
            }
            else
    ...
    if (i >= 3)
    {
        FTS_DEBUG("[%s] Read Chip id 3 times failed, need execute TP reset.", __func__);
        return 1;
    }
...
static bool get_flow_cnt( void )
{
    ...
    if (err < 0)
    {
        FTS_DEBUG("[%s] Read Reg 0x91 failed ret = %d", __func__, err);
        fts_esdcheck_data.i2c_nack_cnt++;
    }
    else
    {
    ...
    {
        FTS_DEBUG("[%s] Flow Work Cnt(reg0x91) keep a value for 5 times, need execute TP reset.", __func__);
        return 1;
    }
...
static int esdcheck_algorithm(void)
{
    ...
    {
        FTS_DEBUG("[%s] In interrupt state, not check esd, return immediately.", __func__);
        return 0;
    }
    ...
    {
        FTS_DEBUG("[%s] In suspend, not check esd, return immediately.", __func__);
    ...
    {
        FTS_DEBUG("[%s] In apk or adb command mode, not check esd, return immediately.", __func__);
        return 0;
    }
    ...
    {
        FTS_DEBUG("[%s] In factory mode, not check esd, return immediately.", __func__);
        return 0;
    }
...
int fts_esdcheck_switch(bool enable)
{
    if (enable == 1)
    {
        ...
        {
            FTS_DEBUG("[%s] ESD check start", __func__);
    ...
    else
    {
        ...
        {
            FTS_DEBUG("[%s] ESD check stop", __func__);
            fts_esdcheck_data.active = 0;
...
int fts_esdcheck_init(void)
{
    ...
    {
        FTS_DEBUG("[%s] Failed to create esd work queue!", __func__);
    }
