...
u8 pb_file_ft3518[] = {
#include "../include/pramboot/FT5452_Pramboot_20170731.i"
};
#endif
...
#if FTS_UPGRADE_ALL_BIN
extern int tp_state2_value;
/************************************************************************
* Name: fts_ft5452_download
* Brief:
* Input:
* Output:
* Return: return 0 if success, otherwise return error code
***********************************************************************/
static int fts_ft5452_download(struct i2c_client *client, u8 *buf, u32 len)
{
    int ret = 0;
    u32 start_addr = 0;
    u8 cmd[4] = { 0 };
    ...
    /* enter into upgrade environment */
    FTS_INFO("enter into pramboot !");

    ret = fts_reset_proc(5);
    if (ret < 0) {
...
struct upgrade_func upgrade_func_ft5452 = {
    ...
    .appoff = 0x0000,
#if FTS_UPGRADE_ALL_BIN
    .pramboot = pb_file_ft3518,
    .pb_length = sizeof(pb_file_ft3518),
    .pramboot_supported = true,
    .hid_supported = false,
    .download = fts_ft5452_download,
#else
