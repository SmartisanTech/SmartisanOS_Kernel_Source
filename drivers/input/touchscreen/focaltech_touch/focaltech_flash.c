...
int fts_read_file(char *file_name, u8 **file_buf)
{
    ...
    *file_buf = (u8 *)kmalloc(file_len, GFP_KERNEL);
    if (NULL == *file_buf) {
...
//#0249842 madan@smartisan.com to adjust size, change size register begin.
		current_fw_version = fw_ver_in_tp;
//#0249842 madan@smartisan.com to adjust size, change size register end.

        FTS_INFO("fw version in tp:%x, host:%x", fw_ver_in_tp, fw_ver_in_host);
        //#0244350 bsp madan@smartisan.com add to modify fw upgrade logic begin.
        if (fw_ver_in_tp != fw_ver_in_host) {
            return true;
        }
        //#0244350 bsp madan@smartisan.com add to modify fw upgrade logic end.
    } else {
...
/************************************************************************
 * Name: fts_fwupg_download
 * Brief: fw upgrade main entry
 * Input:
 * Output:
 * Return: return 0 if success, otherwise return error code
 ***********************************************************************/
int fts_fwupg_download(struct i2c_client *client, struct fts_upgrade *upg)
{
    int ret = 0;
    int upgrade_count = 2;
    u8 ver = 0;

    FTS_INFO("fw auto download function");
    if ((NULL == upg) || (NULL == upg->func)) {
        FTS_ERROR("upg/upg->func is null");
//#0249842 madan@smartisan.com to adjust size, change size register begin.
        current_fw_version = 0x14;
//#0249842 madan@smartisan.com to adjust size, change size register end.
        return -EINVAL;
    }
    ...
    while(upgrade_count--){
        FTS_INFO("upgrade fw app(times:%d)", upgrade_count);
        if (upg->func->download) {
            ret = upg->func->download(client, upg->fw, upg->fw_length);
            if (ret < 0) {
                fts_fwupg_reset_in_boot(client);
            } else {
                fts_fwupg_get_ver_in_tp(client, &ver);
...
#if FTS_UPGRADE_ALL_BIN
    ret = fts_fwupg_download(client, upg);
#else
