...
int fts_enter_slide_mode(int mode );
...
int fts_enter_glove_mode(int mode)
{
    ...
    if(NULL == client) {
        FTS_ERROR("FTS driver not ready for glove mode.");
        return -1;
    }

    if (mode) {
        buf_value[0] = 0x01;
        g_fts_mode_flag.fts_glove_mode_flag = 1;
    } else {
        buf_value[0] = 0x00;
        g_fts_mode_flag.fts_glove_mode_flag = 0;
    }
...
#if FTS_SLIDE_EN
#define FTS_SLIDE_MODE_EN               0xC3

static ssize_t fts_touch_slide_show(struct device *dev, struct device_attribute *attr, char *buf)
{
    int count;
    u8 val;
    struct input_dev *input_dev = fts_data->input_dev;
    ...
    mutex_lock(&input_dev->mutex);
    fts_i2c_read_reg(client, FTS_SLIDE_MODE_EN, &val);
    count = snprintf(buf, PAGE_SIZE, "Slide Mode: %s\n", g_fts_mode_flag.fts_tp_slide_mode_flag ? "On" : "Off");
    count += snprintf(buf + count, PAGE_SIZE, "Slide Reg(0xC0) = %d\n", val);
    mutex_unlock(&input_dev->mutex);

    return count;
}

static ssize_t fts_touch_slide_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
    int ret;
    struct fts_ts_data *ts_data = fts_data;
    struct i2c_client *client;
    ...
    client = ts_data->client;
    if (FTS_SYSFS_ECHO_ON(buf)) {
        if (!g_fts_mode_flag.fts_tp_slide_mode_flag) {
            FTS_INFO("[Mode]enter slide mode");
            ret = fts_enter_slide_mode(true);
            if (ret >= 0) {
                g_fts_mode_flag.fts_tp_slide_mode_flag = true;
            }
        }
    } else if (FTS_SYSFS_ECHO_OFF(buf)) {
        if (g_fts_mode_flag.fts_tp_slide_mode_flag) {
            FTS_INFO("[Mode]exit slide mode");
            ret = fts_enter_slide_mode(false);
            if (ret >= 0) {
                g_fts_mode_flag.fts_tp_slide_mode_flag = false;
            }
        }
    }
    FTS_INFO("[Mode]slide mode status:  %d", g_fts_mode_flag.fts_tp_slide_mode_flag);
    return count;
}
...
int fts_enter_slide_mode(int mode)
{
    ...
    static u8 buf_addr[2] = { 0 };
    static u8 buf_value[2] = { 0 };
    buf_addr[0] = FTS_SLIDE_MODE_EN; /* slide mode control */

    if(NULL == client) {
        FTS_ERROR("FTS driver not ready for slide mode.");
        return -1;
    }

    if (mode) {
        buf_value[0] = 0x01;
        g_fts_mode_flag.fts_tp_slide_mode_flag = 1;
    } else {
        buf_value[0] = 0x00;
        g_fts_mode_flag.fts_tp_slide_mode_flag = 0;
    }

    ret = fts_i2c_write_reg( client, buf_addr[0], buf_value[0]);
    if (ret < 0) {
        FTS_ERROR("[Mode]fts_enter_slide_mode write value fail");
    }
...
static DEVICE_ATTR (fts_slide_mode,  S_IRUGO | S_IWUSR, fts_touch_slide_show, fts_touch_slide_store);
