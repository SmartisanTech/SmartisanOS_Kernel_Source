...
#include <linux/i2c.h>
#include <linux/i2c/colombo_atmel_mxt_ts.h>
...
extern struct semaphore firm_sem;
static bool sleep_flag = false;
static int touch_once = 0;
static atomic_t  stay_awake = ATOMIC_INIT(0);
static wait_queue_head_t commit_wait_q;
static bool fw_updated = false;
...
static void mxt_proc_t6_messages(struct mxt_data *data, u8 *msg)
{
	...
	if (status & MXT_STATUS_CAL) {
		dev_dbg(dev, "Calibration start!\n");
	}

	if (status & MXT_STATUS_SIGERR) {
		dev_dbg(dev, "HSYNC lost!\n");
...
static irqreturn_t mxt_read_messages_t44(struct mxt_data *data)
{
	...
	if (ret) {
		...
		irq_fail_count ++;
		if (irq_fail_count > 3)
			mxt_input_disable(data->input_dev);
		return IRQ_NONE;
	}
	irq_fail_count = 0;
	...
	while (byte_offset < config_mem_size) {
		...
		for(i = 0;i < 5;i++){
			ret = mxt_write_block(data->client,
				      config_start_offset + byte_offset + size / 5 * i,
				      size/5, config_mem + byte_offset + size / 5 * i);
			if (ret != 0) {
		...
		ret = mxt_write_block(data->client,
				      config_start_offset + byte_offset + size / 5 * 5,
				      size % 5, config_mem + byte_offset + size / 5 * 5);
		if (ret != 0) {
			dev_err(dev, "Config write error, ret=%d\n", ret);
			goto release_mem;
		}
...
retry_probe:
	...
	dev_info(&client->dev,
		"Family ID: %x Variant ID: %x Version: %x.%x.%02X "
		"Object Num: %d\n",
...
#else
	//schedule_delayed_work(&data->config_delayed_work, msecs_to_jiffies(10000));
#endif
...
static int mxt_load_fw(struct device *dev, const char *fn)
{
	...
	while (pos < len) {
...
#endif
		for(i = 0;i < 5;i++){
			ret = mxt_bootloader_write(data,buffer + pos + frame_size/5 * i, frame_size/5);
			if (ret) {
				data->state = FAILED;
				goto release_firmware;
			}
		...
		ret = mxt_bootloader_write(data,buffer + pos + frame_size/5 * 5, frame_size % 5);
		if (ret) {
			data->state = FAILED;
			goto release_firmware;
		}
	...
	size_t count, bool *upgraded)
{
	...
	if(temp_buf != NULL){
		temp_buf[0] = '\0';
		write_sign = temp_buf + 1;
	}else
		write_sign = NULL;
	
	if (sleep_flag == true)
	{
		printk("%s: TP is power off\n", __func__);
		return len;
	}
#if 1
	for (i = 0; i < pdata->config_array_size; i++) {
		if ((data->info.family_id == pdata->config_array[i].family_id) &&
			...
			(data->info.build == pdata->config_array[i].build))
		{
			printk("atmel firmware will not update!\n");
			break;
		}
	}

	if ((i < pdata->config_array_size) && write_sign == NULL) {
		/* firmware is same, check config */
		printk("atmel config update\n");
		mxt_disable_irq(data);
		error = mxt_check_reg_init(data);
		if (error == 2)
		{
			printk("atmel power off, can't update firmware\n");
			return len;
		}
		mxt_initialize(data);
		if (data->state == APPMODE) {
	...
	printk("atmel firmware will update \n");
	if (upgraded)
		*upgraded = false;
	...
	//mxt_download_config(data, fw_name);
	...
	if (error) {
		...
		error = mxt_check_reg_init(data);
		if (error) {
			printk("Failed to initialize config\n");
			//	return error;
		}
	}
					...
					const char *buf, size_t count)
{
	...
	down(&firm_sem);
	//stay_awake = true;
	atomic_set(&stay_awake, 1);
	while(re_try){
		ret_count = mxt_update_firmware(dev, attr, buf, count, NULL);
		if(ret_count >= 0)
			break;
		re_try--;
	}
	atomic_set(&stay_awake, 0);
	up(&firm_sem);
	//stay_awake = false;
	wake_up_all(&commit_wait_q);
	return ret_count;
}
...
static int mxt_suspend(struct device *dev)
{
	...
	if (data->pdata->cut_off_power) {
		...
		mxt_disable_irq(data);
		if ((atomic_read(&stay_awake) == 1))
		{
			printk("updating fw, do not sleep\n");
			mutex_unlock(&input_dev->mutex);
			return 0;
		}
			...
			unsigned long event, void *data)
{
	...
	case LCD_EVENT_ON_END:
		if (fw_updated == false)
		{
			dev_info(&mxt_data->client->dev, "update firmware\n");
			mxt_update_firmware(&mxt_data->client->dev, NULL,
					"atmel_firmware.fw", 17, 0);
		}
		if (sleep_flag == false)
		{
			dev_info(&mxt_data->client->dev, "has wakeup\n");
			return 0;
		}
		sleep_flag = false;
	...
	case LCD_EVENT_OFF_START:
		if (sleep_flag == true)
		{
			dev_info(&mxt_data->client->dev, "has sleep\n");
			return 0;
		}
		wait_event(commit_wait_q, !atomic_read(&stay_awake));
...
static void mxt_dump_value(struct device *dev, struct mxt_platform_data *pdata)
{
	...
	for (i = 0; i < /*pdata->config_array_size*/1; i++) {
...
static int mxt_parse_dt(struct device *dev, struct mxt_platform_data *pdata)
{
	...
	ret = of_property_read_u32(np, "atmel,irq-flags", &temp_val);
	if (ret) {
		...
		const struct i2c_device_id *id)
{
	...
	retval = gpio_get_value(ID_SUR_LCD);
	if((of_machine_is_compatible("qcom,surabaya"))){
		dev_err(&client->dev,
				"%s: we need another touch screen. \n",
				__func__);
		return -EIO;
	}else if(retval != 0){
		dev_err(&client->dev,
	...
	init_waitqueue_head(&commit_wait_q);
