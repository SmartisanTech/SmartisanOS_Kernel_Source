...
//			memcpy(&buffer[loc], &cmd_head.data[GTP_ADDR_LENGTH], len);
            		if(copy_to_user(&buffer[loc],&cmd_head.data[GTP_ADDR_LENGTH],len)){
                		GTP_ERROR("copy_to_user faile 1");
            		}
			data_len -= len;addr += len;
			loc += len;
...
//		memcpy(buffer, GTP_DRIVER_VERSION, tmp_len);
        	if(copy_to_user(buffer,GTP_DRIVER_VERSION,tmp_len)){
            		GTP_ERROR("copy_to_user fail 3");
        	}

        	if(copy_to_user(&buffer[tmp_len],temp_buf,1)){
            		GTP_ERROR("copy_to_user fail 4");
        	}
//		buffer[tmp_len] = 0;
		*ppos += tmp_len + 1;
		return (tmp_len + 1);
