...
static u8 gt1x_glove_config[GTP_CONFIG_ORG_LENGTH + GTP_CONFIG_EXT_LENGTH];
...
//	ptr += sprintf(ptr, "==== GT1X default config setting in driver====\n");
	len = sprintf(def_config, "==== GT1X default config setting in driver====\n");
	...
	*ppos += len;
	...
	for (i = 0; i < gt1x_cfg_length; i++) {
		ptr_config += sprintf(ptr_config, "0x%02X,", gt1x_config[i]);
	...
	ptr_config += sprintf(ptr_config, "==== GT1X config read from chip====\n");
	...
	for (i = 0; i < gt1x_cfg_length; i++) {
		ptr_config += sprintf(ptr_config, "0x%02X,", temp_data[i]);
		if (i % 10 == 9 && i !=  GTP_CONFIG_ORG_LENGTH)
			ptr_config += sprintf(ptr_config, "\n");
	...
	ptr_config += sprintf(ptr_config, "\n");
	/* Touch PID & VID */
	ptr_config += sprintf(ptr_config, "==== GT1X Version Info ====\n");

	gt1x_i2c_read(GTP_REG_VERSION, temp_data, 12);
	ptr_config += sprintf(ptr_config, "ProductID: GT%c%c%c%c\n", temp_data[0],
			temp_data[1], temp_data[2], temp_data[3]);
	ptr_config += sprintf(ptr_config, "PatchID: %02X%02X\n", temp_data[4], temp_data[5]);
	ptr_config += sprintf(ptr_config, "MaskID: %02X%02X\n", temp_data[7], temp_data[8]);
	ptr_config += sprintf(ptr_config, "SensorID: %02X\n", temp_data[10] & 0x0F);

	irq_desc = irq_to_desc(gt1x_i2c_client->irq);
	if (irq_desc) {
		ptr_config += sprintf(ptr_config, "IRQ: %d, irq_desc->disable-depth:%d\n",
...
#endif
	cfg_len = gt1x_find_tp_config(dev,"glove-config",gt1x_version.sensor_id,gt1x_glove_config);
...
void gt1x_enter_glove_mode(u8 glove_flag)
{
	u16 cfg_len = 0;
	s32 ret = 0;
	struct device *dev = &gt1x_i2c_client->dev;
	 if(glove_flag == 1){
		cfg_len = gt1x_find_tp_config(dev,"glove-config",gt1x_version.sensor_id,gt1x_glove_config);
		if(cfg_len < 0){
		 	GTP_ERROR("Failed to obtain glove config data");
			return ;
		}
		gt1x_glove_config[0] &=0x7F;
		cfg_len = GTP_CONFIG_ORG_LENGTH;
		ret = gt1x_send_cfg(gt1x_glove_config,cfg_len);
		if(ret < 0){
			return ;
		}
	 }else{
		cfg_len = gt1x_find_tp_config(dev,"default-config",gt1x_version.sensor_id,gt1x_config);
		if(cfg_len < 0){
		 	GTP_ERROR("Failed to obtain glove config data");
			return ;
		}
...
/*	start check current event */
	if ((touch_data[0] & 0x10) && key_value) {
		...
		if (key_value & 0xF0) {
			SET_BIT(cur_key_event, BIT_STYLUS_KEY);
		} else if (key_value & 0x0F) {
			SET_BIT(cur_key_event, BIT_TOUCH_KEY);
		}
#endif
	}
...
#ifdef CONFIG_GTP_HAVE_TOUCH_KEY
	if (CHK_BIT(cur_key_event, BIT_TOUCH_KEY) || CHK_BIT(pre_key_event, BIT_TOUCH_KEY)) {
		for (i = 0; i < GTP_MAX_KEY_NUM; i++) {
			input_report_key(dev, gt1x_touch_key_array[i],
					key_value & (0x01 << i));
		}
		if (CHK_BIT(cur_key_event, BIT_TOUCH_KEY)) {
			GTP_DEBUG("Key Down.");
			key_flag ++;
		} else {
			GTP_DEBUG("Key Up.");
	...
	if (CHK_BIT(cur_event, BIT_TOUCH)) {
		...
		for (i = 0; i < GTP_MAX_TOUCH; i++) {
			if (i == id) {
				...
				input_y = coor_data[3] | (coor_data[4] << 8);
				/* input_w = coor_data[5] | (coor_data[6] << 8); */
				if(coor_data[5] > coor_data[6]){
					input_w = coor_data[5];
				}else{
					input_w = coor_data[6];
				}
				input_w = input_w*4/10+29;

				input_x = GTP_WARP_X(gt1x_abs_x_max, input_x);
				input_y = GTP_WARP_Y(gt1x_abs_y_max, input_y);

				//GTP_INFO("(%d)(%d,%d)[%d]", id, input_x, input_y, input_w);
				LOGD(dev->dev.parent,"id = %d (x = %d y = %d) major = %d\n",id, input_x, input_y, input_w);
...
//		}
	...
	if (CHK_BIT(cur_event, BIT_TOUCH)
		...
		||CHK_BIT(cur_key_event, BIT_TOUCH_KEY)
		||CHK_BIT(pre_key_event, BIT_TOUCH_KEY)) {
		input_sync(dev);
	}
	...
	if (unlikely(!pre_key_event && !cur_key_event)) {
		GTP_DEBUG("Additional Int Pulse.");
	} else {
		pre_key_event = cur_key_event;
	}
...
static ssize_t gtp_tpinfo_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	int count = 0;
	count += snprintf(buf + count, PAGE_SIZE, "FW Ver:%x\n", gt1x_version.patch_id);
	return count;
}

static ssize_t gtp_tpinfo_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	return -EPERM;
}

static DEVICE_ATTR(gtp_fw_info, S_IRUGO | S_IWUSR, gtp_tpinfo_show, gtp_tpinfo_store);

/* add your attr in here*/
static struct attribute *gtp_attributes[] = {
    &dev_attr_gtp_fw_info.attr,
    NULL
};
...
int gtp_create_sysfs(struct i2c_client *client)
{
    ...
    ret = sysfs_create_group(&client->dev.kobj, &gtp_attribute_group);
    if (ret) {
        GTP_ERROR("[GTP EX]: sysfs_create_group() failed!!");
        sysfs_remove_group(&client->dev.kobj, &gtp_attribute_group);
        return -ENOMEM;
    } else {
        GTP_INFO("[GTP EX]: sysfs_create_group() succeeded!!");
    }

    return ret;
}
int gtp_remove_sysfs(struct i2c_client *client)
{
    sysfs_remove_group(&client->dev.kobj, &gtp_attribute_group);
    return 0;
}
...
#endif
	if (goodix_glove_mode)
		gt1x_enter_glove_mode(goodix_glove_mode);
