	...
	if (akm->state.power_on) {
		...
		if (ret) {
			...
			goto exit;
                ...
                schedule_delayed_work(&akm->resume_work,
                        (unsigned long)msecs_to_jiffies(100));
				...
				struct akm_compass_data *pdata)
{
	...
	int rc;
	u32 temp_val;
	if (of_property_read_bool(np, "akm09911,auto-report"))
		s_akm->auto_report = 1;
	else
		s_akm->auto_report = 0;
...
static void akm_resume_work(struct work_struct *work)
{

        struct akm_compass_data *akm;

        akm = container_of((struct delayed_work *)work,
                        struct akm_compass_data,  resume_work);

        if (AKECS_SetMode(akm, akm->state.mode)) {
                dev_err(&akm->i2c->dev, "Sensor state resume fail!\n");
                return;
        }
...
int akm_compass_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if (s_akm->irq) {
        ...
        INIT_DELAYED_WORK(&s_akm->resume_work, akm_resume_work);
