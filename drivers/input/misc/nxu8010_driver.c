...
#else
#define NXU8010DB(x, ...)       do{	}while(0)
#define NXU8010_FUNC_ENTER()    do{	}while(0)
#define NXU8010_FUNC_EXIT()     do{	}while(0)
#endif
...
#define NXU8010_VTG_MIN_UV          3000000
#define NXU8010_VTG_MAX_UV          3000000
...
reg_vdd_put:
    ...
    pr_err("[NXU8010] %s failed", __func__);
    return rc;
}
			...
			void __user *p)
{
    ...
    case NXU8010_IOCTL_INIT:
		...
		ret = nxu8010_init();
		if(ret){
			pr_err("NXU8010_IOCTL_INIT failed");
			return ret;
		}
		break;
...
static int nxu8010_release(struct inode *inode, struct file *file)
{
	int ret = 0;
	NXU8010_FUNC_ENTER();
	ret = nxu8010_power_down(nxu_data);
	if(ret){
		pr_err("%s failed", __func__);
		return ret;
	}
...
static struct of_device_id nxu_match_table[] =
{
    { .compatible = "nxu8010",},
    { },
};
...
struct i2c_driver nxu8010_i2c_driver = {
	...
	.driver = {
        .name = NXU8010_DRVNAME,
        .owner = THIS_MODULE,
        .of_match_table = nxu_match_table,
    },
