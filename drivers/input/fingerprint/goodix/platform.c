...
static const struct vreg_config const vreg_conf[4] = { {"goodix_vddio", 1800000UL, 1800000UL, 1500, },
    { "goodix_pwr", 3250000UL, 3250000UL, 6000, } };

static int gf_power_state = 1;
	...
	bool enable)
{
	...
	if (enable) {
		for (i = 0; i < 2; i++) {
			if (!strcmp(vreg_conf[i].name,name)) {
...
int gf_power_on(struct gf_dev* gf_dev)
{
    ...
    if (gf_power_state == 1) {
        rc = vreg_setup(gf_dev, "goodix_pwr", 1);
        if (rc != 0)
            pr_info("---- goodix power on avdd failed! ----\n");

        msleep(1);
        rc = vreg_setup(gf_dev, "goodix_vddio", 1);
        if (rc != 0)
            pr_info("---- goodix power on vddio failed! ----\n"); 

        msleep(10);
        gf_power_state = 1;
        pr_info("---- goodix power on ok ----\n");

        return rc;
    } else
        pr_info("---- goodix power on already ----\n");
...
/*
	...
	struct device *dev = &gf_dev->spi->dev;
	vreg_setup(gd_dev, "goodix_pwr", 0);
	pr_info("---- power off ----\n");
*/
	return rc;
}
