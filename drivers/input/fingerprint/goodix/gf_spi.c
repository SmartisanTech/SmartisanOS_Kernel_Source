...
static void gf_kernel_key_input(struct gf_dev *gf_dev, struct gf_key *gf_key)
{
    ...
    } else if (gf_key->key == GF_KEY_FINGERPRINT) {
        key_input = GF_KEY_INPUT_FINGERPRINT;
	} else if (gf_key->key == GF_KEY_FINGERPRINT_TEST) {
		key_input = GF_KEY_INPUT_FINGERPRINT_TEST;
	} else {
	...
	if (gf_key->key == GF_KEY_FINGERPRINT_TEST) {
		input_report_key(gf_dev->input, key_input, 1);
		input_sync(gf_dev->input);
        msleep(200);
		input_report_key(gf_dev->input, key_input, 0);
...
#ifdef CONFIG_FACTORY_SUPPORT
		smartisan_hwstate_set("goodixfp", "ok");
#endif
...
error_hw:
	gf_dev->device_available = 0;
#ifdef CONFIG_FACTORY_SUPPORT
		smartisan_hwstate_set("goodixfp", "fail");
#endif
