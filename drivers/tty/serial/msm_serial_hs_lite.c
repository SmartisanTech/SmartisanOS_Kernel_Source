...
static void handle_rx(struct uart_port *port, unsigned int misr)
{
	...
	while (count > 0) {
		...
		break_event = sr & UARTDM_SR_RX_BREAK_BMSK;
		if (break_event) {
			port->icount.brk++;
		...
		/* handle sysrq:
		 * on this uart controller, the press of sysrq(ctrl + break) actually
		 * will not trigger irq until the "command" key pressed. so here the
		 * uart_handle_sysrq_char() will always invoke handle_sysrq(ch).
		 * */
		if (break_event) {
			uart_handle_sysrq_char(port, c);
			/* the error status is sticky so clean it */
