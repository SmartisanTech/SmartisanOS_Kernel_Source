...
static const struct qpnp_vadc_map_pt adcmap_backup_btm_threshold[] = {
	{-300,	1663},
	{-200,	1565},
	{-100,	1426},
	{0,	1248},
...
int32_t qpnp_adc_scale_backup_batt_therm(struct qpnp_vadc_chip *chip,
		int32_t adc_code,
		const struct qpnp_adc_properties *adc_properties,
		const struct qpnp_vadc_chan_properties *chan_properties,
		struct qpnp_vadc_result *adc_chan_result)
{
	...
	return qpnp_adc_map_temp_voltage(
			adcmap_backup_btm_threshold,
			ARRAY_SIZE(adcmap_backup_btm_threshold),
			bat_voltage,
			&adc_chan_result->physical);
}
EXPORT_SYMBOL(qpnp_adc_scale_backup_batt_therm);
