...
extern void log_wakeup_reason(int,const char *);
static void gic_show_resume_irq(struct gic_chip_data *gic)
{
	...
	for (i = find_first_bit((unsigned long *)pending, gic->irq_nr);
		...
		if (desc == NULL)
			...
			log_wakeup_reason(irq,name);
		}
		else if (513 == i)
			name = "pmic controller";//ee0_apps_hlos_spmi_periph_irq, please view kernel/drivers/irqchip/qcom/pdc-sm8150.c

		pr_err("%s: %d triggered %s\n", __func__, irq, name);
		if(strstr(name,"glink")){
