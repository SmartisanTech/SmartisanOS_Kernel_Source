...
void subSystem_sleep_statics(uint32_t *apssSleep,uint32_t *mpssSleep,uint32_t *slpiSleep){
	struct msm_rpmh_master_stats *record = NULL;
	...
	uint64_t accumulated_duration;
	mutex_lock(&rpmh_stats_mutex);
	record = (struct msm_rpmh_master_stats *) qcom_smem_get(rpmh_masters[0].pid,rpmh_masters[0].smem_id, &size);
	if (!IS_ERR_OR_NULL(record)){
		accumulated_duration = record->accumulated_duration;
		if (record->last_entered > record->last_exited)
			accumulated_duration +=	(arch_counter_get_cntvct() - record->last_entered);

		*mpssSleep = (uint32_t) (accumulated_duration/1000000);
		}
	else
		*mpssSleep=0;

	/* adsp always error, so don't read rpmh_masters[2] */
	record = (struct msm_rpmh_master_stats *) qcom_smem_get(rpmh_masters[3].pid,rpmh_masters[3].smem_id, &size);
	if (!IS_ERR_OR_NULL(record)){
		accumulated_duration = record->accumulated_duration;
		if (record->last_entered > record->last_exited)
			accumulated_duration +=	(arch_counter_get_cntvct() - record->last_entered);
		*slpiSleep = (uint32_t) (accumulated_duration/1000000);
		}
	else
		*slpiSleep=0;
	mutex_unlock(&rpmh_stats_mutex);
	*apssSleep = apss_master_stats.accumulated_duration/1000000;
}

EXPORT_SYMBOL(subSystem_sleep_statics);
#endif
