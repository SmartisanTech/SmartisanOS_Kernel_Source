...
extern int smartisan_hwstate_set(char *, char *);
#endif
...
/* BSP-Audio: CCM1006 Start */
static int fsa4480_validate_switch_select_usb(struct fsa4480_priv *fsa_priv)
{
	u32 switch_control = 0;

	regmap_read(fsa_priv->regmap, FSA4480_SWITCH_CONTROL, &switch_control);

	if (switch_control != 0x18) {
		dev_err(fsa_priv->dev, "%s: switch DP_R/DN_L to DP/DN failed = %u\n",
				__func__, switch_control);
		return -EIO;
	}
	dev_dbg(fsa_priv->dev, "%s: switch select usb = %u\n",
			__func__, switch_control);

	return 0;
}
/* BSP-Audio: CCM1006 End */
...
static int fsa4480_usbc_analog_setup_switches(struct fsa4480_priv *fsa_priv)
{
	...
	case POWER_SUPPLY_TYPEC_NONE:
		...
		do {
			...
			if (fsa4480_validate_switch_select_usb(fsa_priv) == 0)
				break;
			/* In order to prevent I2C temporary error cause switch fail */
...
#if defined(CONFIG_FACTORY_SUPPORT)
	smartisan_hwstate_set("fsa4480", "ok");
#endif
...
#if defined(CONFIG_FACTORY_SUPPORT)
	smartisan_hwstate_set("fsa4480", "fail");
#endif
	return rc;
}
