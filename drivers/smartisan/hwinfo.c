...
#define VERSION_ID_GPIO 21
#define GPIO_21 21
#define GPIO_66 66
#define GPIO_9  9
#define GPIO_8  8
#define GPIO_VALUE_GET(v) gpiod_get_value_cansleep(gpio_to_desc(v))
#define board_id_get(a, b, c, d)			                   \
({									   \
        int __v;                                                           \
        __v = GPIO_VALUE_GET(a) << 3 |  GPIO_VALUE_GET(b) << 2  |          \
              GPIO_VALUE_GET(c) << 1 |  GPIO_VALUE_GET(d);                 \
	__v;								   \
})
...
static boardid_match_t board_table[] = {
    ...
    { .index = 13, .hw_version = "PVT",        .qcn_type = "no-ca"      , .model = "primary"    },
    { .index = 14, .hw_version = "PVT",        .qcn_type = "no-ca"      , .model = "standard"   },
...
static int set_version_id(void)
{
    ...
    return sprintf(hwinfo[version_id].hwinfo_buf, "%s:%s:%02d", board_table[id].model,
...
static int __init hwinfo_init(void)
{
	...
	printk(KERN_INFO "hwinfo_value=0x%08x, lpddr_mid=%#x emmc_cap=%#x emmc_mid=%#x\n", \
