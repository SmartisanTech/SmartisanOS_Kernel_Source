...
static hw_version_t hw_version_table[] = {
    ...
    { .index = 0x6,  .hw_version = "PVT"	},
    { .index = 0x7,  .hw_version = "MP"	},
};
...
#endif
	case DEVSERIAL:
    		return sprintf(buf, "%s",  ((i>=HWINFO_MAX || hwinfo[i].hwinfo_buf[0]=='\0')?"unknown":hwinfo[i].hwinfo_buf));
...
//Smartisan BSP gaosc add for ocean no nfc begin
#ifdef CONFIG_NFC_NQ 
	nfc_support_value = 1;     		//trident support nfc
#else
	nfc_support_value = 0;//ocean not support nfc
#endif
//Smartisan BSP gaosc add for ocean no nfc end
	sprintf(hwinfo[nfc_support].hwinfo_buf,"%d", nfc_support_value);

	if (hw_version_id == 0) { // DVT1
		if (product_version_id < 0x02)
			ufs2_support_value = 0;
		else
			ufs2_support_value = 1;
	}
	else if (product_version_id < 0x03)
		ufs2_support_value = 0;
	else
		ufs2_support_value = 1;
...
//device serial from register 0x784138
	
extern uint32_t socinfo_get_serial_number(void);

static int get_devserial(void)
{	
	u32 devserial_val ;
		
	devserial_val = socinfo_get_serial_number();
	printk("get_devserial value=%x\r\n",devserial_val);
	sprintf(hwinfo[DEVSERIAL].hwinfo_buf, "%x", devserial_val);
	return 1;
...
static int __init hwinfo_init(void)
{
	...
	/*cpu_type*/
	sprintf(hwinfo[CPU_TYPE].hwinfo_buf,"%s", of_flat_dt_get_machine_name());
