...
#define PWM_PATTERN_DEFAULT_NS  10000
/* Used for user patterns. */
static struct pwm_output_pattern *output_pattern = NULL;
static u64 duty_pattern[LPG_LUT_COUNT_MAX];
				...
				struct pwm_setting *pwm)
{
	...
	/* Use default pattern in PWM device,unless user effect is valid. */
						...
						const char *buf, size_t count)
{
	...
	char *token;
	u64 val;
	char *pattern_buf = (char *) buf;

	if (pattern_buf == NULL || count == 0)
		return -EINVAL;
	...
	output_pattern = kmalloc(sizeof(struct pwm_output_pattern), GFP_KERNEL);
	if(output_pattern == NULL)
		return -ENOMEM;
	...
	/* Get new patterns. */
	memset(duty_pattern, 0, sizeof(duty_pattern));
	while ((token = strsep(&pattern_buf, " ")) != NULL) {
		rc= sscanf(token, "%d", &val);
		if (rc < 0) {
			rc = -EINVAL;
		goto err;
		}
		duty_pattern[index] = (val & 0xffff) * PWM_PATTERN_DEFAULT_NS;
		index++;
		if (index >= LPG_LUT_COUNT_MAX)
		{
			index = LPG_LUT_COUNT_MAX;
			break;
		}
	}

	output_pattern->duty_pattern = duty_pattern;
	output_pattern->num_entries = index;

	if(led->led_setting.step_time == 0)
		output_pattern->cycles_per_duty = 100;
	else
		output_pattern->cycles_per_duty = led->led_setting.step_time;

	led->pwm_setting.output_pattern = output_pattern;
err:
	return (rc < 0) ? rc : count;
}

static ssize_t pattern_step_time_ms_show(struct device *dev, struct device_attribute *attr,
                                    char *buf)
{
	struct led_classdev *led_cdev = dev_get_drvdata(dev);
	struct qpnp_led_dev *led =
	...
	return snprintf(buf, PAGE_SIZE, "%d\n", led->led_setting.step_time);
}

static ssize_t pattern_step_time_ms_store(struct device *dev, struct device_attribute *attr,
                                const char *buf, size_t count)
{
	struct led_classdev *led_cdev = dev_get_drvdata(dev);
	struct qpnp_led_dev *led =
	...
	if (sscanf(buf, "%d", &step_time) != 1) {
		pr_err("sccanf buf error!\n");
		return count;
	}
	led->led_setting.step_time = step_time;
	pr_info("[%s->%d] step_time:%d ms\n",  __FUNCTION__, __LINE__, led->led_setting.step_time, count);
	return count;
}

static DEVICE_ATTR(breath, 0644, breath_show, breath_store);
static DEVICE_ATTR(pattern, 0644, NULL, pattern_store);
static DEVICE_ATTR(pattern_step_time_ms, 0644, pattern_step_time_ms_show, pattern_step_time_ms_store);
static const struct attribute *breath_attrs[] = {
	&dev_attr_breath.attr,
	&dev_attr_pattern.attr,
	&dev_attr_pattern_step_time_ms.attr,
	NULL
};
