...
static int fg_gen4_get_prop_capacity_raw(struct fg_gen4_chip *chip, int *val)
{
	...
	if (rc < 0) {
		pr_err("Error in getting raw_msoc, rc=%d\n", rc);
		return rc;
	}
	...
	rc = fg_get_sram_prop(fg, FG_SRAM_MONOTONIC_SOC, val);
	if (rc < 0) {
	...
	if (*val > (0x10000 - HIDEN_SOC_RAW*0x100))
		*val = 10000;
	else if (*val < 0x600)
		*val = 100 + *val*100/0x600;
	else if (*val == 0)
		*val = 0;
	else if(msoc % 2)
		*val = msoc*100 + (*val - msoc/2*HIDEN_SOC_RAW*0x100 - 0x400)* 100 / 0x200;
	else
		*val = msoc*100 + (*val - msoc/2*HIDEN_SOC_RAW*0x100 - 0x100)* 100 / 0x300;
...
static int fg_gen4_probe(struct platform_device *pdev)
{
	...
	schedule_delayed_work(&fg->update_soc_work, msecs_to_jiffies(500));
...
static int fg_gen4_resume(struct device *dev)
{
	...
	schedule_delayed_work(&fg->update_soc_work, msecs_to_jiffies(100));
