...
static const struct apsd_result *smblib_update_usb_type(struct smb_charger *chg)
{
	...
	static enum power_supply_type pst_last;
	...
	if (apsd_result->pst == pst_last)
		return apsd_result;

	pst_last = apsd_result->pst;
                ...
                unsigned long event, void *data)
{
	...
	if (chg && (event == MSM_DRM_EVENT_BLANK)) {
		...
		if (blank == MSM_DRM_BLANK_UNBLANK) {
			cancel_delayed_work_sync(&chg->panel_status_work);
			chg->panel_on = true;
			schedule_delayed_work(&chg->panel_status_work, 15*HZ);
		} else if (blank == MSM_DRM_BLANK_POWERDOWN) {
			cancel_delayed_work_sync(&chg->panel_status_work);
			chg->panel_on = false;
			schedule_delayed_work(&chg->panel_status_work, 0);
		} else {
...
static int smblib_register_notifier(struct smb_charger *chg)
{
	...
	if (rc < 0) {
		smblib_err(chg, "Couldn't register dsi panel notifier client rc=%d\n",rc);
		return rc;
	}
...
int smblib_set_icl_current(struct smb_charger *chg, int icl_ua)
{
	...
	smblib_dbg(chg, PR_OEM, "icl_ua value is: %d\n", icl_ua);
	...
	if (chg->real_charger_type == POWER_SUPPLY_TYPE_USB
		...
		if (icl_ua <= USBIN_500MA) {
			rc = set_sdp_current(chg, icl_ua);
			if (rc >= 0 && chg->real_charger_type == POWER_SUPPLY_TYPE_USB)
				goto unsuspend;
		}
		...
		if ((chg->real_charger_type == POWER_SUPPLY_TYPE_USB_HVDCP_3
			...
			&& icl_ua < USBIN_900MA) {
			icl_ua = USBIN_900MA;
			smblib_dbg(chg, PR_OEM, "real icl_ua value is: %d\n", icl_ua);
		}
				...
				bool enable)
{
	...
	smblib_dbg(chg, PR_OEM, "Moisture protection %s\n",
				...
				union power_supply_propval *val)
{
	...
	case DISABLE_CHARGE:
	case PAUSE_CHARGE:
		if (batt_health == POWER_SUPPLY_HEALTH_WARM && usb_online)
			val->intval = POWER_SUPPLY_STATUS_CHARGING;
		else
			val->intval = POWER_SUPPLY_STATUS_NOT_CHARGING;
#ifdef CONFIG_FACTORY_SUPPORT
		if ((get_client_vote(chg->usb_icl_votable, USER_VOTER) == 0) && pval.intval)
			val->intval = POWER_SUPPLY_STATUS_CHARGING;
#endif
		break;
	default:
				...
				const union power_supply_propval *val)
{
	...
	if (chg->thermal_levels_panel_on <= 0)
		return -EINVAL;
	...
	if (chg->panel_on) {
		target_current = chg->thermal_mitigation_panel_on[chg->system_temp_level] + chg->charge_current_offset;
	} else {
		target_current = chg->thermal_mitigation[chg->system_temp_level] + chg->charge_current_offset;
	}

	if (target_current < 0) {
		target_current = 0;
	}
	vote(chg->fcc_votable, THERMAL_DAEMON_VOTER, true, target_current);

	smblib_dbg(chg, PR_OEM, "panel on: %d, system_temp_level %d, thermal_mitigation: %d\n", chg->panel_on, chg->system_temp_level, target_current);
...
out:
	smblib_dbg(chg, PR_OEM, "Current temperatures: \tDIE_TEMP: %d,\tCONN_TEMP: %d,\tSMB_TEMP: %d,\tSKIN_TEMP: %d\nTHERMAL_STATUS: %d\n",
	...
	if (thermal_status != chg->thermal_status) {
		...
		vote(chg->pl_disable_votable, SW_THERM_REGULATION_VOTER,
							disable_smb, 0);
		smblib_dbg(chg, PR_OEM, "Parallel %s as per SW thermal regulation\n",
		...
		vote(chg->usb_icl_votable, SW_THERM_REGULATION_VOTER,
							suspend_input, 0);
		smblib_dbg(chg, PR_OEM, "USB/DC %s as per SW thermal regulation\n",
								...
								int type)
{
	...
	smblib_dbg(chg, PR_OEM, "%s Jeita threshold configured\n",
...
static int smblib_soft_jeita_arb_wa(struct smb_charger *chg)
{
	...
	smblib_dbg(chg, PR_OEM, "JEITA ARB status %d, soft JEITA status %d\n",
...
irqreturn_t usbin_collapse_irq_handler(int irq, void *data)
{
	struct smb_irq_data *irq_data = data;
	struct smb_charger *chg = irq_data->parent_data;
...
irqreturn_t usb_source_change_irq_handler(int irq, void *data)
{
	...
	if (chg->pd_active || chg->apsd_done == true)
		return IRQ_HANDLED;
...
static void smblib_handle_rp_change(struct smb_charger *chg, int typec_mode)
{
	...
	smblib_dbg(chg, PR_OEM, "CC change old_mode=%d new_mode=%d\n",
...
{
	struct smb_charger *chg = container_of(work, struct smb_charger,
						panel_status_work.work);
	union power_supply_propval val;

	smblib_dbg(chg, PR_OEM, "panel_on: %d\n", chg->panel_on);
...
int smblib_init(struct smb_charger *chg)
{
	...
	INIT_DELAYED_WORK(&chg->panel_status_work, panel_status_work);
