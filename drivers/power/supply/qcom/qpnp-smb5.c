...
static int __debug_mask = PR_OEM;
module_param_named(
...
static int smb5_parse_dt(struct smb5 *chip)
{
	...
	if (of_find_property(node, "qcom,thermal-mitigation-panel-on", &byte_len)) {
		...
		if (rc < 0) {
			dev_err(chg->dev,
				"Couldn't read threm panel on limits rc = %d\n", rc);
			return rc;
		}
	}
...
static enum power_supply_property smb5_batt_props[] = {
	...
	POWER_SUPPLY_PROP_CHARGE_CURRENT_OFFSET,
};
		...
		union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_FCC_STEPPER_ENABLE:
		...
		break;
	case POWER_SUPPLY_PROP_CHARGE_CURRENT_OFFSET:
		val->intval = chg->charge_current_offset;
		break;
	default:
		...
		const union power_supply_propval *val)
{
	int rc = 0;
	struct smb_charger *chg = power_supply_get_drvdata(psy);
	union power_supply_propval val_tmp;
	...
	case POWER_SUPPLY_PROP_FCC_STEPPER_ENABLE:
		...
		break;
	case POWER_SUPPLY_PROP_CHARGE_CURRENT_OFFSET:
		chg->charge_current_offset = val->intval;
		val_tmp.intval = chg->system_temp_level;
		smblib_set_prop_system_temp_level(chg, &val_tmp);
		break;
	default:
		rc = -EINVAL;
	}
...
static int smb5_init_hw(struct smb5 *chip)
{
	...
	smblib_masked_write(chg, HVDCP_PULSE_COUNT_MAX_REG, HVDCP_PULSE_COUNT_QC3P0_MASK, QC3_PLUSE_9V);

#ifdef CONFIG_FACTORY_SUPPORT
	smblib_masked_write(chg, HVDCP_PULSE_COUNT_MAX_REG, HVDCP_PULSE_COUNT_QC3P0_MASK, QC3_PLUSE_8V);
	rc = smblib_write(chg, CHGR_SAFETY_TIMER_ENABLE_CFG,
					0);
	if (rc < 0) {
		dev_err(chg->dev, "Couldn't set CHGR_FAST_CHARGE_SAFETY_TIMER_CFG_REG rc=%d\n",
			rc);
	...
	smblib_masked_write(chg, HVDCP_PULSE_COUNT_MAX_REG, HVDCP_PULSE_COUNT_MAX_QC2_MASK, HVDCP_PULSE_COUNT_MAX_QC2_9V);
	...
	rc = smblib_write(chg, TYPEC_TYPE_C_DEBUG_ACCESS_SNK_CFG,
				 EN_DEBUG_ACCESS_SNK | EN_CHG_ON_DEBUG_ACCESS_SNK);
	if (rc < 0) {
		dev_err(chg->dev, "Couldn't set TYPEC_TYPE_C_DEBUG_ACCESS_SNK_CFG rc=%d\n",
			rc);
		return rc;
	}
...
static struct smb_irq_info smb5_irqs[] = {
	...
	[USBIN_COLLAPSE_IRQ] = {
		.name		= "usbin-collapse",
		.handler	= usbin_collapse_irq_handler,
	},
	[USBIN_VASHDN_IRQ] = {
		.name		= "usbin-vashdn",
		.handler	= default_irq_handler,
