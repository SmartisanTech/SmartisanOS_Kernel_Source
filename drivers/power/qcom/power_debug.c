...
#define APQ_NR_GPIOS		142
//#define APQ_GPIO_CFG(n)		(0xffffff8000089000 + (0x10 * n))
//#define APQ_GPIO_IN_OUT(n)	(0xffffff8000089004 + (0x10 * n))

#define APQ_GPIO_CFG(n)		(GPIOMAPBASE + (0x1000 * n))
#define APQ_GPIO_IN_OUT(n)	(GPIOMAPBASE+4 + (0x1000 * n))
...
static void pm8953_ldo_store(struct device *ssbi_dev, void *data, size_t num);
static int   pm8953_ldo_show(struct seq_file *m, void *data, size_t num);

static void pm8953_gpiompp_store(struct device *dev, void *data, size_t num);
static int pm8953_gpiompp_show(struct seq_file *m, void *data, size_t num);
...
struct   RegProperty RegGpioMppTable[ ]=
{
	{ "GPIO1_STATUS" ,  0xC008 },
...
static struct dump_desc dump_devices[] = {
	...
	[DUMP_PM8953_LDO] = {
		.name = "pm8953_ldo",
		.store = pm8953_ldo_store,
		.show = pm8953_ldo_show,
		.dev =NULL,
	...
	[DUMP_PM8953_GPIOMPP] = {
		.name = "pm8953_gpiompp",
		.store = pm8953_gpiompp_store,
		.show = pm8953_gpiompp_show,
		.dev =NULL,
		.item_size = sizeof(struct  GpioMppProperty ),
		.item_count = sizeof(RegGpioMppTable)/sizeof(RegGpioMppTable[0]),
		},
};
/*
...
static void apq_gpio_store(struct device *unused, void *data, size_t num)
{
	...
	memset( data, sizeof(struct apq_gpio)*num, 0x00);
	for (i = 0; i < num; i++) {
                //Skip NFC pins(QUP1) in Odin(0,1,2,3)
                if( i >= 0 && i < 4 ) continue; //  
                //Skip fingerprint pins(QUP7) in Odin(135,136,137,138)
                if( i > 134 && i < 139 ) continue;
...
void pm8953_gpiompp_store(struct device *dev, void *data, size_t num)
{
	int  index;
	uint8_t sid;
	uint8_t  buf[1];
	...
	struct  GpioMppProperty  *GpioMppOnff=(struct  GpioMppProperty  *)data;

	for( index=0; index<(sizeof(RegGpioMppTable)/sizeof(struct  RegProperty)) ; index++)
      	{
		sid = (RegGpioMppTable[index].regaddr>> 16) & 0xF;
		addr = RegGpioMppTable[index].regaddr & 0xFFFF;
		...
		GpioMppOnff[index].name=RegGpioMppTable[index].regname;
		GpioMppOnff[index].Onoff_value=buf[0];
		pr_debug("%s ,  ret=%d, value=0x%x\n", __func__, ret, buf[0] );
	 }
...
int pm8953_gpiompp_show(struct seq_file *m, void *data, size_t num)
{
	int i = 0;
	struct  GpioMppProperty  *GpioMppOnff=(struct  GpioMppProperty  *)data;

	seq_printf(m, "+--+-------+-----+--pmic gpio_show: On/Off--+---+-----------+----+---+"
		"------+\n");
	seq_printf(m, "|GpioName  | On/Off \n");

	for (i = 0; i < num ; i++)
	{
		seq_printf(m, "|%-10s| on_off=0x%x\n", GpioMppOnff[i].name, GpioMppOnff[i].Onoff_value&0xFF );
	}
...
#if 0
static ssize_t current_write(struct file *file,
                        const char __user *buf, size_t count, loff_t *ppos) {
        size_t ret = 0;
        //struct dump_desc *dump_device = file->private_data;
