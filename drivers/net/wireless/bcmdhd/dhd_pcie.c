...
dhdpcie_bus_isr(dhd_bus_t *bus)
{
	...
	do {
			...
			if (msm_pcie_link_is_ready(0) == false) {
				DHD_ERROR(("%s : bus is down. PCIe link status is not ready\n",
					__FUNCTION__));
				break;
			}
#endif
...
dhdpcie_bus_suspend(struct dhd_bus *bus, bool state)
{
	...
	DHD_ERROR(("%s: set state to %s\n",
				__FUNCTION__, state ? "suspend": "resume"));
...
dhdpcie_handle_mb_data(dhd_bus_t *bus)
{
	...
	if (d2h_mb_data == 0x0 || d2h_mb_data == 0xffffffff) {
		DHD_ERROR(("%s:bus is abnormal. d2h_mb_data == 0x%x. do nothing\n",
