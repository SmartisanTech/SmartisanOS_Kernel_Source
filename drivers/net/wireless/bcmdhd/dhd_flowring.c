...
#ifdef DHD_AWDL
/* Handle awdl peer/s delete */
/* buf = NULL means delete all peers in awdl interface */
void
dhd_awdl_peer_delete(dhd_pub_t *dhdp, uint8 ifindex, void *buf, uint32 buflen)
{
	...
	if (buflen < sizeof(awdl_peer_op_t)) {
		DHD_ERROR(("%s: cannot handle awdl peer del\n", __FUNCTION__));
		return;
	}
	if (peer->opcode != AWDL_PEER_OP_DEL)
		return;
	dhd_flow_rings_delete_for_peer(dhdp, ifindex, &peer->addr.octet[0]);
}
#endif /* DHD_AWDL */
