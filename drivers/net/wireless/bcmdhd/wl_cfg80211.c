...
module_param_named(wl_dbg_level, wl_dbg_level, int, S_IRUGO | S_IWUSR | S_IWGRP);
...
#define FIRST_SCAN_ACTIVE_DWELL_TIME_MS 30
	...
	const wl_event_msg_t *e, void *data)
{
	...
	unsigned int fake_gain_value = CUSTOM_FAKE_GAIN_VALUE;
	int fake_gain_threshold = CUSTOM_FAKE_GAIN_THRESHOLD;
	...
	if (status == WLC_E_STATUS_PARTIAL) {
		...
		if (CHSPEC_IS5G(bi->chanspec) && (bi->RSSI > fake_gain_threshold)) {
			WL_INFORM(("%s("MACDBG"), Fake Gain %ddB, RSSI %ddBm\n",
				bi->SSID, MAC2STRDBG(bi->BSSID.octet), fake_gain_value, bi->RSSI));
			bi->RSSI += fake_gain_value;
		}
