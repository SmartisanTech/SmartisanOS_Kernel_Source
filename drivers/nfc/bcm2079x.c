...
static ssize_t nfc_sysfs_state_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	if (0 == ic_state)
		return snprintf(buf, PAGE_SIZE, "ic_failed\n");
	else
		return snprintf(buf, PAGE_SIZE, "ic_ok\n");
}

static struct device_attribute attrs[] = {
	__ATTR(nfc_state, S_IRUGO, nfc_sysfs_state_show, NULL),
};
...
static void set_client_addr(struct bcm2079x_dev *bcm2079x_dev, int addr)
{
	...
	pr_debug("Set client device changed to (0x%04X) flag = %04x\n",
...
static void change_client_addr(struct bcm2079x_dev *bcm2079x_dev, int addr)
{
	...
	pr_debug("Change client device from (0x%04X) flag = "\
		 "%04x, addr_data[%d] = %02x\n",
	...
	pr_debug("Change client device changed to (0x%04X) flag = %04x, ret = %d\n",
					 ...
					 unsigned int cmd, unsigned long arg)
{
	...
	case BCMNFC_POWER_CTL:
		pr_debug("%s, BCMNFC_POWER_CTL (%x, %lx):\n", __func__, cmd, arg);
		if (arg != 1)
			set_client_addr(bcm2079x_dev, bcm2079x_dev->original_address);
		if(0 == arg)
		{
	...
	case BCMNFC_WAKE_CTL:
		pr_debug("%s, BCMNFC_WAKE_CTL (%x, %lx):\n", __func__, cmd, arg);
#ifdef USE_WAKE_LOCK
		if(arg != 0)
		{
			while(wake_lock_active(&bcm2079x_dev->wake_lock ))
			{
				pr_debug("release wake lock!!!\n");
			   ...
			   const struct i2c_device_id *id)
{
	...
	if(client->dev.of_node){
		platform_data->irq_gpio = of_get_named_gpio(client->dev.of_node, "brcm,irq-gpio", 0);
		platform_data->en_gpio = of_get_named_gpio(client->dev.of_node, "brcm,pwr-gpio", 0);
		platform_data->wake_gpio = of_get_named_gpio(client->dev.of_node, "brcm,wak-gpio", 0);
	}
	else{
	...
	xtal_xop = clk_get_sys("nfc-nci", "nfc_clk");
	if (IS_ERR(xtal_xop))
	{
		dev_err(&client->dev, "nfc clk get fail\n");
		return -ENODEV;
	}

	ret = clk_prepare_enable(xtal_xop);
	if (ret)
	{
		dev_err(&client->dev, "nfc clk enable fail\n");
		return -ENODEV;
	}

	ret = gpio_request(platform_data->irq_gpio, "nfc_int");
	if (ret){
		dev_err(&client->dev, "nfc irq_gpio request fail\n");
		return -ENODEV;
	}
	ret = gpio_request(platform_data->en_gpio, "nfc_ven");
	if (ret){
		dev_err(&client->dev, "nfc en_gpio request fail\n");
		goto err_en;
	}
	ret = gpio_request(platform_data->wake_gpio, "nfc_firm");
	if (ret){
		dev_err(&client->dev, "nfc wake_gpio request fail\n");
		goto err_firm;
	}
	...
	ret = sysfs_create_file(&client->dev.kobj, &attrs[0].attr);
	if (ret) {
		dev_err(&client->dev, "sysfs_create_file failed\n");
	}
