...
{
	unsigned long flags;
    printk("---bcm bcm2079x_enable_irq\n");
	spin_lock_irqsave(&bcm2079x_dev->irq_enabled_lock, flags);
	if (!bcm2079x_dev->irq_enabled) {
...
printk("---bcm bcm2079x_dev_irq_handler\n");
...
printk("---bcm bcm2079x_dev_read read %d datas\n",total);
	return total;
}
...
static int bcm2079x_dev_open(struct inode *inode, struct file *filp)
{
	...
	dev_info(&bcm2079x_dev->client->dev,
		 "%d,%d Read RAW NFC packets, count_irq=%d\n",
...
printk("----bcm bcm2079x_dev_unlocked_ioctl cmd = %d\n",cmd);
...
static int nfc_parse_dt(struct device *dev, struct bcm2079x_platform_data *pdata)
{
	...
	ret = of_property_read_string(np, "clock-names", &pdata->clk_src);
	if (ret)
		return ret;
	dev_err(dev, "%s: bcm2079x parse dts success, clk-name=*%s* \n", __func__, pdata->clk_src);
			   ...
			   const struct i2c_device_id *id)
{
	...
	nfc_clk = clk_get(&client->dev, platform_data->clk_src);
	if (nfc_clk == NULL) {
