...
static int adc_tm5_get_temp(struct adc_tm_sensor *sensor, int *temp)
{
	...
	*temp = milli_celsius;
	...
	if (sensor->tzd != NULL && sensor->tzd->tzp != NULL) {
		*temp+= sensor->tzd->tzp->offset;
	}
	return 0;
}
					...
					int low_temp, int high_temp)
{
	...
	if ((sensor->tzd != NULL) && (sensor->tzd->tzp != NULL)) {
		if (high_temp != INT_MAX)
			high_temp-= sensor->tzd->tzp->offset;

		if (low_temp != INT_MIN)
			low_temp-= sensor->tzd->tzp->offset;
	}
...
fail:
		...
		if (!chip->sensor[i].non_thermal) {
			...
			if (chip->sensor[i].tzd->tzp != NULL) {
				of_thermal_handle_trip_temp(chip->sensor[i].tzd,
							temp + chip->sensor[i].tzd->tzp->offset);
			} else {
				of_thermal_handle_trip_temp(chip->sensor[i].tzd,
							temp);
			}
