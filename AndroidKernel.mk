...
ifeq ($(shell echo $(KERNEL_OVERLAYCONFIG) | grep vendor),)
	KERNEL_OVERLAYCONFIG := $(TARGET_KERNEL_SOURCE)/arch/$(KERNEL_ARCH)/configs/vendor/$(KERNEL_OVERLAYCONFIG)
ifeq ($(KERNEL_CONFIG_OVERRIDE),)
	KERNEL_CONFIG_OVERRIDE := $(KERNEL_OVERLAYCONFIG)
else
	KERNEL_CONFIG_OVERLAY := $(KERNEL_OVERLAYCONFIG)
endif
endif
...
$(KERNEL_CONFIG): $(KERNEL_OUT)
	...
	$(hide) if [ ! -z "$(KERNEL_CONFIG_OVERRIDE)" ]; then \
			...
			cat $(KERNEL_CONFIG_OVERRIDE) >> $(KERNEL_OUT)/.config; \
			cat $(KERNEL_CONFIG_OVERLAY) >> $(KERNEL_OUT)/.config; \
