...
s32 ffsUmountVol(struct super_block *sb)
{
	...
	if (p_fs->dev_ejected) {
		printk("[EXFAT] unmounted with media errors. device's already ejected.\n");
		return FFS_MEDIAERR;
	}
...
s32 ffsTruncateFile(struct inode *inode, u64 old_size, u64 new_size)
{
	...
	if (fid->size != old_size) {
		printk(KERN_ERR "[EXFAT] truncate : can't skip it because of size-mismatch(old:%lld->fid:%lld).\n"
				, old_size, fid->size);
	}
...
void exfat_free_cluster(struct super_block *sb, CHAIN_T *p_chain, s32 do_relse)
{
	...
	if (p_chain->size <= 0) {
		printk(KERN_ERR "[EXFAT] free_cluster : skip free-req clu:%u, because of zero-size truncation\n"
				, p_chain->dir);
		return;
	}
	...
	while (clu.dir != CLUSTER_32(~0)) {
		...
		for (i = 0; i < p_fs->dentries_per_clu; i++) {
			...
			if (ep->flags == 0x0) {
				...
				p_fs->vol_amap = kmalloc_array(p_fs->map_sectors, sizeof(struct buffer_head *), GFP_KERNEL);
		...
		for (i = 0; i < p_bd->sector_size && index <= 0xFFFF; i += 2) {
			...
			if (skip) {
				...
				if (upcase_table[col_index] == NULL) {
					DPRINTK("alloc = 0x%X\n", col_index);
					upcase_table[col_index] = kmalloc_array(UTBL_ROW_COUNT, sizeof(u16), GFP_KERNEL);
					if (upcase_table[col_index] == NULL) {
						ret = FFS_MEMORYERR;
						goto error;
					}
