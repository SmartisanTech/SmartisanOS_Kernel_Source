...
static int sdcardfs_open(struct inode *inode, struct file *file)
{
	...
	if(inode){
		if(inode->i_gid.val == AID_SDCARD_RW){
			if(file->f_flags & O_CREAT){
				struct inode * lower_inode = sdcardfs_lower_inode(inode);
				lower_inode->i_mode |= inode->i_mode;
				pr_debug("%s: changed lower_inode->i_mode = 0%o\n", __func__, lower_inode->i_mode);
			}
		}
	}
