...
static int read_package_list(struct packagelist_data *pkgl_dat) {
	...
	pr_debug(KERN_INFO "sdcardfs: read_package_list\n");
...
static int packagelist_reader(void *thread_data)
{
	...
	while (!kthread_should_stop()) {
		...
		if (!active) {
			...
			if (res < 0) {
				...
				/* Framework may not have created yet, sleep and retry */
					pr_debug(KERN_ERR "sdcardfs: missing packages.list; retrying\n");
					ssleep(2);
					pr_debug(KERN_ERR "sdcardfs: missing packages.list_end; retrying\n");
					continue;
				} else {
		...
		while (res >= (int) sizeof(*event)) {
			...
			pr_debug(KERN_INFO "sdcardfs: inotify event: %08x\n", event->mask);
